...
public class SelectionParser extends AssistParser {
    ...
    // OWNER
    static final public int SELECTION_PARSER = 1024;

    static final public int SELECTION_OR_ASSIST_PARSER = ASSIST_PARSER + SELECTION_PARSER;

    // KIND : all values known by SelectionParser are between 1025 and 1549
    // whether we are inside a block
    static final public int K_BETWEEN_CASE_AND_COLONORARROW = SELECTION_PARSER + 1;

    // whether we are between the keyword 'return' and the end of a return statement
    static final public int K_INSIDE_RETURN_STATEMENT = SELECTION_PARSER + 2;

    // whether we are between ')' and the end of a cast statement
    static final public int K_CAST_STATEMENT = SELECTION_PARSER + 3;

    // whether we are in: expression && <here>
    protected static final int K_POST_AND_AND = SELECTION_PARSER + 4;

    // whether we are in: expression || <here>
    protected static final int K_POST_OR_OR = SELECTION_PARSER + 5;

    // whether we are in an if statement
    protected static final int K_INSIDE_IF = SELECTION_PARSER + 6;

    // whether we are in the then portion of an if statement
    protected static final int K_INSIDE_THEN = SELECTION_PARSER + 7;

    // whether we are in the else portion of an if statement
    protected static final int K_INSIDE_ELSE = SELECTION_PARSER + 8;

    // whether we are in a while statement
    protected static final int K_INSIDE_WHILE = SELECTION_PARSER + 9;

    // whether we are in an while statement's body
    protected static final int K_POST_WHILE_EXPRESSION = SELECTION_PARSER + 10;

    // whether we are in a switch statement
    protected static final int K_INSIDE_STATEMENT_SWITCH = SELECTION_PARSER + 11;

    // whether we are in an expression switch
    protected static final int K_INSIDE_EXPRESSION_SWITCH = SELECTION_PARSER + 12;

    // whether we are in the guard
    protected static final int K_INSIDE_WHEN = SELECTION_PARSER + 13;

    // whether we are in a for each statement
    protected static final int K_INSIDE_FOR_EACH = SELECTION_PARSER + 14;

    // whether we are in an initialization of a field/local declaration
    protected static final int K_INSIDE_INITIALIZATION = SELECTION_PARSER + 15;

    // whether we are in an assignment statement
    protected static final int K_INSIDE_ASSIGNMENT = SELECTION_PARSER + 16;
        ...
        if (this.isOrphanCompletionNode) {
            ...
            if (orphan instanceof Expression && ((Expression) orphan).isTrulyExpression()) {
                ...
                if (lastIndexOfElement(K_LAMBDA_EXPRESSION_DELIMITER) < 0) {
                    // lambdas are recovered up to the containing expression statement and will carry along the assist node anyways.
    ...
    private void buildMoreCompletionContext(Expression expression) {
        ...
        for (int i = this.elementPtr; i > -1; i--) {
            switch(kind = this.elementKindStack[i]) {
                case K_INSIDE_WHILE:
                    if (// selection inside while (<here>)
                    whileBody == null)
                ...
                case K_INSIDE_FOR_EACH:
                    ...
                    System.arraycopy(this.astStack, newAstPtr + 1, statements, 0, length);
                    if (orphan != null)
                    ...
                    switch(kind) {
                        ...
                        case K_INSIDE_ELSE:
                            elseStat = b;
                            // orphan subsumed into the dangling else
                            orphan = null;
                            break;
                        case K_INSIDE_FOR_EACH:
                ...
                case K_POST_AND_AND:
                    ...
                    right = new AND_AND_Expression(left, right, AND_AND);
                    parentNode = orphan = right;
                    break;
                case K_POST_OR_OR:
                    left = this.expressionStack[this.elementInfoStack[i]];
                    right = new OR_OR_Expression(left, right, OR_OR);
                    parentNode = orphan = right;
                    break;
                ...
                case K_INSIDE_ASSIGNMENT:
                    if (orphan instanceof SwitchExpression) {
                        // ensure switch expression is not mistakenly seen in vanilla context
                        this.expressionLengthPtr--;
                        parentNode = orphan = new Assignment(this.expressionStack[this.expressionPtr--], (Expression) orphan, orphan.sourceEnd);
                    }
                    break;
        ...
        if ((this.astLengthStack[this.astLengthPtr] == 1) && (this.astStack[this.astPtr] == null)) {
            ...
            if (this.assistNode != null) {
                ...
                if (this.assistNode.sourceStart >= alloc.sourceStart && this.assistNode.sourceEnd <= alloc.sourceEnd)
                    // continue to parse until containing block statement is reduced, so as to
    ...
    protected void consumeCatchFormalParameter() {
        if (this.indexOfAssistIdentifier() < 0) {
            ...
            this.intPtr--;
            Argument arg = new SelectionOnArgumentName(identifierName, namePositions, type, // modifiers
    ...
    protected void consumeFieldAccess(boolean isSuperAccess) {
        ...
        this.identifierLengthPtr--;
        if (isSuperAccess) {
            //considerates the fieldReferenceerence beginning at the 'super' ....
            ...
            //optimize push/pop
            if ((fieldReference.receiver = this.expressionStack[this.expressionPtr]).isThis()) {
            ...
            if (requireExtendedRecovery()) {
                ...
                if (this.astPtr >= 0 && this.astStack[this.astPtr] == this.assistNode && this.assistNode instanceof ThisReference) {
                    ...
                    if (messageSend.receiver instanceof SingleNameReference) {
                        ...
                        if (snr.token == CharOperation.NO_CHAR) {
                            // dummy reference created by getUnspecifiedReference ???
    ...
    protected void consumeToken(TerminalToken token) {
        // before super.consumeToken tramples on it
        TerminalToken lastToken = this.previousToken;
        // before super.consumeToken tramples on it
        boolean betweenCaseAndColonOrArrow = topKnownElementKind(SELECTION_OR_ASSIST_PARSER) == K_BETWEEN_CASE_AND_COLONORARROW;
                ...
                case TokenNameLPAREN:
                    if (lastToken == TokenNameif) {
                        ...
                        if (betweenCaseAndColonOrArrow && topKnownElementKind(SELECTION_OR_ASSIST_PARSER) == K_SELECTOR) {
                            // 'case' 'ID(' is *not* the start of an invocation
    ...
    protected int cookedAstPtr() {
        for (int i = 0; i <= this.elementPtr; i++) {
            switch(this.elementKindStack[i]) {
                ...
                case K_INSIDE_WHILE:
                    if (this.assistNode != null)
                        // buried deep, needs to be dug up and attached.
                        this.isOrphanCompletionNode = true;
