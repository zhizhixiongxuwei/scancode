...
public abstract class AssistParser extends Parser {
    ...
    public boolean resumedAfterRepair = false;

    // last modifiers info
    public int lastModifiers = ClassFileConstants.AccDefault;

    public int lastModifiersStart = -1;
    ...
    // whether we are inside a module info declaration
    protected static final int K_MODULE_INFO_DELIMITER = ASSIST_PARSER + 8;

    // whether we are inside a switch expression
    protected static final int K_SWITCH_EXPRESSION_DELIMITTER = ASSIST_PARSER + 9;

    // whether we are inside record pattern
    protected static final int K_RECORD_PATTERN = ASSIST_PARSER + 10;
 ...
 */
    ...
    protected boolean triggerRecoveryUponLambdaClosure(Statement statement, boolean shouldCommit) {
        ...
        if (statement instanceof AbstractVariableDeclaration) {
            ...
            if (variable.initialization != null && variable.initialization.sourceEnd == 0) {
                // variable end is not yet known, don't make it a limiting factor
                statementEnd = Integer.MAX_VALUE;
            } else {
        ...
        if (lambdaClosed && this.currentElement != null && !(this.currentElement instanceof RecoveredField)) {
            if (!(statement instanceof AbstractVariableDeclaration)) {
                // added already as part of standard recovery since these contribute a name to the scope prevailing at the cursor.
                    ...
                    if (recoveredBlock != null) {
                        ...
                        if (parseTree != null) {
                            ...
                            if ((parseTree.sourceStart == 0 || parseTree.sourceEnd == 0) || (parseTree.sourceStart >= statementStart && parseTree.sourceEnd <= statementEnd)) {
                                ...
                                statement = null;
                            } else if (recoveredStatement instanceof RecoveredLocalVariable && statement instanceof Expression && ((Expression) statement).isTrulyExpression()) {
    ...
    protected void consumeOpenBlock() {
        ...
        if (requireExtendedRecovery()) {
            ...
            if (this.currentToken == TokenNameLBRACE && this.unstackedAct > NUM_RULES) {
                // wait for chain reductions to finish before commit.
                ...
                this.stack[this.stateStackTop++] = this.unstackedAct;
                // transition to Block ::= OpenBlock LBRACE  .BlockStatementsopt RBRACE
                this.stack[this.stateStackTop] = tAction(this.unstackedAct, this.currentToken.tokenNumber());
                commit();
	...
	stored in the identifier stack. */
		...
		should include the entire replaced source. */
        int length = this.identifierLengthStack[this.identifierLengthPtr];
        int subsetLength = modifier == ClassFileConstants.AccStatic ? // include the assistIdentifier
        index + 1 : // segments of a module name have no semantic relevance, use the entire name
        length;
            ...
            switch(token) {
                case TokenNameLPAREN:
                    if (topKnownElementKind(ASSIST_PARSER) == K_RECORD_PATTERN)
                        // after 'case' 'ID(' is *not* the start of an invocation
                        break;
 ...
 */
        ...
        switch(this.currentToken) {
            case TokenNameLBRACE:
                ...
                if (this.currentElement instanceof RecoveredInitializer) {
                    ...
                    if (this.currentElement != oldElement && topKnownElementKind(ASSIST_PARSER) != K_METHOD_DELIMITER && topKnownElementKind(ASSIST_PARSER) != K_LAMBDA_EXPRESSION_DELIMITER) {
                        pushOnElementStack(K_METHOD_DELIMITER);
                    }
                }
                break;
    ...
    protected int fallBackToSpringForward(Statement unused) {
        ...
        if (this.currentToken == TokenNameEOF) {
            ...
            if (this.referenceContext instanceof AbstractMethodDeclaration)
                // no use parsing beyond the method's body end
 ...
 */
        ...
        if (requireExtendedRecovery()) {
            if (this.unstackedAct == ERROR_ACTION) {
                ...
                if (mode == RESUME && !safeToResume()) {
                    // emergency halt to prevent infinite recovery attempts
                    mode = HALT;
                }
    ...
    private boolean safeToResume() {
        if (this.lastResumeState != null && this.noProgressSince(this.lastResumeState)) {
            if (this.resumeCount++ > 0)
                // cause an emergency halt to prevent infinite recovery attempts
                return false;
        } else {
            this.resumeCount = 0;
        }
    ...
    private boolean noProgressSince(ResumeState previous) {
        ...
        if (stateTop == previous.parser.stateStackTop && astTop == previous.parser.astPtr && exprTop == previous.parser.expressionPtr && Arrays.equals(this.stack, 0, stateTop, previous.parser.stack, 0, stateTop) && (astTop == -1 || Arrays.equals(this.astStack, 0, astTop, previous.parser.astStack, 0, astTop)) && (exprTop == -1 || Arrays.equals(this.expressionStack, 0, exprTop, previous.parser.expressionStack, 0, exprTop)))
            // Parser didn't budge.
            return true;
        ASTNode node;
        if (previous.scannerCurrentPosition == this.scanner.currentPosition && previous.currentToken == this.currentToken && astTop == previous.parser.astPtr + 1 && ((node = this.astStack[this.astPtr]) instanceof EmptyStatement) && (node.bits & ASTNode.IsUsefulEmptyStatement) == 0)
            // scanner didn't advance either, attempts to nudge recovery along, merely is producing useless empty statements ...
            return true;
        return false;
    }
    ...
    protected ASTNode wrapWithExplicitConstructorCallIfNeeded(ASTNode ast) {
        int selector;
        if (ast != null && topKnownElementKind(ASSIST_PARSER) == K_SELECTOR && ast instanceof Expression && ((Expression) ast).isTrulyExpression() && (((selector = topKnownElementInfo(ASSIST_PARSER)) == THIS_CONSTRUCTOR) || (selector == SUPER_CONSTRUCTOR))) {
