...
public class CompletionUnitStructureRequestor extends CompilationUnitStructureRequestor {
    ...
    final public ASTNode assistNode;

    final public Map<JavaElement, Binding> bindingCache;

    final public Map<Binding, JavaElement> elementCache;

    final public Map<ASTNode, JavaElement> elementWithProblemCache;
        ...
        if (reference instanceof ParameterizedSingleTypeReference) {
            ...
            if (typeArguments != null) {
                for (TypeReference typeArgument : typeArguments) {
                    if (hasEmptyName(typeArgument, assistNode))
                        return true;
                }
            }
                ...
                for (TypeReference[] typeArgument : typeArguments) {
                    if (typeArgument != null) {
                        for (int j = 0; j < typeArgument.length; j++) {
                            if (hasEmptyName(typeArgument[j], assistNode))
                                return true;
                        }
                    }
                }
