...
public class CompletionJavadocParser extends JavadocParser {
    ...
    public boolean parsingSnippet = false;
    ...
    public char[][][] levelTags = new char[3][][];
    ...
    protected Object createModuleTypeReference(TerminalToken primitiveToken, int moduleRefTokenCount) {
        ...
        if (newSize == 1) {
            // Single Type ref
            typeRef = new CompletionOnJavadocSingleTypeReference(this.identifierStack[moduleRefTokenCount], this.identifierPositionStack[moduleRefTokenCount], this.tagSourceStart, this.tagSourceEnd);
        } else if (newSize > 1) {
            // Qualified Type ref
            for (int i = moduleRefTokenCount + 1; i < this.identifierPtr; i++) {
                ...
                if (start <= this.cursorLocation && this.cursorLocation <= end) {
                    if (i == moduleRefTokenCount) {
                        this.completionNode = new CompletionOnJavadocSingleTypeReference(this.identifierStack[moduleRefTokenCount + 1], this.identifierPositionStack[moduleRefTokenCount + 1], this.tagSourceStart, this.tagSourceEnd);
                    } else {
                        char[][] tokens = new char[i][];
