...
public final class SelectionEngine extends Engine implements ISearchRequestor {
    ...
    public SelectionParser parser;

    public ISelectionRequestor requestor;
    ...
    private boolean checkSelection(char[] source, int selectionStart, int selectionEnd, boolean isModuleInfo) {
        ...
        if (selectionStart > selectionEnd) {
            ...
            if (selectionStart == selectionEnd) {
                // Widen the selection to scan -> || :: if needed. No unicode handling for now.
    ...
    public void select(ICompilationUnit sourceUnit, int selectionSourceStart, int selectionSourceEnd) {
        ...
        } catch (IndexOutOfBoundsException | AbortCompilation e) {
    ...
    private void selectFrom(Binding binding, CompilationUnitDeclaration parsedUnit, ICompilationUnit unit, boolean isDeclaration) {
        if (binding instanceof TypeVariableBinding) {
            ...
            if (this.parser.assistNode instanceof SelectionOnLambdaExpression) {
                ...
                SyntheticMethodBinding methodBinding = new SyntheticMethodBinding(lambdaExpr, CharOperation.concat(TypeConstants.ANONYMOUS_METHOD, Integer.toString(lambdaExpr.ordinal).toCharArray()), (SourceTypeBinding) lambdaExpr.binding.declaringClass);
            ...
            if (cu != null) {
                ...
                if (iClassFile instanceof ClassFile) {
                    ...
                    if (classFile.getPackageFragmentRoot() instanceof JrtPackageFragmentRoot) {
                        ...
                        try {
                            reader = BinaryTypeFactory.rawReadType(descriptor, false);
    ...
    private boolean selectDeclaration(TypeDeclaration typeDeclaration, char[] assistIdentifier, char[] packageName) {
        ...
        for (int i = 0, length = variables.length; i < length; i++) {
            if (variables[i].name == assistIdentifier) {
                ...
                this.requestor.acceptField(packageName, qualifiedSourceName, variable.name, true, variable.getBinding() != null ? variable.getBinding().computeUniqueKey() : null, this.actualSelectionStart, this.actualSelectionEnd);
                this.noProposal = false;
                return true;
            }
        }
