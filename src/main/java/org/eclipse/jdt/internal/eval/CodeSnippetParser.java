...
public class CodeSnippetParser extends Parser implements EvaluationConstants {
    ...
    public int codeSnippetStart, codeSnippetEnd;
    ...
    protected void consumeMethodDeclaration(boolean isNotAbstract, boolean isDefaultMethod) {
        ...
        if (isTopLevelType()) {
            ...
            if (last >= 0 && methodDecl.statements[last] instanceof Expression && ((Expression) methodDecl.statements[last]).isTrulyExpression()) {
 ...
 */
    ...
    protected int resumeOnSyntaxError() {
        if (this.diet || this.hasRecoveredOnExpression) {
            // no reentering inside expression recovery
            return HALT;
        }
