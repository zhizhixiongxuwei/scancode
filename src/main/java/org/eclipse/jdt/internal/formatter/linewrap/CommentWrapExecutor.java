...
public class CommentWrapExecutor extends TokenTraverser {
    ...
    final public ArrayList<Token> nlsTags = new ArrayList<>();
    ...
    public int wrapMultiLineComment(Token commentToken, int startPosition, boolean simulate, boolean noWrap) {
        ...
        if (commentToken.tokenType != TokenNameCOMMENT_MARKDOWN) {
            this.newLinesAtBoundries = commentToken.tokenType == TokenNameCOMMENT_JAVADOC ? this.options.comment_new_lines_at_javadoc_boundaries : this.options.comment_new_lines_at_block_boundaries;
        } else {
