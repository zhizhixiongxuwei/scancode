...
public class WrapPreparator extends ASTVisitor {
    ...
    static final public Map<Operator, Integer> OPERATOR_PRECEDENCE;

    static final public Map<Operator, ToIntFunction<DefaultCodeFormatterOptions>> OPERATOR_WRAPPING_OPTION;

    static final public Map<Operator, Predicate<DefaultCodeFormatterOptions>> OPERATOR_WRAP_BEFORE_OPTION;
     ...
     */
    final static public float PREFERRED = 7f / 8;

    final public TokenManager tm;
    ...
    public boolean visit(RecordDeclaration node) {
        ...
        int rParen = this.tm.firstIndexAfter(components.isEmpty() ? node.getName() : components.get(components.size() - 1), TokenNameRPAREN);
    ...
    public boolean visit(MethodDeclaration node) {
        ...
        if (!parameters.isEmpty() || receiverType != null) {
            ...
            this.wrapGroupEnd = this.tm.lastIndexIn(parameters.isEmpty() ? receiverType : parameters.get(parameters.size() - 1), ANY);
    ...
    public boolean visit(EnumDeclaration node) {
        ...
        if (!enumConstants.isEmpty()) {
            for (EnumConstantDeclaration constant : enumConstants) this.wrapIndexes.add(this.tm.firstIndexIn(constant, ANY));
            this.wrapParentIndex = (this.options.alignment_for_enum_constants & Alignment.M_INDENT_ON_COLUMN) > 0 ? this.tm.firstIndexBefore(enumConstants.get(0), TokenNameLBRACE) : this.tm.firstIndexIn(node, TokenNameenum);
    ...
    public boolean visit(MethodInvocation node) {
        ...
        if (isInvocationChainRoot) {
            ...
            if (this.options.align_selector_in_method_invocation_on_expression_first_line && (this.options.alignment_for_selector_in_method_invocation & Alignment.M_INDENT_ON_COLUMN) == 0) {
    ...
    private void handleFieldAccess(Expression node) {
        ...
        this.wrapGroupEnd = isFollowedByInvocation ? this.tm.lastIndexIn(node.getParent(), ANY) : new FieldAccessAdapter(node).getIdentifierIndex(this.tm);
    ...
    private void findTokensToWrap(InfixExpression node, boolean wrapBeforeOperator, int depth) {
        ...
        this.wrapIndexes.isEmpty() || !wrapBeforeOperator) {
    ...
    public boolean visit(ConditionalExpression node) {
        boolean chainsMatter = (this.options.alignment_for_conditional_expression_chain & Alignment.SPLIT_MASK) != Alignment.M_NO_ALIGNMENT;
        boolean isNextInChain = node.getParent() instanceof ConditionalExpression && node == ((ConditionalExpression) node.getParent()).getElseExpression();
    ...
    public boolean visit(ArrayInitializer node) {
        ...
        if (this.options.brace_position_for_array_initializer.equals(DefaultCodeFormatterConstants.NEXT_LINE_SHIFTED) && openingBrace.getWrapPolicy() == null && (node.getParent() instanceof SingleMemberAnnotation || node.getParent() instanceof MemberValuePair)) {
    ...
    private void handleOneLineEnforced(ASTNode node, List<Statement> statements) {
        ...
        if (areKeptOnOneLine(openBraceIndex, closeBraceIndex)) {
            for (Statement statement : statements) this.wrapIndexes.add(this.tm.firstIndexIn(statement, ANY));
            ...
            this.tm.get(closeBraceIndex).setWrapPolicy(new WrapPolicy(WrapMode.TOP_PRIORITY, openBraceIndex, closeBraceIndex, 0, this.currentDepth, 1, false, false));
            this.currentDepth--;
        }
    }

    private boolean areKeptOnOneLine(int startIndex, int endIndex) {
        return this.tm.stream().skip(startIndex + 1).limit(endIndex - startIndex - 1).allMatch(t -> t.getLineBreaksBefore() == 0 && t.getLineBreaksAfter() == 0);
    }
    ...
    public boolean visit(SingleVariableDeclaration node) {
        handleAnnotations(node.modifiers(), node.getParent() instanceof EnhancedForStatement ? this.options.alignment_for_annotations_on_local_variable : this.options.alignment_for_annotations_on_parameter);
        return true;
    }
    ...
    public boolean visit(SwitchCase node) {
        if (node.isSwitchLabeledRule()) {
            List<Statement> siblings = node.getParent() instanceof SwitchStatement ? ((SwitchStatement) node.getParent()).statements() : ((SwitchExpression) node.getParent()).statements();
            Statement nextStatement = siblings.get(siblings.indexOf(node) + 1);
            if (!(nextStatement instanceof Block) || areKeptOnOneLine(this.tm.firstIndexIn(nextStatement, ANY), this.tm.lastIndexIn(nextStatement, ANY))) {
        ...
        handleWrap(node.isSwitchLabeledRule() ? this.options.alignment_for_expressions_in_switch_case_with_arrow : this.options.alignment_for_expressions_in_switch_case_with_colon);
        return true;
    }
    ...
    void forceContinuousWrapping(ASTNode node, int parentIndex) {
        ...
        for (int i = from; i <= to; i++) {
            Token token = this.tm.get(i);
            if ((token.getLineBreaksBefore() > 0 || (previous != null && previous.getLineBreaksAfter() > 0)) && (token.getWrapPolicy() == null || token.getWrapPolicy().wrapMode == WrapMode.BLOCK_INDENT)) {
    ...
    private void doHandleWrap(int wrappingOption, boolean wrapPreceedingComments, ASTNode parentNode) {
        ...
        while (this.tm.get(this.wrapParentIndex).isComment() && this.wrapParentIndex > 0) this.wrapParentIndex--;
        ...
        if (existing != null && existing.wrapMode == WrapMode.TOP_PRIORITY) {
            ...
            policy = new WrapPolicy(WrapMode.TOP_PRIORITY, policy.wrapParentIndex, this.wrapGroupEnd, policy.extraIndent, policy.structureDepth, policy.penaltyMultiplier, true, policy.indentOnColumn);
        }
    ...
    private WrapPolicy getWrapPolicy(int wrappingOption, float penaltyMultiplier, boolean isFirst, ASTNode parentNode) {
        ...
        if (indentOnColumn) {
            ...
            this.wrapParentIndex = this.tm.firstIndexIn(parentNode, ANY);
    ...
    private void preserveExistingLineBreaks() {
        ...
        } else if ((this.kind & (CodeFormatter.K_COMPILATION_UNIT | CodeFormatter.K_MODULE_INFO)) != 0 && this.options.insert_new_line_at_end_of_file_if_missing) {
            last.breakAfter();
        }
    }

    int getLineBreaksToPreserve(Token token1, Token token2) {
        if ((token1 != null && !token1.isPreserveLineBreaksAfter()) || (token2 != null && !token2.isPreserveLineBreaksBefore())) {
            return 0;
        }
        if (token1 != null) {
    ...
    private void handleParenthesesPositions(int openingParenIndex, int closingParenIndex, String positionsSetting) {
        ...
        switch(positionsSetting) {
            ...
            case DefaultCodeFormatterConstants.SEPARATE_LINES_IF_WRAPPED:
                if (isEmpty)
                    break;
                this.tm.get(openingParenIndex + 1).setWrapPolicy(new WrapPolicy(WrapMode.TOP_PRIORITY, openingParenIndex, closingParenIndex, this.options.indentation_size, this.currentDepth, 1, true, false));
                this.tm.get(closingParenIndex).setWrapPolicy(new WrapPolicy(WrapMode.TOP_PRIORITY, openingParenIndex, closingParenIndex, 0, this.currentDepth, 1, false, false));
                break;
            ...
            default:
                //$NON-NLS-1$
                throw new IllegalArgumentException("Unrecognized parentheses positions setting: " + positionsSetting);
        }
    }
}
