...
public class TextEditsBuilder extends TokenTraverser {
    ...
    final public StringBuilder buffer;

    final public List<Token> stringLiteralsInLine = new ArrayList<>();
    ...
    private List<IRegion> adaptRegions(List<IRegion> givenRegions) {
        ...
        for (IRegion region : givenRegions) {
            ...
            if (start > sourceStart) {
                ...
                if ((token.tokenType == TokenNameCOMMENT_BLOCK || token.tokenType == TokenNameCOMMENT_JAVADOC || token.tokenType == TokenNameCOMMENT_MARKDOWN) && start <= token.originalEnd) {
                    start = token.originalStart;
                }
            }
            if (end > start && end > sourceStart) {
                Token token = this.tm.get(this.tm.findIndex(end, ANY, false));
                if ((token.tokenType == TokenNameCOMMENT_BLOCK || token.tokenType == TokenNameCOMMENT_JAVADOC || token.tokenType == TokenNameCOMMENT_MARKDOWN) && end < token.originalEnd) {
            ...
            if (getLineBreaksBefore() > 1) {
                ...
                if (this.options.indent_empty_lines && token.tokenType != TokenNameNotAToken) {
                    if (index == 0) {
                        ...
                        boolean isBlockIndent = token.getWrapPolicy() != null && token.getWrapPolicy().wrapMode == WrapMode.BLOCK_INDENT;
        ...
        for (int i = this.counter; i < searchLimit; i++) {
            ...
            if (c == markerChar) {
                if (!isMarkdown)
                    this.buffer.append(' ');
                flushBuffer(i);
                while (i + 1 < this.sourceLimit && this.source.charAt(i + 1) == markerChar) i++;
                this.counter = i + 1;
