    ...
    public String format(String string, int indentLevel, int[] positions, String lineSeparator) {
        ...
        if (formatterClearBlankLines != null) {
            if (JavaCore.PRESERVE_ONE.equals(formatterClearBlankLines)) {
                //$NON-NLS-1$
                newOptions.put(DefaultCodeFormatterConstants.FORMATTER_NUMBER_OF_EMPTY_LINES_TO_PRESERVE, "1");
            } else {
                //$NON-NLS-1$
                newOptions.put(DefaultCodeFormatterConstants.FORMATTER_NUMBER_OF_EMPTY_LINES_TO_PRESERVE, "0");
            }
        }
