...
public class WrapExecutor {
    ...
    private class LineAnalyzer extends TokenTraverser {
        ...
        protected boolean token(Token token, int index) {
            ...
            } else if (isSpaceBefore() && getLineBreaksBefore() == 0 && index > 0 && token.tokenType != TokenNameCOMMENT_LINE) {
                this.counter++;
            }
            if (token.tokenType == TokenNameTextBlock) {
                ...
                if (lines == null) {
                    ...
                    lines.stream().skip(1).forEach(e -> this.lineWidthExtent = Math.max(this.lineWidthExtent, this.counter + this.tm2.getLength(e, this.counter)));
            ...
            boolean isLineEnd = getLineBreaksAfter() > 0 || getNext() == null || (getNext().isNextLineOnWrap() && this.tm2.get(this.tm2.findFirstTokenInLine(index)).isWrappable());
            return !isLineEnd;
        }
    ...
    private class WrapsApplier extends TokenTraverser {
        ...
        protected boolean token(Token token, int index) {
            if (index == 0 || getLineBreaksBefore() > 0) {
                newLine(token, index);
            } else if ((this.nextWrap != null && index == this.nextWrap.wrapTokenIndex) || checkForceWrap(token, index, this.currentIndent) || (token.isNextLineOnWrap() && this.tm2.get(this.tm2.findFirstTokenInLine(index)).isWrappable())) {
                token.breakBefore();
                newLine(token, index);
            } else {
        ...
        private void newLine(Token token, int index) {
            while (!this.stack.isEmpty() && index > this.stack.peek().getWrapPolicy().groupEndIndex) this.stack.pop();
        ...
        private boolean isFixedLineStart(Token token, int index) {
            ...
            boolean inDisableFormat = this.tm2.getDisableFormatTokenPairs().stream().anyMatch(p -> p[0].originalStart <= start && p[1].originalStart >= start);
            if (inDisableFormat)
                return false;
            return true;
        }
    ...
    private class NLSTagHandler extends TokenTraverser {
        ...
        protected boolean token(final Token token, final int index) {
            ...
            if (getLineBreaksAfter() > 0 || getNext() == null) {
                ...
                if (structure.isEmpty() || (structure.size() == 1 && structure.get(0).tokenType == TokenNameWHITESPACE)) {
            ...
            TokenTraverser traverser = new TokenTraverser() {
                ...
                protected boolean token(Token token, int index) {
                    ...
                    if (policy != null && policy.wrapParentIndex > changeIndex) {
                        WrapPolicy changedWp = this.policyCache.computeIfAbsent(policy, p -> new WrapPolicy(p.wrapMode, p.wrapParentIndex + delta, p.groupEndIndex == -1 ? -1 : p.groupEndIndex + delta, p.extraIndent, p.structureDepth, p.penaltyMultiplier, p.isFirstInGroup, p.indentOnColumn));
    ...
    final static public int[] EMPTY_ARRAY = {};

    final public HashMap<WrapInfo, WrapResult> wrapSearchResults = new HashMap<>();

    final public ArrayDeque<WrapInfo> wrapSearchStack = new ArrayDeque<>();

    final public LineAnalyzer lineAnalyzer;
    ...
    WrapResult findWrapsCached(final int startTokenIndex, final int indent) {
        ...
        if (this.wrapSearchStack.size() > 1 && cacheMissAllowed)
            // cache miss, need to find wraps later in main stack processing
            return null;
        ...
        while (true) {
            ...
            if (wrapResult != null) {
                ...
                while (this.wrapSearchStack.peek() != item) reverseStackTemp.add(this.wrapSearchStack.pop());
                for (WrapInfo item2 : reverseStackTemp) this.wrapSearchStack.push(item2);
    ...
    private WrapResult findWraps(int wrapTokenIndex, int indent) {
        ...
        for (int i = lastIndex; firstPotentialWrap >= 0 && i >= firstPotentialWrap; i--) {
            ...
            WrapPolicy wrapPolicy = token.getWrapPolicy();
            if (!token.isWrappable() || (activeTopPriorityWrap >= 0 && i != activeTopPriorityWrap) || policiesTried.contains(wrapPolicy) || wrapPolicy.structureDepth >= depthLimit || !isWrapInsideFormatRegion(i))
                continue;
            ...
            boolean isBetter = totalExtraPenalty < bestExtraPenalty || i == activeTopPriorityWrap || (bestNextWrap < 0 && wrapRequired);
    ...
    boolean isWrapInsideFormatRegion(int tokenIndex) {
        ...
        return this.regions.stream().anyMatch(r -> (pos1 >= r.getOffset() && pos1 < r.getOffset() + r.getLength()) || (pos2 >= r.getOffset() && pos2 < r.getOffset() + r.getLength()));
    }
