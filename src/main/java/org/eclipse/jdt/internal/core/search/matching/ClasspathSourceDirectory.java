...
public class ClasspathSourceDirectory extends ClasspathLocation implements IModulePathEntry {
    ...
    final public IContainer sourceFolder;

    final public Map<String, Map<String, IResource>> directoryCache = new ConcurrentHashMap<>();

    static final public Map<String, IResource> missingPackageHolder = new HashMap<>();

    final public char[][] fullExclusionPatternChars;

    final public char[][] fulInclusionPatternChars;
    ...
    Map<String, IResource> directoryTable(String qualifiedPackageName) {
        ...
        if (dirTable == missingPackageHolder)
            return null;
        if (dirTable != null)
            return dirTable;
        try {
                ...
                for (IResource m : members) {
                    ...
                    if (m.getType() == IResource.FILE) {
                        ...
                        if (index >= 0) {
                            ...
                            if (!org.eclipse.jdt.internal.compiler.util.Util.isExcluded(fullPath.toCharArray(), this.fulInclusionPatternChars, this.fullExclusionPatternChars, false)) {
