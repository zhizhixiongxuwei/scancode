...
public class HierarchyResolver implements ITypeRequestor {
    ...
    public ReferenceBinding focusType;

    public boolean superTypesOnly;
    ...
    public void accept(ICompilationUnit sourceUnit, AccessRestriction accessRestriction) {
        if (CharOperation.equals(TypeConstants.MODULE_INFO_NAME, sourceUnit.getMainTypeName())) {
            ...
            // work done inside checkAndSetImports()
            this.lookupEnvironment.completeTypeBindings(parsedUnit, true, false);
        } else {
            this.lookupEnvironment.problemReporter.abortDueToInternalError(new StringBuilder(Messages.accept_cannot).append(sourceUnit.getFileName()).toString());
        }
    }
    ...
    public void accept(ISourceType[] sourceTypes, PackageBinding packageBinding, AccessRestriction accessRestriction) {
        ...
        SourceTypeConverter.MEMBER_TYPE | SourceTypeConverter.METHOD, // no need for field initialization
            ...
            if (typeBinding instanceof SourceTypeBinding) {
                ...
                if (scope != null) {
                    ...
                    if (superInterfaces != null && (length = superInterfaces.length) > (interfaceBindings == null ? 0 : interfaceBindings.length)) {
                        // check for interfaceBindings being null (see https://bugs.eclipse.org/bugs/show_bug.cgi?id=139689)
 ...
 */
    ...
    public void resolve(IGenericType suppliedType) {
        try {
            if (suppliedType.isBinaryType()) {
                ...
                BinaryTypeBinding binaryTypeBinding = this.lookupEnvironment.cacheBinaryType((IBinaryType) suppliedType, false, /*don't need field and method (bug 125067)*/
                null);
    ...
    public void resolve(Openable[] openables, HashSet localTypes, IProgressMonitor monitor) {
        ...
        try {
            ...
            for (int i = 0; i < openablesLength; i++) {
                ...
                if (openable instanceof org.eclipse.jdt.core.ICompilationUnit) {
                    ...
                    if (cu.isOpen()) {
                        ...
                        int flags = !containsLocalType ? SourceTypeConverter.MEMBER_TYPE | SourceTypeConverter.METHOD : SourceTypeConverter.FIELD_AND_METHOD | SourceTypeConverter.MEMBER_TYPE | SourceTypeConverter.LOCAL_TYPE;
                    ...
                    if (binaryType != null) {
                        try {
                            sanitizeBinaryType(binaryType);
                            BinaryTypeBinding binaryTypeBinding = this.lookupEnvironment.cacheBinaryType(binaryType, false, /*don't need field and method (bug 125067)*/
                            null);
            ...
            // work-around for 1GF5W1S - can happen in case duplicates are fed to the hierarchy with binaries hiding sources
        } catch (AbortCompilation e) {
            // ignore this exception for now since it typically means we cannot find java.lang.Object
 ...
 */
    ...
    private boolean subTypeOfType(ReferenceBinding subType, ReferenceBinding typeBinding, Map<ReferenceBinding, Object> visited) {
        if (typeBinding == null || subType == null)
            return false;
        // don't evaluate the same "subType" twice
        ...
        if (subTypeOfType(superclass, typeBinding, visited))
            return true;
        ...
        if (superInterfaces != null) {
            for (ReferenceBinding si : superInterfaces) {
                ...
                if (superInterface.isHierarchyInconsistent())
                    return false;
                if (subTypeOfType(superInterface, typeBinding, visited))
                    return true;
            }
        }
        return false;
