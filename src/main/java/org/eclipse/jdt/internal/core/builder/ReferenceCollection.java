...
public class ReferenceCollection {
    ...
    public boolean insideRoot(char[] rootName) {
        ...
        if (REFERENCE_COLLECTION_DEBUG) {
            if (result != debugIncludes(rootName)) {
                String message = //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
                "Mismatch: " + String.valueOf(rootName) + (result ? " should not " : " should ") + " be included in " + Arrays.asList(CharOperation.toStrings(this.rootReferences));
    ...
    private static <T> boolean sortedArrayContains(T[] array, T element, Comparator<? super T> comparator) {
        ...
        if (l < SortedCharArrays.BINARY_SEARCH_THRESHOLD) {
            for (int i = 0; i < l; i++) if (element == array[i])
                return true;
            return false;
        }
    ...
    static final public char[][][] WellKnownQualifiedNames = new char[][][] { TypeConstants.JAVA_LANG_RUNTIMEEXCEPTION, TypeConstants.JAVA_LANG_THROWABLE, TypeConstants.JAVA_LANG_OBJECT, TypeConstants.JAVA_LANG, new char[][] { TypeConstants.JAVA }, new char[][] { new char[] { 'o', 'r', 'g' } }, new char[][] { new char[] { 'c', 'o', 'm' } }, // default package
    CharOperation.NO_CHAR_CHAR };

    static final public char[][] WellKnownSimpleNames = new char[][] { TypeConstants.JAVA_LANG_RUNTIMEEXCEPTION[2], TypeConstants.JAVA_LANG_THROWABLE[2], TypeConstants.JAVA_LANG_OBJECT[2], TypeConstants.JAVA, TypeConstants.LANG, new char[] { 'o', 'r', 'g' }, new char[] { 'c', 'o', 'm' } };
    ...
    public static char[][][] internQualifiedNames(Set<String> qualifiedStrings) {
        ...
        for (String qualifiedString : qualifiedStrings) if (qualifiedString != null)
    ...
    public static char[][] internSimpleNames(Set<String> simpleStrings, boolean removeWellKnown) {
        ...
        if (length == 0)
            return EmptySimpleNames;
        char[][] result = new char[length][];
        for (String simpleString : simpleStrings) if (simpleString != null)
    ...
    private void assertIncludes(boolean expectation, char[] simpleName) {
        if (expectation != debugIncludes(simpleName)) {
            String message = //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            "Mismatch: " + String.valueOf(simpleName) + (expectation ? " should not " : " should ") + " be included in " + Arrays.asList(CharOperation.toStrings(this.simpleNameReferences));
            throw new IllegalStateException(message);
        }
    }
    ...
    private void assertIncludes(boolean expectation, char[][] qualifiedName) {
        if (expectation != debugIncludes(qualifiedName)) {
            String message = //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            "Mismatch: " + CharOperation.toString(qualifiedName) + (expectation ? " should not " : " should ") + " be included in " + qualifiedNamesToString(this.qualifiedNameReferences);
            throw new IllegalStateException(message);
        }
    }
    ...
    private void assertIncludes(boolean expectation, char[][][] qualifiedNames, char[][] simpleNames, char[][] rootNames) {
        if (expectation != debugIncludes(qualifiedNames, simpleNames, rootNames)) {
            ...
            String.//$NON-NLS-1$
            format(//$NON-NLS-1$
            "Mismatched includes(..): ReferenceCollection([%s], %s, %s).includes([%s], %s, %s)", qualifiedNamesToString(this.qualifiedNameReferences), Arrays.toString(CharOperation.toStrings(this.simpleNameReferences)), Arrays.toString(CharOperation.toStrings(this.rootReferences)), qualifiedNamesToString(qualifiedNames), Arrays.toString(CharOperation.toStrings(simpleNames)), Arrays.toString(CharOperation.toStrings(rootNames)));
            throw new IllegalStateException(message);
        }
    }
    ...
    private boolean debugIncludes(char[][][] qualifiedNames, char[][] simpleNames, char[][] rootNames) {
        ...
        if (rootNames != null) {
            ...
            for (int i = 0, l = rootNames.length; !foundRoot && i < l; i++) foundRoot = debugInsideRoot(rootNames[i]);
            if (!foundRoot)
                return false;
        }
        if (simpleNames == null || qualifiedNames == null) {
            if (simpleNames == null && qualifiedNames == null) {
                ...
                for (char[] simpleName : simpleNames) {
                    if (debugIncludes(simpleName)) {
                        if (JavaBuilder.DEBUG)
                            //$NON-NLS-1$
                            System.out.println("Found match in well known package to " + new String(simpleName));
                        return true;
                    }
                }
            } else {
                    ...
                    for (int j = 0; j < qLength; j++) {
                        ...
                        if (qualifiedName.length == 1 ? debugIncludes(qualifiedName[0]) : debugIncludes(qualifiedName)) {
                            if (JavaBuilder.DEBUG)
                                ...
                                "Found match in " + CharOperation.toString(qualifiedName) + " to " + //$NON-NLS-1$
    ...
    private boolean debugInsideRoot(char[] rootName) {
        for (char[] name : this.rootReferences) if (rootName == name)
            return true;
        return false;
    }

    private boolean debugIncludes(char[] simpleName) {
        for (char[] name : this.simpleNameReferences) if (simpleName == name)
            return true;
        return false;
    }

    private boolean debugIncludes(char[][] qualifiedName) {
        for (char[][] name : this.qualifiedNameReferences) if (qualifiedName == name)
            return true;
        return false;
    }
    ...
    public boolean equals(Object obj) {
        ...
        return Arrays.deepEquals(this.qualifiedNameReferences, other.qualifiedNameReferences) && Arrays.deepEquals(this.rootReferences, other.rootReferences) && Arrays.deepEquals(this.simpleNameReferences, other.simpleNameReferences);
    }
}
