...
public class ModuleSourcePathManager {
    ...
    final public Map<String, IModulePathEntry> knownModules = new ConcurrentHashMap<>();
    ...
    public void removeEntry(JavaProject javaProject) {
        Entry<String, IModulePathEntry> entry = this.knownModules.entrySet().stream().filter(e -> ProjectEntry.representsProject(e.getValue(), javaProject)).findFirst().orElse(null);
