...
public class ASTRewriteFlattener extends ASTVisitor {
     ...
     * @deprecated using deprecated code
     */
    static final public SimplePropertyDescriptor INTERNAL_INITIALIZER_MODIFIERS_PROPERTY = Initializer.MODIFIERS_PROPERTY;

    /**
     * @deprecated using deprecated code
     */
    static final public SimplePropertyDescriptor INTERNAL_METHOD_MODIFIERS_PROPERTY = MethodDeclaration.MODIFIERS_PROPERTY;
    ...
    public boolean visit(MethodDeclaration node) {
        ...
        ChildListPropertyDescriptor exceptionsProperty = node.getAST().apiLevel() < JLS8_INTERNAL ? INTERNAL_METHOD_THROWN_EXCEPTIONS_PROPERTY : MethodDeclaration.THROWN_EXCEPTION_TYPES_PROPERTY;
        //$NON-NLS-1$
    ...
    public boolean visit(RecordDeclaration node) {
        ...
        this.result.append(' ');
        //$NON-NLS-1$
        visitList(node, RecordDeclaration.SUPER_INTERFACE_TYPES_PROPERTY, String.valueOf(','), "implements ", Util.EMPTY_STRING);
        this.result.append('{');
    ...
    public boolean visit(SwitchCase node) {
        if ((node.getAST().apiLevel() >= JLS14_INTERNAL)) {
            if (node.isDefault()) {
                ...
                this.result.append("default");
                //$NON-NLS-1$ //$NON-NLS-2$
                this.result.append(getBooleanAttribute(node, SwitchCase.SWITCH_LABELED_RULE_PROPERTY) ? " ->" : ":");
            } else {
                //$NON-NLS-1$
