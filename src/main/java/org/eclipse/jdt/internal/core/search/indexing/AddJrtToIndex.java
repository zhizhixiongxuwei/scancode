...
public class AddJrtToIndex extends BinaryContainer {
    ...
    private class JrtTraverser implements org.eclipse.jdt.internal.compiler.util.JRTUtil.JrtFileVisitor<java.nio.file.Path> {
        ...
        public FileVisitResult visitFile(java.nio.file.Path path, java.nio.file.Path mod, BasicFileAttributes attrs) throws IOException {
            ...
            if (isClassFileName(name) && isValidPackageNameForClassOrisModule(name)) {
    ...
    public boolean execute(IProgressMonitor progressMonitor) {
        ...
        try {
            ...
            if (this.manager.getIndexForUpdate(this.containerPath, false, /*do not reuse index file*/
            false) != null) {
            ...
            final Index index = this.manager.getIndexForUpdate(this.containerPath, true, /*reuse index file*/
            true);
            if (index == null) {
                if (JobManager.VERBOSE) {
                    //$NON-NLS-1$
            ...
            try {
                ...
                if (this.resource != null) {
                    ...
                    if (JavaModelManager.JRT_ACCESS_VERBOSE) {
                        //$NON-NLS-1$	//$NON-NLS-2$
                        trace("(" + Thread.currentThread() + ") [AddJrtFileToIndex.execute()] Creating ZipFile on " + location.getPath());
                    }
                    File file = null;
                    try {
                ...
                if (paths != null) {
                    ...
                    for (int i = 0; i < max; i++) indexedFileNames.put(paths[i], FILE_INDEX_STATE.DELETED);
                    org.eclipse.jdt.internal.compiler.util.JRTUtil.walkModuleImage(new File(fileName), new JrtTraverser(indexedFileNames), JRTUtil.NOTIFY_FILES);
                    ...
                    if (!needToReindex) {
                        ...
                        if (!needToReindex) {
                            if (JobManager.VERBOSE)
                                trace(//$NON-NLS-1$
                                "-> no indexing required (index is consistent with library) for " + fileName + //$NON-NLS-1$
                ...
                org.eclipse.jdt.internal.compiler.util.JRTUtil.walkModuleImage(jrt, new JrtIndexer(jrt, SearchEngine.getDefaultSearchParticipant(), index, container, this.manager), JRTUtil.NOTIFY_FILES);
                ...
                if (JobManager.VERBOSE)
                    trace(//$NON-NLS-1$
                    "-> done indexing of " + fileName + //$NON-NLS-1$
                    " (" + (System.currentTimeMillis() - initialTime) + //$NON-NLS-1$
                    "ms)");
            } finally {
                monitor.exitWrite();
            ...
            //$NON-NLS-1$
            Util.log(e, "Failed to index " + this.containerPath);
            this.manager.removeIndex(this.containerPath);
            return false;
        }
        return true;
