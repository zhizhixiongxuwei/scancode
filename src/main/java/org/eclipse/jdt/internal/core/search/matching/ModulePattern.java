...
public class ModulePattern extends JavaSearchPattern {
    ...
    /* package visible */
    static public char[][] REF_CATEGORIES = { MODULE_REF };

    static public char[][] REF_AND_DECL_CATEGORIES = { MODULE_REF, MODULE_DECL };
    ...
    public static char[] createIndexKey(char[] name) {
        // until a need arises, let the name itself be the index key.
        return name;
    }
        ...
        switch(limitTo & 0xF) {
            ...
            case IJavaSearchConstants.ALL_OCCURRENCES:
                if ((getMatchRule() & SearchPattern.R_REGEXP_MATCH) != 0)
                    //regex implemented only for module declarations.
                    this.findReferences = false;
                break;
        }
    ...
    private static int trapDoorRegexMatchRule(char[] name2, int matchRule) {
        return CharOperation.indexOf(regexPrefix, name2, false) == 0 ? SearchPattern.R_REGEXP_MATCH : matchRule;
    }
