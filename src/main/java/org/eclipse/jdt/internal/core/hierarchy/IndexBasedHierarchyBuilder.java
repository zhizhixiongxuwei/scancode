...
public class IndexBasedHierarchyBuilder extends HierarchyBuilder implements SuffixConstants {
     ...
     */
    public Map binariesFromIndexMatches;
    ...
    protected IBinaryType createInfoFromClassFileInJar(Openable classFile) {
        ...
        if (md != null) {
            ...
            documentPath = rootPath + IJavaSearchScope.JAR_FILE_ENTRY_SEPARATOR + module + IJavaSearchScope.JAR_FILE_ENTRY_SEPARATOR + filePath;
        } else {
    ...
    int waitingPolicy, final IProgressMonitor monitor) {
        legacySearchAllPossibleSubTypes(type, scope, binariesFromIndexMatches, pathRequestor, waitingPolicy, monitor);
    }

    private static void legacySearchAllPossibleSubTypes(IType type, IJavaSearchScope scope, final Map binariesFromIndexMatches, final IPathRequestor pathRequestor, // WaitUntilReadyToSearch | ForceImmediateSearch | CancelIfNotReadyToSearch
    int waitingPolicy, final IProgressMonitor progressMonitor) {
        ...
        IndexQueryRequestor searchRequestor = new IndexQueryRequestor() {
            ...
            public boolean acceptIndexMatch(String documentPath, SearchPattern indexRecord, SearchParticipant participant, AccessRuleSet access) {
                ...
                !isLocalOrAnonymous && !foundSuperNames.containsKey(fqnSuperName)) {
        ...
        try {
            while (queue.start <= queue.end) {
                ...
                char[] qualifiedTypeName = query.qualifiedName != null ? query.qualifiedName : QualifierQuery.NO_CHARS;
                if (CharOperation.equals(currentTypeName, IIndexConstants.OBJECT)) {
                    currentTypeName = null;
                } else {
                    MatchLocator.setIndexQualifierQuery(pattern, QualifierQuery.encodeQuery(new QueryCategory[] { QueryCategory.SUPER }, currentTypeName, qualifiedTypeName));
                }
            ...
            if (JobManager.VERBOSE) {
                ...
                //$NON-NLS-1$//$NON-NLS-2$
                JavaModelManager.trace("-> execution time: " + wallClockTime + "ms - " + IndexBasedHierarchyBuilder.class.getSimpleName());
            }
        }
    }
}
