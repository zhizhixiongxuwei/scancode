...
public final class ASTRewriteAnalyzer extends ASTVisitor {
     ...
     */
    static final public ChildPropertyDescriptor INTERNAL_ARRAY_COMPONENT_TYPE_PROPERTY = ArrayType.COMPONENT_TYPE_PROPERTY;

    /**
     * @deprecated using deprecated code
     */
    static final public SimplePropertyDescriptor INTERNAL_FIELD_MODIFIERS_PROPERTY = FieldDeclaration.MODIFIERS_PROPERTY;

    /**
     * @deprecated using deprecated code
     */
    static final public SimplePropertyDescriptor INTERNAL_INITIALIZER_MODIFIERS_PROPERTY3 = Initializer.MODIFIERS_PROPERTY;

    /**
     * @deprecated using deprecated code
     */
    static final public SimplePropertyDescriptor INTERNAL_METHOD_MODIFIERS_PROPERTY2 = MethodDeclaration.MODIFIERS_PROPERTY;

    /**
     * @deprecated using deprecated code
     */
    static final public ChildPropertyDescriptor INTERNAL_METHOD_RETURN_TYPE_PROPERTY = MethodDeclaration.RETURN_TYPE_PROPERTY;
    ...
    final private String getIndentString(String line, int minimumIndentInSpaces) {
        ...
        if (indentInSpaces < minimumIndentInSpaces) {
            //$NON-NLS-1$
            indent = indent + " ".repeat(minimumIndentInSpaces - indentInSpaces);
        }
        return indent;
    }
    ...
    class ListRewriter {
        ...
        private int rewriteList(ASTNode parent, StructuralPropertyDescriptor property, String keyword, String endKeyword, int offset) {
            ...
            if (property == Block.STATEMENTS_PROPERTY || property == SwitchStatement.STATEMENTS_PROPERTY || property == SwitchExpression.STATEMENTS_PROPERTY || property == TypeDeclaration.BODY_DECLARATIONS_PROPERTY) {
            ...
            if (insertNew) {
                // only inserts
                if (keyword.length() > 0) {
                    // creating a new list -> insert keyword first (e.g. " throws ")
            ...
            for (int i = 0; i < total; i++) {
                ...
                if (currMark == RewriteEvent.INSERTED) {
                    ...
                    if (separatorState == NONE) {
                        // element after last existing element (but not first)
                        // insert separator
                        doTextInsert(currPos, getSeparatorString(i - 1), editGroup);
                        separatorState = NEW;
                    ...
                    if (separatorState == NEW || insertAfterSeparator(node)) {
                        ...
                        if (maintainMinimumIndent && separatorState != EXISTING) {
                            // insert node
                            doTextInsert(currPos, node, getNodeIndent(i), true, getNodeIndentInSpaces(i), editGroup);
                        } else {
                            // insert node
                    ...
                    if (i > lastNonDelete && separatorState == EXISTING) {
                        ...
                        if (mustRemoveSeparator(currPos, i)) {
                            // remove separator
                            doTextRemove(currEnd, end - currEnd, editGroup);
                        }
                        currPos = end;
                        prevEnd = currEnd;
                    ...
                    if (i == lastNonInsert) {
                        ...
                        // start of next
                        currPos = getStartOfNextNode(nextIndex, prevEnd);
                ...
                case RewriteEvent.REPLACED:
                    {
                        ...
                        if (endPos == -1) {
                            ...
                            if (endPos != previousEnd) {
                                ...
                                // Sentinel value - was TokenScanner.END_OF_FILE before refactor
                ...
                case RewriteEvent.INSERTED:
                    {
                        ...
                        //$NON-NLS-1$
                        doTextInsert(startPos, ".", editGroup);
                        return startPos;
                    }
                case RewriteEvent.REMOVED:
                    {
    ...
    private int rewriteOptionalTypeParameters(ASTNode parent, StructuralPropertyDescriptor property, int offset, String keyword, boolean adjustOnNext, boolean needsSpaceOnRemoveAll) {
        ...
        if (event != null && event.getChangeKind() != RewriteEvent.UNCHANGED) {
            ...
            try {
                ...
                if (isAllRemoved) {
                    // all removed: remove right and space up to next element
                    // set pos to '>'
                    int endPos = getScanner().getTokenEndOffset(TerminalToken.TokenNameGREATER, pos);
    ...
    final void doTextInsert(int insertOffset, ASTNode node, int initialIndentLevel, boolean removeLeadingIndent, int minimumIndentInSpaces, TextEditGroup editGroup) {
        ...
        for (int i = 0; i < markers.size(); i++) {
            ...
            if (data instanceof TextEditGroup) {
                ...
                String destIndentString = (lineOffset == 0) ? createIndentString(initialIndentLevel, minimumIndentInSpaces) : getIndentString(formatted.substring(lineOffset, offset), minimumIndentInSpaces);
                if (data instanceof CopyPlaceholderData) {
                    // replace with a copy/move target
                    ...
                    if (minimumIndentInSpaces == destIndentStringInSpaces && minimumIndentInSpaces % this.formatter.getIndentWidth() != 0 && lines.length > 1) {
        ...
        for (int i = 0; i < lines.length - 1; ++i) {
            ...
            if (!line.isEmpty()) {
                ...
                if (indentOfLineInSpaces < minimumIndentInSpaces) {
                    String indentOfLine = this.formatter.getIndentStringWithSpaces(line);
                    int padding = minimumIndentInSpaces - indentOfLineInSpaces;
                    line = indentOfLine + //$NON-NLS-1$
                    " ".repeat(padding) + line.substring(indentOfLine.length());
                }
            }
            //$NON-NLS-1$
            s.append(line + "\n");
    ...
    public boolean visit(TypeDeclaration node) {
        ...
        if (!isInterface || invertType) {
            ...
            switch(changeKind) {
                case RewriteEvent.INSERTED:
                    {
                        //$NON-NLS-1$
                        doTextInsert(pos, " extends ", getEditGroup(superClassEvent));
        ...
        if (DOMASTUtil.isFeatureSupportedinAST(node.getAST(), Modifier.SEALED)) {
            ...
            if (permitsEvent == null || permitsEvent.getChangeKind() == RewriteEvent.UNCHANGED) {
                ...
                //$NON-NLS-1$ //$NON-NLS-2$
                pos = rewriteNodeList(node, permitsProperty, pos, " permits ", ", ");
            }
        }
        // type members
    ...
    private void rewriteReturnType(MethodDeclaration node, boolean isConstructor, boolean isConstructorChange) {
        ...
        if (isConstructorChange || !returnTypeExists && newReturnType != originalReturnType) {
            ...
            if (isConstructor || !returnTypeExists) {
                ...
                //$NON-NLS-1$
                doTextInsert(nextStart, " ", editGroup);
            } else {
    ...
    private int rewriteMethodReceiver(MethodDeclaration method, int offset) throws CoreException {
        ...
        if (event != null && event.getChangeKind() != RewriteEvent.UNCHANGED) {
            ...
            if (changeKind == RewriteEvent.INSERTED) {
                ...
                //$NON-NLS-1$
                doTextInsert(offset, "this", editGroup);
                if (newParamCount > 0) {
                    //$NON-NLS-1$
                    doTextInsert(offset, ", ", editGroup);
                }
    ...
    public boolean visit(Dimension node) {
        ...
        if (parent.getNodeType() == ASTNode.ARRAY_TYPE) {
            ...
            if (oldAnnotationSize > 0 && newAnnotationSize == 0) {
            ...
            } else if (oldAnnotationSize == 0 && newAnnotationSize > 0) {
                /* inserting first annotation */
        ...
        if (event != null && event.getChangeKind() != RewriteEvent.UNCHANGED) {
            ...
            try {
                ...
                if (wasOpen) {
                    ...
                    //$NON-NLS-1$
                    doTextInsert(pos, "open ", getEditGroup(event));
                }
            } catch (CoreException e) {
                handleException(e);
            } finally {
    ...
    public boolean visit(RecordDeclaration node) {
        ...
        //$NON-NLS-1$
        pos = rewriteNodeList(node, RecordDeclaration.RECORD_COMPONENTS_PROPERTY, pos, Util.EMPTY_STRING, ", ");
        try {
            pos = getScanner().getTokenEndOffset(TerminalToken.TokenNameRPAREN, pos);
        } catch (CoreException ex) {
            //ignore
    ...
    public boolean visit(RecordPattern node) {
        ...
        //$NON-NLS-1$
        rewriteNodeList(node, RecordPattern.PATTERNS_PROPERTY, pos, Util.EMPTY_STRING, ", ");
        return false;
    }
    ...
    public boolean visit(EitherOrMultiPattern node) {
        ...
        //$NON-NLS-1$
        rewriteNodeList(node, EitherOrMultiPattern.PATTERNS_PROPERTY, pos, Util.EMPTY_STRING, ", ");
        return false;
    }
    ...
    public boolean visit(ArrayCreation node) {
        ...
        if (typeEvent != null && typeEvent.getChangeKind() == RewriteEvent.REPLACED) {
            ...
            if (annotations != null) {
                ...
                if (size > 0) {
                    ...
                    for (int j = 0; j < size; j++) {
                        ...
                        //$NON-NLS-1$
                        doTextInsert(pos, annotation.toString() + " ", editGroup);
                    }
                }
            }
        }
    ...
    public boolean visit(ClassInstanceCreation node) {
        ...
        if (node.getAST().apiLevel() == JLS2_INTERNAL) {
            ...
            if (isChanged(node, ClassInstanceCreation.TYPE_ARGUMENTS_PROPERTY)) {
                try {
                    //after 'new'
                    pos = getScanner().getTokenEndOffset(TerminalToken.TokenNamenew, pos);
                    //$NON-NLS-1$
                    rewriteOptionalTypeParameters(node, ClassInstanceCreation.TYPE_ARGUMENTS_PROPERTY, pos, " ", true, true);
                } catch (CoreException e) {
                    handleException(e);
                }
            } else {
    ...
    public boolean visit(DoStatement node) {
        ...
        try {
            ...
            if (event != null && event.getChangeKind() == RewriteEvent.REPLACED) {
                ...
                // body
                rewriteBodyNode(node, DoStatement.BODY_PROPERTY, startOffset, endPos, getIndent(node.getStartPosition()), this.formatter.DO_BLOCK);
            } else {
    ...
    public boolean visit(ExportsDirective node) {
        ...
        //$NON-NLS-1$ //$NON-NLS-2$
        rewriteNodeList(node, ExportsDirective.MODULES_PROPERTY, pos, "to ", ", ");
        return false;
    }
    ...
    public boolean visit(ForStatement node) {
        ...
        try {
            ...
            if (bodyEvent != null && bodyEvent.getChangeKind() == RewriteEvent.REPLACED) {
                ...
                // body
                rewriteBodyNode(node, ForStatement.BODY_PROPERTY, startOffset, -1, getIndent(node.getStartPosition()), this.formatter.FOR_BLOCK);
            } else {
    ...
    public boolean visit(IfStatement node) {
        ...
        if (thenEvent != null && thenEvent.getChangeKind() != RewriteEvent.UNCHANGED) {
            try {
                ...
                TerminalToken tok = getScanner().readNext(pos, true);
                ...
                if (elseStatement != null) {
                    ...
                    // else keyword
                    endPos = getScanner().getTokenStartOffset(TerminalToken.TokenNameelse, thenStatement.getStartPosition() + thenStatement.getLength());
                }
            ...
            if (event != null && event.getChangeKind() != RewriteEvent.UNCHANGED) {
                try {
                    ...
                    if (wasStatic) {
                        ...
                        //$NON-NLS-1$
                        doTextInsert(pos, " static", getEditGroup(event));
                    }
                } catch (CoreException e) {
                    handleException(e);
                }
    ...
    public boolean visit(Javadoc node) {
        ...
        String separator = getLineDelimiter() + getIndentAtOffset(node.getStartPosition()) + //$NON-NLS-1$ //$NON-NLS-2$
        ((node.getAST().apiLevel() >= AST.JLS23 && node.isMarkdown()) ? "/// " : " * ");
    ...
    public boolean visit(LambdaExpression node) {
        ...
        if (!hasParentheses) {
            // Parentheses can be absent if and only if there is one and only one type elided parameter.
        ...
        if (event != null) {
            ...
            if (event.getChangeKind() == RewriteEvent.REPLACED) {
                if (newValue != Boolean.FALSE) {
                    ...
                    // apply the stricter check for parentheses deletion
                    deleteParentheses = !hasParentheses;
                }
            }
        } else if (!oldHasParentheses && hasParentheses) {
            // parameter property changed to effect parentheses insertion
            if ((event = getEvent(node, LambdaExpression.PARAMETERS_PROPERTY)) != null) {
                // a null check though event cannot be null here
                editGroup = getEditGroup(event);
                insertParentheses = true;
            }
        }
        ...
        if (isChanged(node, LambdaExpression.PARAMETERS_PROPERTY)) {
            try {
                ...
                //$NON-NLS-1$
                pos = rewriteNodeList(node, LambdaExpression.PARAMETERS_PROPERTY, pos, Util.EMPTY_STRING, ", ");
            } catch (CoreException e) {
                handleException(e);
            }
        } else {
    ...
    public boolean visit(ProvidesDirective node) {
        ...
        //$NON-NLS-1$ //$NON-NLS-2$
        pos = rewriteNodeList(node, ProvidesDirective.IMPLEMENTATIONS_PROPERTY, pos, " with ", ", ");
        return false;
    }
    ...
    public boolean visit(SwitchCase node) {
        ...
        if (node.getAST().apiLevel() >= JLS14_INTERNAL) {
            int pos = node.expressions().size() == 0 ? node.getStartPosition() : //$NON-NLS-1$
    ...
    class SwitchListLabeledRuleRewriter extends SwitchListRewriter {
        ...
        protected String getSeparatorString(int nodeIndex, int nextNodeIndex) {
            ...
            //$NON-NLS-1$ //$NON-NLS-2$
            String spaceDelim = JavaCore.INSERT.equals(ASTRewriteAnalyzer.this.options.get(DefaultCodeFormatterConstants.FORMATTER_INSERT_SPACE_AFTER_ARROW_IN_SWITCH_CASE)) ? " " : "";
    ...
    public boolean visit(TryStatement node) {
        ...
        if (level >= JLS4_INTERNAL) {
            ...
            if (isChanged(node, desc)) {
                ...
                //$NON-NLS-1$
                String newParen = this.formatter.TRY_RESOURCES_PAREN.getPrefix(indent) + "(";
                //$NON-NLS-1$ //$NON-NLS-2$
                pos = rewriteResourcesNodeList(node, desc, getPosAfterTry(pos), newParen, ")", ";" + prefix);
            } else {
    ...
    public boolean visit(WhileStatement node) {
        ...
        try {
            if (isChanged(node, WhileStatement.BODY_PROPERTY)) {
                int startOffset = getScanner().getTokenEndOffset(TerminalToken.TokenNameRPAREN, pos);
                // body
                rewriteBodyNode(node, WhileStatement.BODY_PROPERTY, startOffset, -1, getIndent(node.getStartPosition()), this.formatter.WHILE_BLOCK);
            } else {
        ...
        if (node.getAST().apiLevel() >= JLS3_INTERNAL) {
            if (isChanged(node, MethodRefParameter.VARARGS_PROPERTY)) {
                if (getNewValue(node, MethodRefParameter.VARARGS_PROPERTY).equals(Boolean.TRUE)) {
                    //$NON-NLS-1$
                    doTextInsert(pos, "...", getEditGroup(node, MethodRefParameter.VARARGS_PROPERTY));
                } else {
                    try {
    ...
    public boolean visit(EnhancedForStatement node) {
        ...
        if (bodyEvent != null && bodyEvent.getChangeKind() == RewriteEvent.REPLACED) {
            ...
            try {
                ...
                // body
                rewriteBodyNode(node, EnhancedForStatement.BODY_PROPERTY, startOffset, -1, getIndent(node.getStartPosition()), this.formatter.FOR_BLOCK);
            } catch (CoreException e) {
                handleException(e);
            }
        } else {
    ...
    public boolean visit(EnumConstantDeclaration node) {
        ...
        if (argsEvent != null && argsEvent.getChangeKind() != RewriteEvent.UNCHANGED) {
            ...
            try {
                ...
                //$NON-NLS-1$
                pos = rewriteNodeList(node, EnumConstantDeclaration.ARGUMENTS_PROPERTY, pos, prefix, ", ");
                if (!hasParents) {
                    //$NON-NLS-1$
                    doTextInsert(pos, ")", getEditGroup(children[children.length - 1]));
                } else if (isAllRemoved) {
                ...
                if (!hasSemicolon && isAllOfKind(children, RewriteEvent.INSERTED)) {
                    ...
                    if (token == TerminalToken.TokenNameCOMMA) {
                        ...
                        if (nextToken != TerminalToken.TokenNameSEMICOLON) {
                            //$NON-NLS-1$
                            doTextInsert(endPos, ";", getEditGroup(children[0]));
                        } else {
                        ...
                        //$NON-NLS-1$
                        doTextInsert(pos, ";", getEditGroup(children[0]));
                    }
    ...
    public boolean visit(ModuleModifier node) {
        ...
        // type ModuleModifier.ModuleModifierKeyword
        String newText = getNewValue(node, ModuleModifier.KEYWORD_PROPERTY).toString();
    ...
    public boolean visit(YieldStatement node) {
        ...
        try {
            ...
            if ((node.getAST().apiLevel() >= JLS14_INTERNAL)) {
                // space between yield and label
                rewriteNode(node, YieldStatement.EXPRESSION_PROPERTY, offset, ASTRewriteFormatter.SPACE);
            }
        } catch (CoreException e) {
            handleException(e);
        }
    ...
    final void handleException(Throwable e) {
        //$NON-NLS-1$
        throw new IllegalArgumentException("Document does not match the AST", e);
    }
}
