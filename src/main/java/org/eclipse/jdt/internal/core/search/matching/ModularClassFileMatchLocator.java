...
public class ModularClassFileMatchLocator implements IIndexConstants {
    ...
    public IBinaryModule binaryModule;

    public ModularClassFile modularClassFile;

    public IModuleDescription moduleDesc;
    ...
    private void matchModuleReference(MatchLocator locator, ModulePattern modulePattern, char[][] modules, boolean isTarget) throws CoreException {
        ...
        for (char[] module1 : modules) {
            if (module1 == null || module1.length == 0)
                continue;
            if (!locator.patternLocator.matchesName(modulePattern.name, module1))
                continue;
    ...
    private void matchModuleReferences(MatchLocator locator, ModulePattern modulePattern, IPackageExport[] pvs) throws CoreException {
        if (pvs == null)
            return;
        for (IPackageExport pv : pvs) {
            matchModuleReference(locator, modulePattern, pv.targets(), true);
        }
    }
