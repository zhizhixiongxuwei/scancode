...
public class IndexSelector {
    ...
    public IJavaSearchScope searchScope;
 ...
 */
    ...
    public IndexLocation[] getIndexLocations() {
        ...
        if (this.pattern != null && this.pattern.indexQualifierQuery != null && this.pattern.indexQualifierQuery.length > 0) {
            Optional<Set<String>> indexNamesResult = JavaModelManager.getIndexManager().findMatchingIndexNames(QualifierQuery.fromEncodedQuery(this.pattern.indexQualifierQuery));
            if (indexNamesResult.isPresent()) {
                Set<String> indexNames = indexNamesResult.get();
                filtered = Stream.of(this.indexLocations).filter(l -> indexNames.contains(l.fileName())).toArray(IndexLocation[]::new);
                if (filtered.length == 0) {
                    if (JobManager.VERBOSE) {
                        trace(String.format(//$NON-NLS-1$
                        "-> current index selection and qualifying indexes has no intersection, " + //$NON-NLS-1$
                        "to keep search backward compatible using selected index locations - %s", this.toString()));
                    }
        ...
        if (JobManager.VERBOSE) {
            ...
            String.//$NON-NLS-1$
            format(//$NON-NLS-1$
            "-> selected %s indexes out of total indexes %s after qualify filtering - %s", filtered.length, this.indexLocations.length, this.toString()));
        }
        return filtered;
    }
