...
public class BuildNotifier {
    ...
    volatile public boolean cancelling;
    ...
    private static final int millisecondsBeforeCancelAutoBuild = Integer.getInteger("org.eclipse.jdt.MillisecondsBeforeCancelAutoBuild", //$NON-NLS-1$
    1000);

    private static final int millisecondsBeforeInterruptAutoBuild = Integer.getInteger("org.eclipse.jdt.MillisecondsBeforeInterruptAutoBuild", //$NON-NLS-1$
    3000);
    ...
    protected void updateProblemCounts(CategorizedProblem[] newProblems) {
        for (CategorizedProblem newProblem : newProblems) if (newProblem.isError())
            this.newErrorCount++;
        else
