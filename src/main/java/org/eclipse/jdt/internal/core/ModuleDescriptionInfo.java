...
public class ModuleDescriptionInfo extends AnnotatableInfo implements ISourceModule {
    ...
    static final public char[][] NO_USES = new char[0][0];

    static final public ModuleReferenceInfo[] NO_REQUIRES = new ModuleReferenceInfo[0];

    static final public PackageExportInfo[] NO_EXPORTS = new PackageExportInfo[0];

    static final public ServiceInfo[] NO_PROVIDES = new ServiceInfo[0];

    static final public PackageExportInfo[] NO_OPENS = new PackageExportInfo[0];
    ...
    static class ModuleReferenceInfo extends MemberElementInfo implements IModule.IModuleReference {
        ...
        public String toString() {
            ...
            //$NON-NLS-1$
            builder.append("ModuleReferenceInfo [");
            if (this.name() != null) {
                //$NON-NLS-1$
                builder.append("name=");
    ...
    static class ServiceInfo extends MemberElementInfo implements IModule.IService {
        ...
        public boolean equals(Object obj) {
            ...
            return Arrays.deepEquals(this.implNames, other.implNames) && Arrays.equals(this.serviceName, other.serviceName);
        }
    }
    ...
    public static ModuleDescriptionInfo createModule(ModuleDeclaration module) {
        ...
        if (module.requiresCount > 0) {
            ...
            for (int i = 0; i < refs.length; i++) {
                ...
                // Check why ModuleReference#tokens must be a char[][] and not a char[] or String;
                mod.requires[i + 1].name = CharOperation.concatWith(refs[i].module.tokens, '.');
                mod.requires[i + 1].modifiers = refs[i].modifiers;
            }
        } else {
            mod.requires = CharOperation.equals(module.moduleName, TypeConstants.JAVA_DOT_BASE) ? NO_REQUIRES : new ModuleReferenceInfo[] { getJavaBaseReference() };
        }
