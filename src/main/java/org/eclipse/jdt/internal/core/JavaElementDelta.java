...
public class JavaElementDelta extends SimpleDelta implements IJavaElementDelta {
     ...
     */
    public IJavaElementDelta[] affectedChildren = EMPTY_DELTA;
	 ...
	 */
    public IJavaElement changedElement;

    /**
     * Collection of resource deltas that correspond to non java resources deltas.
     */
    public IResourceDelta[] resourceDeltas = null;

    /**
     * Counter of resource deltas
     */
    public int resourceDeltasCounter;
            ...
            switch(existingChild.getKind()) {
                case ADDED:
                    switch(child.getKind()) {
                        ...
                        case ADDED:
                        case // child was added then changed -> it is added
                        CHANGED:
                            return;
                        case // child was added then removed -> noop
                        REMOVED:
                ...
                case REMOVED:
                    switch(child.getKind()) {
                        case // child was removed then added -> it is changed
                        ADDED:
                            child.kind = CHANGED;
                        ...
                        case CHANGED:
                        case // child was removed then removed -> it is removed
                        REMOVED:
                            return;
                    }
                    break;
                case CHANGED:
                    switch(child.getKind()) {
                        ...
                        case ADDED:
                        case // child was changed then removed -> it is removed
                        REMOVED:
                            this.affectedChildren[existingChildIndex] = child;
                            return;
                        case // child was changed then changed -> it is changed
                        CHANGED:
    ...
    protected boolean toDebugString(StringBuilder buffer, int flags) {
        ...
        if ((flags & IJavaElementDelta.F_CLASSPATH_ATTRIBUTES) != 0) {
            if (prev)
                ...
                buffer.append(" | ");
            //$NON-NLS-1$
            buffer.append("CLASSPATH ATTRIBUTES");
            prev = true;
        }
        return prev;
    }
