...
public class Disassembler extends ClassFileBytesDisassembler {
    ...
    static final public char[] ANY_EXCEPTION = Messages.classfileformat_anyexceptionhandler.toCharArray();

    static final public String VERSION_UNKNOWN = Messages.classfileformat_versionUnknown;
        ...
        for (int checkBit : checkBits) {
            switch(checkBit) {
                case IModifierConstants.ACC_PUBLIC:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_PUBLIC, "public", firstModifier);
                    break;
                case IModifierConstants.ACC_PROTECTED:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_PROTECTED, "protected", firstModifier);
                    break;
                case IModifierConstants.ACC_PRIVATE:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_PRIVATE, "private", firstModifier);
                    break;
                case IModifierConstants.ACC_ABSTRACT:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_ABSTRACT, "abstract", firstModifier);
                    break;
                case IModifierConstants.ACC_STATIC:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_STATIC, "static", firstModifier);
                    break;
                case IModifierConstants.ACC_FINAL:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_FINAL, "final", firstModifier);
                    break;
                case IModifierConstants.ACC_SYNCHRONIZED:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_SYNCHRONIZED, "synchronized", firstModifier);
                    break;
                case IModifierConstants.ACC_NATIVE:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_NATIVE, "native", firstModifier);
                    break;
                case IModifierConstants.ACC_STRICT:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_STRICT, "strictfp", firstModifier);
                    break;
                case IModifierConstants.ACC_TRANSIENT:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_TRANSIENT, "transient", firstModifier);
                    break;
                case IModifierConstants.ACC_VOLATILE:
                    // case IModifierConstants.ACC_BRIDGE :
                    if (asBridge) {
                        //$NON-NLS-1$
                        firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_BRIDGE, "bridge", firstModifier);
                    } else {
                        //$NON-NLS-1$
                        firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_VOLATILE, "volatile", firstModifier);
                    }
                    break;
                case IModifierConstants.ACC_ENUM:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_ENUM, "enum", firstModifier);
                    break;
                case IModifierConstants.ACC_SYNTHETIC:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_SYNTHETIC, "synthetic", firstModifier);
                    break;
                case IModifierConstants.ACC_MANDATED:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_MANDATED, "mandated", firstModifier);
                    break;
            }
        }
        if (!firstModifier) {
        ...
        for (int checkBit : checkBits) {
            switch(checkBit) {
                case IModifierConstants.ACC_TRANSITIVE:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_TRANSITIVE, "transitive", firstModifier);
                    break;
                case IModifierConstants.ACC_STATIC_PHASE:
                    //$NON-NLS-1$
                    firstModifier = appendModifier(buffer, accessFlags, IModifierConstants.ACC_STATIC_PHASE, "static", firstModifier);
                    break;
            }
        }
        if (!firstModifier) {
    ...
    private final void decodeModifiersForModule(StringBuilder buffer, int accessFlags) {
        //$NON-NLS-1$
        appendModifier(buffer, accessFlags, IModifierConstants.ACC_OPEN, "open", true);
        buffer.append(Messages.disassembler_space);
    }
                ...
                switch(constantPoolEntry.getKind()) {
                    ...
                    case IConstantPoolConstant.CONSTANT_Integer:
                        ...
                        switch(annotationComponentValue.getTag()) {
                            ...
                            case IAnnotationComponentValue.BOOLEAN_TAG:
                                //$NON-NLS-1$//$NON-NLS-2$
                                temp.append(constantPoolEntry.getIntegerValue() == 1 ? "true" : "false");
                                break;
                            case IAnnotationComponentValue.BYTE_TAG:
                                //$NON-NLS-1$
                                temp.append("(byte) ").append(constantPoolEntry.getIntegerValue());
                                break;
                            case IAnnotationComponentValue.SHORT_TAG:
                                //$NON-NLS-1$
                                temp.append("(short) ").append(constantPoolEntry.getIntegerValue());
                                break;
                            case IAnnotationComponentValue.INTEGER_TAG:
                                //$NON-NLS-1$
                                temp.append("(int) ").append(constantPoolEntry.getIntegerValue());
                        }
    ...
    public String disassemble(IClassFileReader classFileReader, String lineSeparator, int mode) {
        ...
        if (checkMode(mode, SYSTEM | DETAILED)) {
            ...
            buffer.append(Messages.bind(Messages.classfileformat_versiondetails, new String[] { versionNumber, Integer.toString(majorVersion), Integer.toString(minorVersion), ((accessFlags & IModifierConstants.ACC_SUPER) != 0 ? Messages.classfileformat_superflagisset : Messages.classfileformat_superflagisnotset) + //$NON-NLS-1$
            (isDeprecated(classFileReader) ? ", deprecated" : org.eclipse.jdt.internal.compiler.util.Util.EMPTY_STRING) }));
        ...
        if (isModule && moduleAttribute != null) {
            // print attributes - module package and main class only if the mandatory module attribute non-null
        ...
        if (checkMode(mode, SYSTEM | DETAILED)) {
            ...
            if (innerClassesAttribute != null || enclosingMethodAttribute != null || nestHostAttribute != null || nestMembersAttribute != null || bootstrapMethods != null || moduleAttribute != null || recordAttribute != null || permittedSubclassesAttribute != null || remainingAttributesLength != 0) {
    ...
    private void disassembleModule(IModuleAttribute moduleAttribute, StringBuilder buffer, String lineSeparator, int tabNumber) {
        ...
        if (usesNames.length > 0) {
            ...
            for (char[] usesName : usesNames) {
                writeNewLine(buffer, lineSeparator, tabNumber);
                //$NON-NLS-1$
                buffer.append("uses " + CharOperation.charToString(CharOperation.replaceOnCopy(usesName, '/', '.')));
            }
        }
    ...
    private void disassemble(INestHostAttribute nestHostAttribute, StringBuilder buffer, String lineSeparator, int tabNumber) {
        ...
        buffer.append(Messages.disassembler_constantpoolindex).append(nestHostAttribute.getNestHostIndex()).append(//$NON-NLS-1$
    ...
    private void disassemble(IComponentInfo componentInfo, StringBuilder buffer, String lineSeparator, int tabNumber, int mode) {
        ...
        if (checkMode(mode, SYSTEM | DETAILED)) {
            buffer.append(Messages.bind(Messages.classfileformat_componentdescriptor, new String[] { Integer.toString(componentInfo.getDescriptorIndex()), new String(descriptor) }));
            writeNewLine(buffer, lineSeparator, tabNumber);
            if (signatureAttribute != null) {
                buffer.append(Messages.bind(Messages.disassembler_signatureattributeheader, new String(signatureAttribute.getSignature())));
                writeNewLine(buffer, lineSeparator, tabNumber);
        ...
        if (checkMode(mode, SYSTEM)) {
            ...
            if (length != 0) {
                for (int i = 0; i < length; i++) {
                    IClassFileAttribute attribute = attributes[i];
                    if (attribute != signatureAttribute && attribute != runtimeInvisibleAnnotationsAttribute && attribute != runtimeVisibleAnnotationsAttribute && attribute != runtimeInvisibleTypeAnnotationsAttribute && attribute != runtimeVisibleTypeAnnotationsAttribute) {
                        disassemble(attribute, buffer, lineSeparator, tabNumber, mode);
                    }
                }
            }
    ...
    private void disassemble(IPackageVisibilityInfo iPackageVisibilityInfo, StringBuilder buffer, String lineSeparator, int tabNumber, boolean isExports) {
        //$NON-NLS-1$ //$NON-NLS-2$
        buffer.append(isExports ? "exports" : "opens");
        buffer.append(Messages.disassembler_space);
                ...
                case IConstantPoolConstant.CONSTANT_MethodHandle:
                    ...
                    switch(constantPoolEntry2.getReferenceKind()) {
                        ...
                        case IConstantPoolConstant.METHOD_TYPE_REF_NewInvokeSpecial:
                            constantPoolEntry = (ConstantPoolEntry) constantPool.decodeEntry(constantPoolEntry2.getReferenceIndex());
                            //$NON-NLS-1$
                            builder.append(Messages.bind("{0}.{1}:{2}", methodDescription(constantPoolEntry)));
                            arguments[i] = builder.toString();
                            break;
                    }
                    break;
    ...
    private String[] fieldDescription(IConstantPoolEntry constantPoolEntry) {
        return new String[] { new String(constantPoolEntry.getFieldName()), new String(constantPoolEntry.getFieldDescriptor()) };
    }
        ...
        for (int i = 1; i < length; i++) {
            ...
            switch(kind) {
                ...
                case IConstantPoolConstant.CONSTANT_Dynamic:
                    entry2 = (IConstantPoolEntry2) constantPoolEntry;
                    buffer.append(Messages.bind(Messages.disassembler_constantpool_dynamic, new String[] { Integer.toString(i), Integer.toString(entry2.getBootstrapMethodAttributeIndex()), Integer.toString(entry2.getNameAndTypeIndex()), new String(constantPoolEntry.getFieldName()), new String(constantPoolEntry.getFieldDescriptor()) }));
                    break;
            }
        }
    }
    ...
    private String toTypePathString(int[][] typepath) {
        ...
        for (int i = 0, max = typepath.length; i < max; i++) {
            ...
            switch(typepathElement[0]) {
                ...
                default:
                    //$NON-NLS-1$
                    throw new IllegalStateException("Unrecognized type_path_kind: " + typepathElement[0]);
            }
        }
        buffer.append(']');
        return String.valueOf(buffer);
