...
public class ClasspathMultiReleaseJar extends ClasspathJar {
    ...
    //$NON-NLS-1$
    static final public String META_INF_VERSIONS = "META-INF/versions/";

    static final public int META_INF_LENGTH = META_INF_VERSIONS.length();

    volatile public String[] supportedVersions;
    ...
    IModule initializeModule() {
        ...
        try (ZipFile file = new ZipFile(this.zipFilename)) {
            ...
            if (classfile == null) {
                // FIXME: use jar cache
                classfile = ClassFileReader.read(file, IModule.MODULE_INFO_CLASS);
            }
            if (classfile != null) {
