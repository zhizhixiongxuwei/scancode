...
public abstract class ClasspathLocation {
    ...
    public IUpdatableModule.UpdatesByKind updates;

    public Set<String> limitModuleNames = null;

    public String patchModuleName = null;

    public String externalAnnotationPath;

    // when configured to search all classpath locations for external annotations (eea) this is where to look
    ...
    abstract public NameEnvironmentAnswer findClass(String typeName, String qualifiedPackageName, String moduleName, String qualifiedBinaryFileName, boolean asBinaryOnly, Predicate<String> moduleNameFilter);
    ...
    static ClasspathLocation forSourceFolder(IContainer sourceFolder, IContainer outputFolder, char[][] inclusionPatterns, char[][] exclusionPatterns, boolean ignoreOptionalProblems, IPath externalAnnotationPath, int release) {
    ...
    static ClasspathLocation forLibrary(String libraryPathname, long lastModified, AccessRuleSet accessRuleSet, IPath annotationsPath, boolean isOnModulePath, String compliance) {
        return Util.archiveFormat(libraryPathname) == Util.JMOD_FILE ? new ClasspathJMod(libraryPathname, lastModified, accessRuleSet, annotationsPath) : (compliance == null || (CompilerOptions.versionToJdkLevel(compliance) < ClassFileConstants.JDK9) ? new ClasspathJar(libraryPathname, lastModified, accessRuleSet, annotationsPath, isOnModulePath) : new ClasspathMultiReleaseJar(libraryPathname, lastModified, accessRuleSet, annotationsPath, isOnModulePath, compliance));
    }
    ...
    public static ClasspathJrt forJrtSystem(String jrtPath, AccessRuleSet accessRuleSet, IPath annotationsPath, String release) throws CoreException {
        ...
        if (useRelease) {
            ...
            try {
                ...
                //$NON-NLS-1$
                throw new CoreException(new Status(IStatus.ERROR, ClasspathLocation.class, "Failed to detect JDK release for: " + jrtPath, e));
            }
    ...
    public static ClasspathLocation forLibrary(IFile library, AccessRuleSet accessRuleSet, IPath annotationsPath, boolean isOnModulePath, String compliance) {
        ...
        return (CompilerOptions.versionToJdkLevel(compliance) < ClassFileConstants.JDK9) ? new ClasspathJar(library, accessRuleSet, annotationsPath, isOnModulePath) : new ClasspathMultiReleaseJar(library, accessRuleSet, annotationsPath, isOnModulePath, compliance);
    }
    ...
    public static ClasspathLocation forLibrary(ZipFile zipFile, AccessRuleSet accessRuleSet, boolean isOnModulePath, String compliance) {
        return (CompilerOptions.versionToJdkLevel(compliance) < ClassFileConstants.JDK9) ? new ClasspathJar(zipFile, accessRuleSet, isOnModulePath) : new ClasspathMultiReleaseJar(zipFile, accessRuleSet, isOnModulePath, compliance);
    }
    ...
    protected IBinaryType decorateWithExternalAnnotations(IBinaryType reader, String fileNameWithoutExtension) {
        // default: don't decorate. Subclasses to override.
        return reader;
    }
        ...
        if (this.externalAnnotationPath != null) {
            ...
            for (ClasspathLocation annotationLocation : this.allLocationsForEEA) {
                ...
                if (reader.getExternalAnnotationStatus() == ExternalAnnotationStatus.TYPE_IS_ANNOTATED) {
                    isAnnotated = true;
                    // if merging of eea at method granularity should be supported, remove this break
                    break;
                }
            }
            if (!isAnnotated) {
    ...
    public void connectAllLocationsForEEA(Collection<ClasspathLocation> allLocations, boolean add) {
        // shared within the project, may be updated after setting
