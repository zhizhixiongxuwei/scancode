...
public class BindingKeyParser {
    ...
    static final public char C_THROWN = '|';
        ...
        int nextToken() {
            ...
            while (this.index <= length) {
                ...
                switch(currentChar) {
                    ...
                    case 'Z':
                        // base type
                        if (this.index == previousTokenEnd && (this.index == 0 || this.source[this.index - 1] != '.')) {
                    ...
                    case '.':
                        if (this.token == MODULE)
                            // don't treat '.' as a separator in module names
                            break;
                    //$FALL-THROUGH$
                    case '%':
                    case ':':
    ...
    public boolean parsingPaused;

    public Scanner scanner;

    public boolean hasTypeName = true;
    ...
    private void parseParameterizedType(char[] typeName, boolean isRaw) {
        ...
        if (this.scanner.isAtMemberTypeStart() && this.scanner.nextToken() == Scanner.TYPE) {
            ...
            if (this.scanner.isAtParametersStart()) {
                ...
                consumeParameterizedType(typeName, true);
        }
    }
