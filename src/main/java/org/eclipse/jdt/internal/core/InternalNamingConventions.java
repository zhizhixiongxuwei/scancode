...
public class InternalNamingConventions {
    ...
    //$NON-NLS-1$
    static final public char[] DEFAULT_NAME = "name".toCharArray();

    private static Scanner getNameScanner(CompilerOptions compilerOptions) {
        return new Scanner(false, /*comment*/
        false, /*whitespace*/
        false, /*nls*/
            ...
            if (!isConstantField) {
                unprefixedName[0] = ScannerHelper.toUpperCase(unprefixedName[0]);
                    ...
                    if (j == internalPrefix.length) {
                        matchingIndex = j;
                        break done;
                    } else if (CharOperation.prefixEquals(CharOperation.subarray(internalPrefix, j, -1), unprefixedName, j != 0)) {
