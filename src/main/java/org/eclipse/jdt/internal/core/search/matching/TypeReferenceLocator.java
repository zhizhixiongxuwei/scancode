...
public class TypeReferenceLocator extends PatternLocator {
    ...
    final public TypeReferencePattern pattern;

    final public boolean isDeclarationOfReferencedTypesPattern;

    final public int fineGrain;

    final public Map<QualifiedTypeReference, List<TypeBinding>> recordedResolutions = new HashMap<>();
    ...
    public int match(Reference node, MatchingNodeSet nodeSet) {
        ...
        if (node instanceof SingleNameReference) {
            ...
            for (char[] token : tokens) if (matchesName(this.pattern.simpleName, token))
                // resolution is needed to find out if it is a type ref
                return nodeSet.addMatch(node, POSSIBLE_MATCH);
        }
        return IMPOSSIBLE_MATCH;
    ...
    protected void matchReportReference(ASTNode reference, IJavaElement element, IJavaElement localElement, IJavaElement[] otherElements, Binding elementBinding, int accuracy, MatchLocator locator) throws CoreException {
        ...
        if (reference instanceof QualifiedNameReference)
            ...
            TypeBinding typeBinding = reference instanceof Expression && ((org.eclipse.jdt.internal.compiler.ast.Expression) reference).isTrulyExpression() ? ((Expression) reference).resolvedType : null;
            if (typeBinding != null) {
