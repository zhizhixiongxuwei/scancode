...
public class ModuleUpdater {
    ...
    final public JavaProject javaProoject;

    final public Map<String, UpdatesByKind> moduleUpdates = new HashMap<>();
    ...
    public void computeModuleUpdates(IClasspathEntry entry) throws JavaModelException {
        for (IClasspathAttribute attribute : entry.getExtraAttributes()) {
            ...
            // the attributes considered here may have multiple values separated by ':'
            String values = attribute.getValue();
            if (attributeName.equals(IClasspathAttribute.ADD_EXPORTS)) {
                for (String value : values.split(":")) {
                    // format: <source-module>/<package>=<target-module>(,<target-module>)* //$NON-NLS-1$
                    ...
                    if (slash != -1 && equals != -1) {
                        ...
                        //$NON-NLS-1$
                        Util.log(IStatus.WARNING, "Invalid argument to add-exports: " + value);
                    }
                }
            } else if (attributeName.equals(IClasspathAttribute.ADD_READS)) {
                for (String value : values.split(":")) {
                    // format: <source-module>=<target-module> //$NON-NLS-1$
                    int equals = value.indexOf('=');
                    if (equals != -1) {
                        ...
                        //$NON-NLS-1$
                        Util.log(IStatus.WARNING, "Invalid argument to add-reads: " + value);
                    }
                }
    ...
    public void applyModuleUpdates(IUpdatableModule compilerModule, UpdateKind kind) {
        ...
        if (name != ModuleBinding.UNNAMED) {
            // can't update the unnamed module
            UpdatesByKind updates = this.moduleUpdates.get(String.valueOf(name));
            if (updates != null) {
                for (Consumer<IUpdatableModule> update : updates.getList(kind, false)) update.accept(compilerModule);
            }
        }
    }
