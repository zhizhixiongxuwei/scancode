...
public class ClassFileMatchLocator implements IIndexConstants {
    ...
    static final public long TARGET_ANNOTATION_BITS = TagBits.AnnotationForType | TagBits.AnnotationForParameter | TagBits.AnnotationForPackage | TagBits.AnnotationForMethod | TagBits.AnnotationForLocalVariable | TagBits.AnnotationForField | TagBits.AnnotationForConstructor | TagBits.AnnotationForAnnotationType | TagBits.AnnotationForModule | TagBits.AnnotationForRecordComponent;

    static final public char[] JAVA_LANG_ANNOTATION_ELEMENTTYPE = CharOperation.concatWith(TypeConstants.JAVA_LANG_ANNOTATION_ELEMENTTYPE, '.');
 ...
 */
    ...
    boolean matchBinary(SearchPattern pattern, Object binaryInfo, IBinaryType enclosingBinaryType) {
        switch(pattern.kind) {
            ...
            case OR_PATTERN:
                SearchPattern[] patterns = ((OrPattern) pattern).patterns;
                for (SearchPattern p : patterns) if (matchBinary(p, binaryInfo, enclosingBinaryType))
                    return true;
        }
        return false;
    }
