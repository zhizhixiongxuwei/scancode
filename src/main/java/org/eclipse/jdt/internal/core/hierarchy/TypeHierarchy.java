...
public class TypeHierarchy implements ITypeHierarchy, IElementChangedListener {
    ...
    static final public byte VERSION = 0x0000;

    // SEPARATOR
    static final public byte SEPARATOR1 = '\n';

    static final public byte SEPARATOR2 = ',';

    static final public byte SEPARATOR3 = '>';
    ...
    protected void cacheSuperclass(IType type, IType superclass) {
        if (superclass != null) {
            if (superclass.equals(type)) {
                //$NON-NLS-1$//$NON-NLS-2$
                Util.log(IStatus.ERROR, "Type " + type.getFullyQualifiedName() + " is it's own superclass");
                // refuse to enter what could lead to a stackoverflow later
                return;
            }
    ...
    public void fireChange() {
        ...
        if (DEBUG) {
            //$NON-NLS-1$ //$NON-NLS-2$
            trace("FIRING hierarchy change [" + Thread.currentThread() + "]");
            if (this.focusType != null) {
                //$NON-NLS-1$
                trace("    for hierarchy focused on " + ((JavaElement) this.focusType).toStringWithAncestors());
            }
        }
    ...
    private Set<IType> getAllSupertypes0(IType type, Set<IType> supers) {
        ...
        if (superinterfaces == null) {
            // type is not part of the hierarchy
            return supers;
        }
        for (IType superinterface : superinterfaces) {
            ...
            case IJavaElementDelta.ADDED:
                try {
                    ...
                    for (IClasspathEntry cpe : classpath) {
                        if (cpe.getEntryKind() == IClasspathEntry.CPE_PROJECT && cpe.getPath().equals(element.getPath())) {
                            return true;
                        }
                    }
                    if (this.focusType != null) {
                        ...
                        for (IClasspathEntry cpe : classpath) {
                            if (cpe.getEntryKind() == IClasspathEntry.CPE_PROJECT && cpe.getPath().equals(hierarchyProject)) {
                                return true;
                            }
                        }
                    }
        ...
        try {
            ...
            if (DEBUG) {
                ...
                if (this.computeSubtypes) {
                    //$NON-NLS-1$ //$NON-NLS-2$
                    trace("CREATING TYPE HIERARCHY [" + Thread.currentThread() + "]");
                } else {
                    //$NON-NLS-1$ //$NON-NLS-2$
                    trace("CREATING SUPER TYPE HIERARCHY [" + Thread.currentThread() + "]");
                }
                if (this.focusType != null) {
                    //$NON-NLS-1$
                    trace("  on type " + ((JavaElement) this.focusType).toStringWithAncestors());
                }
            }
            compute();
            initializeRegions();
            ...
            if (DEBUG) {
                if (this.computeSubtypes) {
                    //$NON-NLS-1$ //$NON-NLS-2$
                    trace("CREATED TYPE HIERARCHY in " + (System.currentTimeMillis() - start) + "ms");
                } else {
                    //$NON-NLS-1$ //$NON-NLS-2$
                    trace("CREATED SUPER TYPE HIERARCHY in " + (System.currentTimeMillis() - start) + "ms");
                }
                trace(this.toString());
            }
        } catch (JavaModelException e) {
