...
public final class ImportRewriteAnalyzer {
    ...
    private static int getBlankLinesBetweenImportGroups(IJavaProject javaProject) {
        ...
        try {
            ...
            String message = String.format(//$NON-NLS-1$
            "Could not parse the value of %s as an integer: %s", DefaultCodeFormatterConstants.FORMATTER_BLANK_LINES_BETWEEN_IMPORT_GROUPS, blankLinesOptionValue);
    ...
    private static RewriteSite determineRewriteSite(CompilationUnit compilationUnit, List<OriginalImportEntry> originalImports) throws JavaModelException {
    ...
    final public List<OriginalImportEntry> originalImportEntries;

    final public List<ImportName> originalImportsList;

    final public Set<ImportName> originalImportsSet;

    final public ImportDeclarationWriter importDeclarationWriter;

    final public ImportAdder importAdder;
    ...
    private List<ImportName> computeImportOrder(IProgressMonitor progressMonitor) throws JavaModelException {
        ...
        Collection<OnDemandReduction> onDemandReductions = this.onDemandComputer.identifyPossibleReductions(new HashSet<>(importsWithoutImplicits), touchedContainers, allTypeExplicitSimpleNames, allStaticExplicitSimpleNames);
    ...
    private Set<ImportName> determineTouchedContainers() {
        Collection<ImportName> touchedContainers = new ArrayList<>(this.importsToAdd.size() + this.importsToRemove.size());
