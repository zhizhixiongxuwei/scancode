...
public abstract class AbstractIndexer implements IIndexConstants {
    ...
    final public SearchDocument document;
    ...
    public void addAnnotationTypeDeclaration(int modifiers, char[] packageName, char[] name, char[][] enclosingTypeNames, boolean secondary) {
        ...
        addIndexEntry(SUPER_REF, SuperTypeReferencePattern.createIndexKey(modifiers, packageName, name, enclosingTypeNames, null, ANNOTATION_TYPE_SUFFIX, annotationSuperClass, ANNOTATION_TYPE_SUFFIX));
    ...
    public void addConstructorDeclaration(char[] typeName, int argCount, char[] signature, char[][] parameterTypes, char[][] parameterNames, int modifiers, char[] packageName, int typeModifiers, char[][] exceptionTypes, int extraFlags) {
        ...
        if (exceptionTypes != null)
            for (char[] exceptionType : exceptionTypes) addTypeReference(exceptionType);
    }

    public void addConstructorReference(char[] typeName, int argCount) {
        char[] simpleTypeName = CharOperation.lastSegment(typeName, '.');
        // The implementation will add simple name to index.
        addTypeReference(typeName);
        // second entry for super type
        addIndexMetaQualification(typeName, true);
    ...
    public void addModuleExportedPackages(char[] packageName) {
        ...
        for (char[] token : tokens) addNameReference(token);
    }
    ...
    protected void addTypeDeclaration(int modifiers, char[] packageName, char[] name, char[][] enclosingTypeNames, boolean secondary) {
        ...
        if (enclosingTypeNames != null && enclosingTypeNames.length > 0) {
            addIndexMetaQualification(CharOperation.concat(packageName, '.', CharOperation.concatWith(enclosingTypeNames, '$'), '$', name), false);
        } else {
            addIndexMetaQualification(CharOperation.concat(packageName, name, '.'), false);
        }
    }
