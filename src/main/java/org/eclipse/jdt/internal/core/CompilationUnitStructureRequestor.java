...
public class CompilationUnitStructureRequestor extends ReferenceInfoAdapter implements ISourceElementRequestor {
     ...
     */
    public CompilationUnitElementInfo unitInfo;

    /**
     * The import container info - null until created
     */
    public ImportContainerInfo importContainerInfo = null;

    public ImportContainer importContainer;
     ...
     * info objects.
     */
    public Map<IJavaElement, IElementInfo> newElements;
 ...
 */
    ...
    private SourceMethodElementInfo createMethodInfo(MethodInfo methodInfo, SourceMethod handle) {
        ...
        for (int i = 0, length = parameterNames.length; i < length; i++) parameterNames[i] = DeduplicationUtil.intern(parameterNames[i]);
        ...
        for (int i = 0, length = exceptionTypes.length; i < length; i++) exceptionTypes[i] = DeduplicationUtil.intern(exceptionTypes[i]);
        this.newElements.put(handle, info);
    ...
    private LocalVariable[] acceptMethodParameters(AbstractVariableDeclaration[] arguments, JavaElement methodHandle, MethodInfo methodInfo) {
        ...
        for (int i = 0; i < arguments.length; i++) {
            ...
            result[i] = new LocalVariable(methodHandle, DeduplicationUtil.toString(argument.name), argument.declarationSourceStart, argument.declarationSourceEnd, argument.sourceStart, argument.sourceEnd, paramTypeSig, argument.annotations, argument.modifiers, true);
    ...
    private SourceTypeElementInfo createTypeInfo(TypeInfo typeInfo, SourceType handle) {
        ...
        for (int i = 0, length = typeNames == null ? 0 : typeNames.length; i < length; i++) typeNames[i] = DeduplicationUtil.intern(typeNames[i]);
