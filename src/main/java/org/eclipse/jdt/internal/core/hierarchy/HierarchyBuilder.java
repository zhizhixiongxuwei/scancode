...
public abstract class HierarchyBuilder {
    ...
    public void connect(IGenericType type, IType typeHandle, IType superclassHandle, IType[] superinterfaceHandles) {
        ...
        if (TypeHierarchy.DEBUG) {
            ...
            trace(//$NON-NLS-1$
            "  to superclass: " + (superclassHandle == null ? //$NON-NLS-1$
            "<None>" : ((JavaElement) superclassHandle).toStringWithAncestors()));
            //$NON-NLS-1$
            trace("  and superinterfaces:");
            if (superinterfaceHandles == null || superinterfaceHandles.length == 0) {
                ...
                for (IType superinterfaceHandle : superinterfaceHandles) {
                    if (superinterfaceHandle == null)
                        continue;
                    //$NON-NLS-1$
                    trace("    " + ((JavaElement) superinterfaceHandle).toStringWithAncestors());
                }
            }
        }
        // now do the caching
    ...
    protected IType getHandle(IGenericType genericType, ReferenceBinding binding) {
        ...
        if (genericType instanceof HierarchyType) {
            ...
            return new ResolvedBinaryType(classFile, classFile.getTypeName(), DeduplicationUtil.toString(binding.computeUniqueKey()));
    ...
    protected IType lookupBinaryHandle(IBinaryType typeInfo) {
        ...
        if (qualifiedName.equals(this.focusQualifiedName))
            return getType();
        NameLookup.Answer answer = this.nameLookup.findType(qualifiedName, false, flag, true, /* consider secondary types */
