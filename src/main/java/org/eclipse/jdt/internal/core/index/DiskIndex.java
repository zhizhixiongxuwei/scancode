...
public class DiskIndex {
    ...
    //$NON-NLS-1$
    public static final String INDEX_VERSION = "1.134";

    //$NON-NLS-1$
    public static final String SIGNATURE = "INDEX VERSION " + INDEX_VERSION;
    ...
    SimpleSet addDocumentNames(String substring, MemoryIndex memoryIndex) throws IOException {
        ...
        if (substring == null) {
            if (memoryIndex == null) {
                for (String docName : docNames) results.add(docName);
            } else {
            ...
            if (memoryIndex == null) {
                for (String docName : docNames) if (docName.startsWith(substring, 0))
                    results.add(docName);
            } else {
                SimpleLookupTable docsToRefs = memoryIndex.docsToReferences;
                for (String docName : docNames) {
    ...
    HashtableOfObject addQueryResults(char[][] categories, char[] key, int matchRule, MemoryIndex memoryIndex) throws IOException {
        ...
        if (key == null) {
            for (char[] category : categories) {
                ...
                HashtableOfObject wordsToDocNumbers = readCategoryTable(category, true);
    ...
    private String[] computeDocumentNames(String[] onDiskNames, int[] positions, SimpleLookupTable indexedDocuments, MemoryIndex memoryIndex) {
        ...
        if (onDiskLength == 0) {
            ...
            for (Object newDocName : added) if (newDocName != null)
    ...
    private void mergeCategories(DiskIndex onDisk, int[] positions, OutputStream stream) throws IOException {
        ...
        for (char[] categoryName : categoryNames) if (categoryName != null)
    ...
    DiskIndex mergeWith(MemoryIndex memoryIndex) throws IOException {
        ...
        if (indexFilePath.endsWith(TMP_EXT)) {
            // the tmp file could not be renamed last time
        ...
        try {
            ...
            try {
                ...
                //$NON-NLS-1$
                throw new IOException("Failed to delete index file " + this.indexLocation, e2);
            }
            if (!usingTmp) {
                try {
                    ...
                    try {
                        ...
                        //$NON-NLS-1$
                        Util.log(ioe, "mergeWith - Failed to rename index " + this.indexLocation);
                        usingTmp = true;
                    }
                }
            }
            ...
            try {
                ...
                //$NON-NLS-1$
                Util.log(e2, "mergeWith - Failed to delete temp index " + newDiskIndex.indexLocation);
            }
            throw e;
        }
    ...
    private synchronized String[] readAllDocumentNames() throws IOException {
        ...
        try {
            ...
            //$NON-NLS-1$
            ILog.get().warn("Java Index broken - will be automatically deleted to repair: " + this.indexLocation, ue);
        ...
        try {
            try (InputStream stream = this.indexLocation.getInputStream()) {
                ...
                try {
                    ...
                    "Failed to read index data from " + this.indexLocation + " at offset " + offset + " and size " + //$NON-NLS-1$
                    size;
