...
public class BinaryType extends BinaryMember implements IType, SuffixConstants {
    ...
    static final public IField[] NO_FIELDS = new IField[0];

    static final public IMethod[] NO_METHODS = new IMethod[0];

    static final public IType[] NO_TYPES = new IType[0];

    static final public IInitializer[] NO_INITIALIZERS = new IInitializer[0];
 ...
 */
    ...
    public void codeComplete(char[] snippet, int insertion, int position, char[][] localVariableTypeNames, char[][] localVariableNames, int[] localVariableModifiers, boolean isStatic, CompletionRequestor requestor, WorkingCopyOwner owner, IProgressMonitor monitor) throws JavaModelException {
        ...
        if (NameLookup.VERBOSE) {
            //$NON-NLS-1$ //$NON-NLS-2$
            JavaModelManager.trace(Thread.currentThread() + " TIME SPENT in NameLoopkup#seekTypesInSourcePackage: " + environment.nameLookup.timeSpentInSeekTypesInSourcePackage + "ms");
            //$NON-NLS-1$ //$NON-NLS-2$
            JavaModelManager.trace(Thread.currentThread() + " TIME SPENT in NameLoopkup#seekTypesInBinaryPackage: " + environment.nameLookup.timeSpentInSeekTypesInBinaryPackage + "ms");
        }
    }
    ...
    public IType getDeclaringType() {
        ...
        if (classFile.isOpen()) {
            ...
            if (lastDollar == -1) {
                ...
                return new BinaryType((JavaElement) getPackageFragment().getClassFile(enclosingClassFileName), DeduplicationUtil.intern(Util.localTypeName(enclosingName, enclosingName.lastIndexOf('$'), enclosingName.length())));
            }
        }
    }
