...
public class VerboseElementCache<K extends IJavaElement & IOpenable> extends ElementCache<K> {
    ...
    protected boolean makeSpace(int space) {
        ...
        if (!fillingRatio.equals(newFillingRatio)) {
            //$NON-NLS-1$
            JavaModelManager.trace(Thread.currentThread() + " " + new Date(System.currentTimeMillis()).toString());
            //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            JavaModelManager.trace(Thread.currentThread() + " MADE SPACE FOR " + fillingRatio + " (NOW " + NumberFormat.getInstance().format(fillingRatio()) + "% full)");
            //$NON-NLS-1$
            JavaModelManager.trace(Thread.currentThread() + " WHILE OPENING " + ((JavaElement) this.beingAdded).toStringWithAncestors());
            //$NON-NLS-1$
