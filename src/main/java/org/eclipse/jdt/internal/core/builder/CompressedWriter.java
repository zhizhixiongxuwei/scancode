...
public class CompressedWriter {
    ...
    final public DataOutputStream out;

    // --- context state: ---
    public char[] lastName;

    final public HashMap<String, Integer> lastWords;
    ...
    public void writeIntWithHint(int v, int[] typical) throws IOException {
        ...
        // write index
        writeIntInRange(i, typical.length + 1);
        if (i == typical.length) {
            // write plain
            writeInt(v);
        }
    ...
    public void writeStringUsingDictionary(String v) throws IOException {
        ...
        if (index == null) {
            ...
            // write magic 0, which is skipped in the other case
            writeIntInRange(0, size + 1);
            this.out.writeUTF(v);
        } else {
            // write index+1 to skip magic 0
            writeIntInRange(index.intValue() + 1, size + 1);
        }
    }
