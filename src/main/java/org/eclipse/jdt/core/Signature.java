...
public final class Signature {
    ...
    //$NON-NLS-1$
    static final public char[] BOOLEAN = "boolean".toCharArray();

    //$NON-NLS-1$
    static final public char[] BYTE = "byte".toCharArray();
    ...
    private static int encodeTypeSignature(char[] typeName, int start, boolean isResolved, int length, StringBuilder buffer) {
        ...
        while (true) {
            ...
            checkPos = checkNextChar(typeName, '&', pos, length, true);
            if (checkPos > 0) {
                if (// the constant name is wrong, its value is correct :-X
                buffer.charAt(0) != C_UNION)
