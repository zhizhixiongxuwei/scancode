...
public class ASTParser {
            ...
            if (this.sourcepaths != null) {
                for (int i = 0, max = this.sourcepaths.length; i < max; i++) {
                    ...
                    if (sourcePath != null) {
                        ...
                        main.processPathEntries(Main.DEFAULT_SIZE_CLASSPATH, allClasspaths, sourcePath, encoding, true, false);
                    }
                }
            }
    ...
    private void checkForSystemLibrary(List<Classpath> allClasspaths) {
        ...
        // default for 1.8 setting without a valid project
        ...
        if (CompilerOptions.versionToJdkLevel(compliance) >= ClassFileConstants.JDK9) {
            ...
            if (!hasSystemLibrary)
                //$NON-NLS-1$
                throw new IllegalStateException("Missing system library", exception);
        }
    }
     ...
     */
    static IBinding[] resolve(final IJavaElement[] elements, int apiLevel, Map<String, String> compilerOptions, IJavaProject javaProject, WorkingCopyOwner owner, int flags, ICompilationUnitResolver unitResolver, IProgressMonitor monitor) {
        final int length = elements.length;
        // a map from ICompilationUnit to int[] (positions in elements)
        final HashMap<IJavaElement, IntArrayList> sourceElementPositions = new HashMap<>();
        int cuNumber = 0;
            ...
            if (cu != null) {
                ...
                try {
                    ...
                    if (element instanceof BinaryMember)
                        ...
                        key = ((org.eclipse.jdt.internal.core.TypeParameter) element).getKey(true);
