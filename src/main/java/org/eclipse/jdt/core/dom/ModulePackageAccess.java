...
public abstract class ModulePackageAccess extends ModuleDirective {
     ...
     */
    public ASTNode.NodeList modules = null;
    ...
    static final ChildPropertyDescriptor internalNamePropertyFactory(Class nodeClass) {
        //$NON-NLS-1$
        return new ChildPropertyDescriptor(nodeClass, "name", Name.class, MANDATORY, NO_CYCLE_RISK);
    }
    ...
    static final ChildListPropertyDescriptor internalModulesPropertyFactory(Class nodeClass) {
        //$NON-NLS-1$
        return new ChildListPropertyDescriptor(nodeClass, "modules", Name.class, NO_CYCLE_RISK);
    }
    ...
    public Name getName() {
        if (this.name == null) {
            ...
            synchronized (this) {
                if (this.name == null) {
                    preLazyInit();
                    this.name = postLazyInit(this.ast.newQualifiedName(new SimpleName(this.ast), new SimpleName(this.ast)), internalNameProperty());
                }
            }
        }
        return this.name;
