    ...
    private void checkCanceled() {
        if (this.monitor != null && this.monitor.isCanceled()) {
            if (NameLookup.VERBOSE) {
                //$NON-NLS-1$
                trace(Thread.currentThread() + " CANCELLING LOOKUP ");
            }
            throw new AbortCompilation(true, /*silent*/
    ...
    public NameEnvironmentAnswer findType(char[] typeName, char[][] packageName, boolean searchWithSecondaryTypes, char[] moduleName) {
        ...
        if (answer == null && searchWithSecondaryTypes) {
            ...
            for (Classpath classpath : this.classpaths) {
                ...
                if (!strategy.matchesWithName(classpathDirectory, loc -> loc.getModule() != null, loc -> loc.servesModule(moduleName))) {
                    continue;
                }
