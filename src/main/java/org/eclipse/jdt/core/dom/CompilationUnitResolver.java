...
public class CompilationUnitResolver extends Compiler {
    ...
    private static final class ECJCompilationUnitResolver implements ICompilationUnitResolver {
        ...
        @Override
        public void resolve(String[] sourceFilePaths, String[] encodings, String[] bindingKeys, FileASTRequestor requestor, int apiLevel, Map<String, String> compilerOptions, List<Classpath> classpath, int flags, IProgressMonitor monitor) {
        ...
        @Override
        public CompilationUnit toCompilationUnit(org.eclipse.jdt.internal.compiler.env.ICompilationUnit sourceUnit, final boolean initialNeedsToResolveBinding, IJavaProject project, List<Classpath> classpaths, int focalPosition, int apiLevel, Map<String, String> compilerOptions, WorkingCopyOwner parsedUnitWorkingCopyOwner, WorkingCopyOwner typeRootWorkingCopyOwner, int flags, IProgressMonitor monitor) {
    ...
    public static CompilationUnit convert(CompilationUnitDeclaration compilationUnitDeclaration, char[] source, int apiLevel, Map options, boolean needToResolveBindings, WorkingCopyOwner owner, DefaultBindingResolver.BindingTables bindingTables, int flags, IProgressMonitor monitor, boolean fromJavaProject) {
        return convert(compilationUnitDeclaration, source, apiLevel, options, needToResolveBindings, owner, bindingTables, flags, monitor, fromJavaProject, null);
    }
    ...
    public static void parse(ICompilationUnit[] compilationUnits, ASTRequestor astRequestor, int apiLevel, Map options, int flags, IProgressMonitor monitor) {
        ...
        for (int i = 0; i < unitLength; i++) {
            ...
            // convert AST
            CompilationUnit node = convert(compilationUnitDeclaration, parser.scanner.getSource(), apiLevel, options, false, /* don't resolve binding */
    ...
    public static void resolve(ICompilationUnit[] compilationUnits, String[] bindingKeys, ASTRequestor requestor, int apiLevel, Map options, IJavaProject javaProject, WorkingCopyOwner owner, int flags, IProgressMonitor monitor) {
        ...
        try {
            ...
            compilerOptions.ignoreMethodBodies = (flags & ICompilationUnit.IGNORE_METHOD_BODIES) != 0;
            CompilationUnitResolver resolver = new CompilationUnitResolver(environment, getHandlingPolicy(), compilerOptions, getRequestor(), problemFactory, subMonitor, javaProject != null);
    ...
    public static void resolve(String[] sourceUnits, String[] encodings, String[] bindingKeys, FileASTRequestor requestor, int apiLevel, Map options, List classpaths, int flags, IProgressMonitor monitor) {
        ...
        try {
            ...
            CompilerOptions compilerOptions = getCompilerOptions(options, (flags & ICompilationUnit.ENABLE_STATEMENTS_RECOVERY) != 0);
            compilerOptions.ignoreMethodBodies = (flags & ICompilationUnit.IGNORE_METHOD_BODIES) != 0;
            CompilationUnitResolver resolver = new CompilationUnitResolver(environment, getHandlingPolicy(), compilerOptions, getRequestor(), problemFactory, subMonitor, false);
                ...
                try {
                    ...
                    if (source != null) {
                        ...
                        try {
                            ...
                            //$NON-NLS-1$
                            throw new RuntimeException("Error on " + source.getPath(), e);
                        }
                        worked(1);
    ...
    public CompilationUnitDeclaration resolve(org.eclipse.jdt.internal.compiler.env.ICompilationUnit sourceUnit, boolean verifyMethods, boolean analyzeCode, boolean generateCode) {
        ...
        sourceUnit, null, /*no node searcher*/
        verifyMethods, analyzeCode, generateCode);
    }
    ...
    boolean resolvedRequestedSourcesAndKeys(int unitIndexToProcess) {
        ...
        for (Object source : sources) if (source != null)
            return false;
        Object[] keys = this.requestedKeys.valueTable;
        for (Object key : keys) if (key != null)
            return false;
        return true;
    }
    ...
    public CompilationUnitDeclaration resolve(CompilationUnitDeclaration unit, org.eclipse.jdt.internal.compiler.env.ICompilationUnit sourceUnit, boolean verifyMethods, boolean analyzeCode, boolean generateCode) {
        return resolve(unit, sourceUnit, null, /*no node searcher*/
        verifyMethods, analyzeCode, generateCode);
    }

    public static CompilationUnit toCompilationUnit(org.eclipse.jdt.internal.compiler.env.ICompilationUnit sourceUnit, final boolean initialNeedsToResolveBinding, IJavaProject project, List<Classpath> classpaths, NodeSearcher nodeSearcher, int apiLevel, Map<String, String> compilerOptions, WorkingCopyOwner parsedUnitWorkingCopyOwner, WorkingCopyOwner typeRootWorkingCopyOwner, int flags, IProgressMonitor monitor) {
        ...
        try {
            if (initialNeedsToResolveBinding) {
                try {
                    ...
                    compilationUnitDeclaration = CompilationUnitResolver.parse(sourceUnit, nodeSearcher, compilerOptions, flags);
            ...
            return CompilationUnitResolver.convert(compilationUnitDeclaration, sourceUnit.getContents(), apiLevel, compilerOptions, needsToResolveBindingsState, typeRootWorkingCopyOwner, needsToResolveBindingsState ? new DefaultBindingResolver.BindingTables() : null, flags, monitor, project != null, project);
        } finally {
