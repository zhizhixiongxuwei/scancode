...
public final class ExternalAnnotationUtil {
    ...
    static final public int POSITION_RETURN_TYPE = -1;

    static final public int POSITION_FULL_SIGNATURE = -2;
    ...
    private static void annotateMember(String typeName, IFile file, String selector, String originalSignature, String annotatedSignature, int updatePosition, MergeStrategy mergeStrategy, IProgressMonitor monitor) throws CoreException, IOException, IllegalArgumentException {
        if (!file.exists()) {
            ...
            // new entry:
            if (selector != null) {
                // otherwise we are annotating a class type parameter
                ...
                while ((line = reader.readLine()) != null) {
                    if (selector == null) {
                        ...
                        if (line.trim().startsWith("<")) {
                            ...
                            // reading old annotated type parameters //$NON-NLS-1$
    ...
    private static String updateSignature(String originalSignature, String annotatedSignature, int updatePosition, MergeStrategy mergeStrategy) {
        ...
        if (updatePosition <= POSITION_TYPE_PARAMETER) {
            ...
            SignatureWrapper wrapper = new SignatureWrapper(originalSignature.toCharArray(), true);
            // skip '<'
            wrapper.start = 1;
    ...
    private static boolean updateType(StringBuilder buf, char[] oldType, char[] newType, MergeStrategy mergeStrategy) {
        ...
        try {
            ...
            // from several locations inside match() or mergeAnnotation().
            //$NON-NLS-1$ //$NON-NLS-2$
    ...
    private static boolean match(StringBuilder buf, SignatureWrapper sig1, SignatureWrapper sig2, char expected, boolean force) {
        ...
        if (match1 != match2) {
            ...
            new StringBuilder("Mismatching type structures ").append(sig1.signature).append(" vs ").append(//$NON-NLS-1$
            sig2.signature);
            throw new IllegalArgumentException(msg.toString());
        }
        if (match1) {
     ...
     */
    private static void writeFile(IFile annotationFile, StringBuilder head, String annotatedSignature, String nextLines, BufferedReader tailReader, IProgressMonitor monitor) throws CoreException, IOException {
        ...
        if (file.exists()) {
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(file.getContents()))) {
                ...
                while (true) {
                    ...
                    } else if (line != null && line.trim().startsWith("<")) {
                        //$NON-NLS-1$
                        // original signature:
                        if (originalSignature.equals(ExternalAnnotationProvider.extractSignature(line))) {
                            // annotated signature:
