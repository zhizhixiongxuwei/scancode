...
public class DefaultGnuToolFactory implements IGnuToolFactory {
    ...
    protected String[] getEnvironment() {
        ...
        return //$NON-NLS-1$
        Arrays.stream(vars).map(v -> String.format("%s=%s", v.getName(), v.getValue())).toArray(String[]::new);
    }
    ...
    protected String getToolPrefix() {
        ...
        ICdtVariable var = Arrays.stream(userVars).filter(v -> v.getName().equals(GNU_TOOL_PREFIX_VARIABLE)).findFirst().orElse(null);
        ...
        if (var != null) {
            try {
                ...
                Platform.getLog(getClass()).log(//$NON-NLS-1$
                Status.error("Error getting CDT variable string value: " + GNU_TOOL_PREFIX_VARIABLE, e));
            }
        }
        //$NON-NLS-1$
        return "";
