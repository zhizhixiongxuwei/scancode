...
public class DwarfReader extends Dwarf implements ISymbolReader, ICompileOptionsFinder {
    ...
    final static public String[] DWARF_SectionsToParse = { DWARF_DEBUG_INFO, DWARF_DEBUG_LINE, DWARF_DEBUG_ABBREV, // this is optional. Some compilers don't generate it.
    DWARF_DEBUG_STR, DWARF_DEBUG_MACRO, DWARF_DEBUG_LINE_STR, DWARF_DEBUG_STR_OFFSETS };

    final static public String[] DWARF_ALT_SectionsToParse = { DWARF_DEBUG_STR, DWARF_DEBUG_MACRO };

    final public Collection<String> m_fileCollection = new HashSet<>();

    final public Map<Long, String> m_stmtFileMap = new HashMap<>();

    final public Map<String, ArrayList<String>> m_compileOptionsMap = new HashMap<>();
    ...
    private class ContentForm {
        ...
        // content type code DW_LNCT_*
        long lnct;

        // content form code DW_FORM_*
        long form;
    }
    ...
    public void init(Elf exe) throws IOException {
        ...
        if (!have_build_id) {
            ...
            if (gnuDebugLink != null) {
                ...
                if (data != null) {
                    // we have non-empty debug info link
                    try {
                        ...
                        if (debugName.length() > 0) {
                            ...
                            if (!debugFile.exists()) {
                                ...
                                //$NON-NLS-1$
                                debugFile = p.append(".debug").append(debugName).toFile();
                                if (!debugFile.exists())
    ...
    private static String printHexBinary(byte[] byteArray) {
        ...
        for (byte b : byteArray) //$NON-NLS-1$
    ...
    private ContentForm readContentForm(ByteBuffer in) throws IOException {
        ...
        // DW_LNCT_*
        contentForm.lnct = read_unsigned_leb128(in);
        // DW_FORM_*
        contentForm.form = read_unsigned_leb128(in);
        return contentForm;
    }
    ...
    private void addSourceFilesDwarf5(String cuCompDir, ByteBuffer data, byte offsetSize) throws IOException {
        ...
        for (int directory_index = 0; directory_index < directories_count; directory_index++) {
            for (ContentForm contentForm : directoryContentForms) {
                if ((DwarfConstants.DW_FORM_line_strp == contentForm.form) && (DwarfConstants.DW_LNCT_path == contentForm.lnct)) {
                    ...
                    ILog.get().warn(//$NON-NLS-1$
                    String.//$NON-NLS-1$
                    format(//$NON-NLS-1$
                    "DWARF directory content 0x%x form 0x%x not handled", contentForm.lnct, contentForm.form));
                }
            }
        }
        ...
        for (int file_index = 0; file_index < file_names_count; file_index++) {
            ...
            for (ContentForm contentForm : fileNameContentForms) {
                if ((DwarfConstants.DW_FORM_line_strp == contentForm.form) && (DwarfConstants.DW_LNCT_path == contentForm.lnct)) {
                ...
                } else if ((DwarfConstants.DW_FORM_udata == contentForm.form) && (DwarfConstants.DW_LNCT_directory_index == contentForm.lnct)) {
                    long directory_index = read_unsigned_leb128(data);
                    directory = directories.get((int) directory_index);
                } else if ((DwarfConstants.DW_FORM_data16 == contentForm.form) && (DwarfConstants.DW_LNCT_MD5 == contentForm.lnct)) {
                    // skip MD5 digest
                    ...
                    String.//$NON-NLS-1$
                    format(//$NON-NLS-1$
                    "DWARF file content 0x%x form 0x%x not handled", contentForm.lnct, contentForm.form));
                }
            }
    ...
    void // compilation directory of the CU
    parseSourceInCULineInfo(// compilation directory of the CU
    String cuCompDir, // offset of the CU line table in .debug_line section
    int cuStmtList) {
        ...
        if (data != null) {
            try {
                ...
                short skip_bytes = 8;
                if (version >= 5) {
                    // see address_size and segment_selector_size
                    skip_bytes += 2;
                }
                if (version >= 4)
                    // see maximum_operations_per_instruction
                    skip_bytes += 1;
                if (dwarf64Bit)
                    // see prologue length for 64-bit DWARF format
                    skip_bytes += 4;
    ...
    private void getSourceFilesFromDebugLineSection() {
		 ...
		 * opcode_base:				1
		 * standard_opcode_lengths: (value of opcode_base)		 */
        // offset in the .debug_line section
        ...
        try {
            while (lineTableStart < sectionSize - minHeaderSize) {
                ...
                if (lineTableStart < sectionSize - minHeaderSize && (lineTableStart & 0x3) != 0) {
                    ...
                    boolean dataValid = // One source file has that much line data ?
                    ltLength > minHeaderSize && ltLength < 16 * 64 * 1024 && dwarfVer > 0 && // ver 5 is still draft at present.
                    dwarfVer < 5 && minInstLengh > 0 && minInstLengh <= 8;
                    if (// padding exists !
                    !dataValid)
    ...
    private String addSourceFile(String dir, String name) {
        if (name == null || name.length() == 0)
            return null;
        if (//  don't count the entry "<internal>" from GCCE compiler
        name.charAt(0) == '<')
            return null;
    ...
    @Override
    void processDebugInfoEntry(IDebugEntryRequestor requestor, AbbreviationEntry entry, List<Dwarf.AttributeValue> list) throws IOException {
    ...
    void processCompileUnit(IDebugEntryRequestor requestor, List<AttributeValue> list) throws IOException {
            ...
            try {
                ...
                switch(name) {
                    ...
                    case DwarfConstants.DW_AT_comp_dir:
                        if ((av.attribute.form == DwarfConstants.DW_FORM_strp) || (av.attribute.form == DwarfConstants.DW_FORM_line_strp)) {
                            cuCompDir = (String) av.value;
                        } else if ((av.attribute.form == DwarfConstants.DW_FORM_strx1) && (offsets != null) && (offsetsBase != -1L)) {
                            ...
                            //$NON-NLS-1$
                            ILog.get().warn(String.format("DW_AT_comp_dir form 0x%x not handled", av.attribute.form));
                        }
                        break;
        ...
        if (// this CU has "stmt_list" attribute
        stmtList > -1)
    ...
    private void getCommandMacrosFromMacroSection() {
        ...
        //$NON-NLS-1$
        parseMacroSection(data, str, altstr, fixupList, fixupAltList, "=FIXUP=", DEBUG, t_macros);
        ...
        if (altdata != null) {
            ...
            //$NON-NLS-1$
            parseMacroSection(altdata, altstr, altstr, fixupAltList, fixupAltList, "=FIXUPALT=", DEBUG, t_alt_macros);
        }
        // Fix up all forward references from transparent includes
        //$NON-NLS-1$
        fixupMacros(fixupList, "=FIXUP=", DEBUG, t_macros);
        ...
        //$NON-NLS-1$
        fixupMacros(fixupAltList, "=FIXUPALT=", DEBUG, t_alt_macros);
    }
    ...
    private void fixupMacros(Set<String> fixupList, String fixupMarker, boolean DEBUG, HashMap<Long, ArrayList<String>> t_macros) {
        for (String name : fixupList) {
            ...
            m_compileOptionsMap.put(name, macros);
        }
    }
                        ...
                        case DwarfConstants.DW_MACRO_start_file:
                            {
                                ...
                                if (fileName != null)
                                    if (DEBUG)
                                        ...
                                        " DW_MACRO_start_file - lineno: " + lineno + " filenum: " + filenum + " " + //$NON-NLS-1$
                                        fileName);
                                    else if (DEBUG)
                                        System.out.println(//$NON-NLS-1$ //$NON-NLS-2$
                                        " DW_MACRO_start_file - lineno: " + lineno + " filenum: " + filenum);
                            }
                            break;
                        ...
                        case DwarfConstants.DW_MACRO_define:
                            {
                                ...
                                if (DEBUG)
                                    System.out.println(//$NON-NLS-1$ //$NON-NLS-2$
                                    " DW_MACRO_define - lineno : " + lineno + " macro : " + string);
                            }
                            break;
                        case DwarfConstants.DW_MACRO_undef:
                            {
                                ...
                                if (DEBUG)
                                    System.out.println(//$NON-NLS-1$ //$NON-NLS-2$
                                    " DW_MACRO_undef - lineno : " + lineno + " macro : " + macro);
                            }
                            break;
                        case DwarfConstants.DW_MACRO_define_indirect:
                            {
                                ...
                                if (DEBUG)
                                    System.out.println(//$NON-NLS-1$ //$NON-NLS-2$
                                    " DW_MACRO_define_indirect - lineno : " + lineno + " macro : " + macro);
                            }
                            break;
                        case DwarfConstants.DW_MACRO_define_indirect_alt:
                            {
                                ...
                                if (DEBUG)
                                    System.out.println(//$NON-NLS-1$ //$NON-NLS-2$
                                    " DW_MACRO_define_indirect_alt - lineno : " + lineno + " macro : " + macro);
                            }
                            break;
                        case DwarfConstants.DW_MACRO_undef_indirect:
                            {
                                ...
                                if (DEBUG)
                                    System.out.println(//$NON-NLS-1$ //$NON-NLS-2$
                                    " DW_MACRO_undef_indirect - lineno : " + lineno + " macro : " + macro);
                            }
                            break;
                        case DwarfConstants.DW_MACRO_undef_indirect_alt:
                            {
                                ...
                                if (DEBUG)
                                    System.out.println(//$NON-NLS-1$ //$NON-NLS-2$
                                    " DW_MACRO_undef_indirect_alt - lineno : " + lineno + " macro : " + macro);
                            }
                            break;
                        case DwarfConstants.DW_MACRO_transparent_include:
                            {
                                ...
                                if (foundMacros != null)
                                    ...
                                    // leave a marker we can fix up later
                        ...
                        case DwarfConstants.DW_MACRO_transparent_include_alt:
                            {
                                ...
                                offset = (offset_size_8 ? read_8_bytes(data) : read_4_bytes(data));
                                if (lt_offset >= 0) {
                                    // leave a marker we can fix up later //$NON-NLS-1$
                        ...
                        case DwarfConstants.DW_MACRO_end:
                            {
                                if (lt_offset < 0) {
                                    if (DEBUG)
                                        ...
                                        "creating transparent include macros for offset: " + original_position);
