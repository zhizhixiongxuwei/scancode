...
public class XCoff32 {
    ...
    public static class FileHeader {
        ...
        // Consts
        // XCOFF32
        public final static int U802TOCMAGIC = 0x01df;

        // obsolete XCOFF64 - not used
        public final static int U803TOCMAGIC = 0x01e7;

        // discontinued AIX XCOFF64
        public final static int U803XTOCMAGIC = 0x01ef;

        // XCOFF64
        public final static int U64_TOCMAGIC = 0x01f7;
        ...
        /* 00-01 2 bytes: magic number                 */
        public short f_nscns;

        /* 02-03 2 bytes: number of sections: 2 bytes  */
        public int f_timdat;

        /* 04-07 4 bytes: time & date stamp            */
        public int f_symptr;

        /* 08-11 4 bytes: file pointer to symtab       */
        public int f_nsyms;

        /* 12-15 4 bytes: number of symtab entries     */
        public short f_opthdr;

        /* 16-17 2 bytes: sizeof(optional hdr)         */
    ...
    public static class OptionalHeader {
        ...
        // First 28 bytes same as for COFF
        public final static int AOUTHDRSZ = 72;

        // Fields (as in COFF)
        public short magic;

        /* 2 bytes: type of file (0x010B)               	*/
        public short vstamp;

        /* 2 bytes: version stamp (1)                   	*/
        public int tsize;

        /* 4 bytes: text size in bytes, padded to FW bdry	*/
        public int dsize;

        /* 4 bytes: initialized data "  "               	*/
        public int bsize;

        /* 4 bytes: uninitialized data "   "            	*/
        public int entry;

        /* 4 bytes: entry pt.                           	*/
        public int text_start;

        /* 4 bytes: base of text used for this file     	*/
        public int data_start;

        /* 4 bytes: base of data used for this file     	*/
        // Additional fields
        public int o_toc;

        /* 4 bytes: Address of TOC anchor 					*/
        public short o_snentry;

        /* 2 bytes: Section number for entry point			*/
        public short o_sntext;

        /* 2 bytes: Section number for .text				*/
        public short o_sndata;

        /* 2 bytes: Section number for .data				*/
        public short o_sntoc;

        /* 2 bytes: Section number for TOC					*/
        public short o_snloader;

        /* 2 bytes: Section number for loader data			*/
        public short o_snbss;

        /* 2 bytes: Section number for .bss					*/
        public short o_algntext;

        /* 2 bytes: Maximum alignment for .text				*/
        public short o_algndata;

        /* 2 bytes: Maximum alignment for .data				*/
        public short o_modtype;

        /* 2 bytes: Maximum alignment for .data				*/
        public byte o_cpuflag;

        /* 1 byte: Bit flags - cpu types of objects			*/
        public byte o_cputype;

        /* 1 byte: Reserved for cpu type					*/
        public int o_maxstack;

        /* 4 bytes: Maximum stack size allowed (bytes)		*/
        public int o_maxdata;

        /* 4 bytes: Maximum data size allowed (bytes)		*/
        public int o_debugger;

        /* 4 bytes: Reserved for debuggers					*/
    ...
    public static class SectionHeader {
        ...
        /* section contains exceptions info only */
        ...
        // 4 bytes: physical address, aliased s_nlib
        public int s_paddr;
        ...
        // 4 bytes: file ptr to raw data for section
        public int s_scnptr;

        // 4 bytes: file ptr to relocation
        public int s_relptr;

        // 4 bytes: file ptr to line numbers
        public int s_lnnoptr;

        // 2 bytes: number of relocation entries
        public short s_nreloc;

        // 2 bytes: number of line number entries
    ...
    public class Symbol {
        ...
        public final static int XMC_PR = 0;

        /* program code */
        public final static int XMC_RO = 1;

        /* read only constant */
        public final static int XMC_DB = 2;

        /* debug dictionary table */
        public final static int XMC_TC = 3;

        /* general TOC entry */
        public final static int XMC_UA = 4;

        /* unclassified */
        public final static int XMC_RW = 5;

        /* read/write data */
        public final static int XMC_GL = 6;

        /* global linkage */
        public final static int XMC_XO = 7;

        /* extended operation */
        public final static int XMC_SV = 8;

        /* 32-bit supervisor call descriptor csect */
        public final static int XMC_BS = 9;

        /* BSS class (uninitialized static internal) */
        public final static int XMC_DS = 10;

        /* csect containing a function descriptor */
        public final static int XMC_UC = 11;

        /* unnamed FORTRAN common */
        public final static int XMC_TI = 12;

        /* reserved */
        public final static int XMC_TB = 13;

        /* reserved */
        public final static int XMC_TC0 = 15;

        /* TOC anchor for TOC addressability */
        public final static int XMC_TD = 16;

        /* scalar data entry in TOC */
        public final static int XMC_SV64 = 17;

        /* 64-bit supervisor call descriptor csect */
        public final static int XMC_SV3264 = 18;

        /* supervisor call descriptor csect for both 32-bit and 64-bit */
        // fields
        public byte[] _n_name = new byte[SYMNMLEN];

        /* Symbol name, or pointer into
													string table if symbol name
													is greater than SYMNMLEN.  */
        public int n_value;

        /* long. Symbol's value: dependent on section number,
        ...
        /* Unsigned short. Symbolic type. Obsolete in XCOFF */
        public byte n_sclass;

        /* char, Storage class.  */
        public byte n_numaux;

        /* char. Nuymber of auxiliary enties.  */
        private byte[] aux;
        ...
        public boolean isFunction() {
            return ((n_sclass == C_EXT || n_sclass == C_HIDEXT || n_sclass == C_WEAKEXT) && n_scnum == opthdr.o_sntext && !getShortName().equals(SectionHeader._TEXT));
        }

        public boolean isVariable() {
            return ((n_sclass == C_EXT || n_sclass == C_HIDEXT || n_sclass == C_WEAKEXT) && (n_scnum == opthdr.o_snbss || n_scnum == opthdr.o_sndata) && x_smclas != XMC_TC0 && x_smclas != XMC_TC && x_smclas != XMC_DS && !getShortName().equals(SectionHeader._BSS) && !getShortName().equals(SectionHeader._DATA));
        }
    ...
    public static Attribute getAttributes(byte[] hints) throws IOException {
        ...
        // big endian
        emptyXCoff.filehdr = new XCoff32.FileHeader(hints, false);
