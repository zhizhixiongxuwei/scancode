...
public class AR implements AutoCloseable {
    ...
    public long strtbl_pos = -1;

    public ARHeader[] headers;
    ...
    public class ARHeader {
        ...
        public byte[] getObjectData() throws IOException {
            ...
            if (efile != null) {
                ...
                try (ERandomAccessFile tempfile = new ERandomAccessFile(filename, "r")) {
                    //$NON-NLS-1$
    ...
    public static boolean isARHeader(byte[] ident) {
        if (ident.length < 7 || ident[0] != '!' || ident[1] != '<' || ident[2] != 'a' || ident[3] != 'r' || ident[4] != 'c' || ident[5] != 'h' || ident[6] != '>')
            return false;
        return true;
    }
    ...
    private boolean stringInStrings(String str, String[] set) {
        for (int i = 0; i < set.length; i++) if (str.compareTo(set[i]) == 0)
            return true;
        return false;
    }
