...
public class SupplierBasedCdtVariableSubstitutor implements IVariableSubstitutor {
    ...
    public IVariableContextInfo fContextInfo;

    public String fInexistentMacroValue;

    public String fListDelimiter;

    public String fIncorrectlyReferencedMacroValue;
        ...
        protected String stringListToString(String[] values) throws CdtVariableException {
            ...
            if (values != null) {
                if (values.length == 0)
                    ...
                    ICdtVariableStatus eStatus = new SupplierBasedCdtVariableStatus(ICdtVariableStatus.TYPE_MACRO_NOT_STRING, null, null, fName, fContextInfo);
        ...
        if (CdtVariableResolver.isStringListVariable(macro.getValueType())) {
            ...
            if (unresolvedValues == null || unresolvedValues.length == 0)
                    ...
                    try {
                        ...
                        if (statuses != null && statuses.length == 1) {
                            ...
                            if (status.getVariableName() == null) {
                                ICdtVariableStatus eStatus = new SupplierBasedCdtVariableStatus(status.getCode(), macro.getName(), status.getExpression(), status.getReferencedMacroName(), fContextInfo);
                ...
                if (resolvedValues.length == 1)
                    ...
                    for (String[] resolvedValue : resolvedValues) list.addAll(Arrays.asList(resolvedValue));
        ...
        if (value == null) {
            if (fMacrosUnderResolution.add(name)) {
                ...
                if (last != null && last.fName.equals(name)) {
                    ...
                    ICdtVariableStatus status = new SupplierBasedCdtVariableStatus(ICdtVariableStatus.TYPE_MACRO_REFERENCE_INCORRECT, (String) null, null, name, fContextInfo);
