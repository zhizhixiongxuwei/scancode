...
public class AR implements AutoCloseable {
    ...
    public byte[] ar_magic = new byte[8];

    public LSTHeader lstHeader;
    ...
    public static boolean isARHeader(byte[] ident) {
        if (ident == null || ident.length < 8 || ident[0] != '!' || ident[1] != '<' || ident[2] != 'a' || ident[3] != 'r' || ident[4] != 'c' || ident[5] != 'h' || ident[6] != '>' || ident[7] != '\n')
            return false;
        return true;
    }
    ...
    private boolean stringInStrings(String str, String[] set) {
        for (String element : set) if (str.compareTo(element) == 0)
            return true;
        return false;
    }
    ...
    public String toString() {
        ...
        if (lstHeader != null) {
            ...
            //$NON-NLS-1$
            buffer.append("system_id    = ").append(lstHeader.system_id).append(NL);
            //$NON-NLS-1$
            buffer.append("a_magic      = ").append(lstHeader.a_magic).append(NL);
            //$NON-NLS-1$
            buffer.append("version_id   = ").append(lstHeader.version_id).append(NL);
            //$NON-NLS-1$
            buffer.append("module_count = ").append(lstHeader.module_count).append(NL);
            //$NON-NLS-1$
            buffer.append("module_limit = ").append(lstHeader.module_limit).append(NL);
            //$NON-NLS-1$
            buffer.append("dir_loc      = ").append(lstHeader.dir_loc).append(NL);
            for (int i = 0; i < memberHeaders.length; ++i) {
                //$NON-NLS-1$
                buffer.append("MEMBER HEADER VALUES").append(NL);
                //$NON-NLS-1$
                buffer.append("name      = ").append(memberHeaders[i].getObjectName()).append(NL);
                //$NON-NLS-1$
                buffer.append("somOffset = ").append(memberHeaders[i].somOffset).append(NL);
                //$NON-NLS-1$
                buffer.append("somSize   = ").append(memberHeaders[i].getSize()).append(NL);
            }
        }
