...
public class SOM {
    ...
    public static class FileHeader {
        ...
        // executable SOM library
        public static final short EXE_SOM_LIB = 0x104;

        // relocatable SOM
        public static final short REL_SOM = 0x106;

        // non-sharable, executable SOM
        public static final short PRIV_EXEC_SOM = 0x107;

        // sharable, executable SOM
        public static final short SHARE_EXEC_SOM = 0x108;

        // sharable, demand-loadable executable SOM
        public static final short SHARE_DEMAND_LOAD_EXE_SOM = 0x10b;

        // dynamic load library
        public static final short DYN_LOAD_LIB = 0x10d;

        // shared library
        public static final short SHARED_LIB = 0x10e;

        // relocatable SOM library
        public static final short RELOC_SOM_LIB = 0x619;
        ...
        // location in file of fix-up requests
        ...
        // byte offset of first byte of datafor unloadable spaces
        ...
        public String toString() {
            ...
            //$NON-NLS-1$
            buffer.append("system_id               = ").append(system_id).append(NL);
            //$NON-NLS-1$
            buffer.append("a_magic                 = ").append(a_magic).append(NL);
            //$NON-NLS-1$
            buffer.append("version_id              = ").append(version_id).append(NL);
            //$NON-NLS-1$
            buffer.append("file_time_sec           = ").append(file_time_sec).append(NL);
            //$NON-NLS-1$
            buffer.append("file_time_nano          = ").append(file_time_nano).append(NL);
            //$NON-NLS-1$
            buffer.append("entry_space             = ").append(entry_space).append(NL);
            //$NON-NLS-1$
            buffer.append("entry_subspace          = ").append(entry_subspace).append(NL);
            //$NON-NLS-1$
            buffer.append("aux_header_location     = ").append(aux_header_location).append(NL);
            //$NON-NLS-1$
            buffer.append("aux_header_size         = ").append(aux_header_size).append(NL);
            //$NON-NLS-1$
            buffer.append("som_length              = ").append(som_length).append(NL);
            //$NON-NLS-1$
            buffer.append("presumed_dp             = ").append(presumed_dp).append(NL);
            //$NON-NLS-1$
            buffer.append("space_location          = ").append(space_location).append(NL);
            //$NON-NLS-1$
            buffer.append("space_total             = ").append(space_total).append(NL);
            //$NON-NLS-1$
            buffer.append("subspace_location       = ").append(subspace_location).append(NL);
            //$NON-NLS-1$
            buffer.append("subspace_total          = ").append(subspace_total).append(NL);
            //$NON-NLS-1$
            buffer.append("loader_fixup_location   = ").append(loader_fixup_location).append(NL);
            //$NON-NLS-1$
            buffer.append("loader_fixup_total      = ").append(loader_fixup_total).append(NL);
            //$NON-NLS-1$
            buffer.append("space_strings_location  = ").append(space_strings_location).append(NL);
            //$NON-NLS-1$
            buffer.append("space_strings_size      = ").append(space_strings_size).append(NL);
            //$NON-NLS-1$
            buffer.append("init_array_location     = ").append(init_array_location).append(NL);
            //$NON-NLS-1$
            buffer.append("init_array_total        = ").append(init_array_total).append(NL);
            //$NON-NLS-1$
            buffer.append("compiler_location       = ").append(compiler_location).append(NL);
            //$NON-NLS-1$
            buffer.append("compiler_total          = ").append(compiler_total).append(NL);
            //$NON-NLS-1$
            buffer.append("symbol_location         = ").append(symbol_location).append(NL);
            //$NON-NLS-1$
            buffer.append("symbol_total            = ").append(symbol_total).append(NL);
            //$NON-NLS-1$
            buffer.append("fixup_request_location  = ").append(fixup_request_location).append(NL);
            //$NON-NLS-1$
            buffer.append("fixup_request_total     = ").append(fixup_request_total).append(NL);
            //$NON-NLS-1$
            buffer.append("symbol_strings_location = ").append(symbol_strings_location).append(NL);
            //$NON-NLS-1$
            buffer.append("symbol_strings_size     = ").append(symbol_strings_size).append(NL);
            //$NON-NLS-1$
            buffer.append("unloadable_sp_location  = ").append(unloadable_sp_location).append(NL);
            //$NON-NLS-1$
            buffer.append("unloadable_sp_size      = ").append(unloadable_sp_size).append(NL);
            //$NON-NLS-1$
    ...
    public class Symbol {
        ...
        // 5 words = 20 bytes
        public static final int SYMSZ = 5 * 4;
        ...
        // Any code entry point
        public static final int ENTRY = 6;

        // Uninitialized common data blocks
        public static final int STORAGE = 7;

        // Import external call stub or a parameter relocation stub
        ...
        // Export stub for a procedure
        public static final int PLABEL = 13;

        // Pointer to translated code segment exists but disabled
        public static final int OCT_DIS = 14;

        // Address of an external millicode routine
        ...
        // symbol scope
        // Import request that has not been satisfied
        public static final int UNSAT = 0;

        // Import request linked to a symbol in another SOM
        public static final int EXTERNAL = 1;

        // The symbol is not exported for use outside the SOM
        public static final int LOCAL = 2;

        // The symbol is exported for use outside the SOM
        ...
        public String toString() {
            ...
            //$NON-NLS-1$
            buffer.append("symbol_value = ").append(symbol_value).append(NL);
            //$NON-NLS-1$
            buffer.append("symbol_type  = ").append(symbol_type).append(NL);
            //$NON-NLS-1$
            buffer.append("symbol_scope = ").append(symbol_scope).append(NL);
            return buffer.toString();
        }
    }
    ...
    public static boolean isSOMHeader(byte[] hints) {
        if (hints != null && hints[0] == 0x02 && (hints[1] == (byte) 0xb || hints[1] == (byte) 0x10 || hints[1] == (byte) 0x14)) {
            return true;
        }
        return false;
    }
    ...
    public static Attribute getAttributes(byte[] hints) throws IOException {
        ...
        // big endian
        emptyXCoff.filehdr = new SOM.FileHeader(hints, false);
