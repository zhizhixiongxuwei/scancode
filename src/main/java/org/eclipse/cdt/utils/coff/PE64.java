...
public class PE64 implements AutoCloseable {
    ...
    // the offset of this PE within the file (eg archive file)
    private final long peOffset;
    ...
    public static class DOSHeader {
        ...
        /* 4 byte File address of new exe header, offset 60(0x3c), 0x80. */
        byte[] dos_message = new byte[64];

        /* Other stuff, always follow DOS header.  */
    ...
    public PE64(String filename, long pos, boolean filter) throws IOException {
        ...
        try {
            ...
            if (!isValidMachine(fileHeader.f_magic)) {
                //$NON-NLS-1$
                throw new IOException(CCorePlugin.getResourceString("Util.exception.unknownFormat"));
            }
    ...
    public static Attribute getAttribute(byte[] data) throws IOException {
        ...
        try {
            ...
            if (idx + 4 < data.length) {
                if (!((data[idx + 0] == 'P') && (data[idx + 1] == 'E') && (data[idx + 2] == '\0') && (data[idx + 3] == '\0'))) {
                    //$NON-NLS-1$
                    throw new IOException(CCorePlugin.getResourceString("Util.exception.notPE"));
                }
                idx += 4;
    ...
    public Symbol[] getSymbols() throws IOException {
        if (symbolTable == null) {
            ...
            for (int i = 0; i < symbolTable.length; i++, offset += Symbol.SYMSZ) {
                Symbol newSym = new Symbol(accessFile, offset + peOffset, (fileHeader.f_flags & FileHeader.F_AR32WR) == 0);
                // Now convert section offset of the symbol to image offset.
                if (// valid section #
                newSym.n_scnum >= 1 && newSym.n_scnum <= secHeaders.length)
            ...
            for (int i = 0; i < sections.length; i++) {
                ...
                if (name.equals(".rdata")) {
                    ...
                    for (int j = 0; j < debugFormats; j++) {
                        PE64.IMAGE_DEBUG_DIRECTORY dir = new PE64.IMAGE_DEBUG_DIRECTORY(accessFile, fileOffset + peOffset);
