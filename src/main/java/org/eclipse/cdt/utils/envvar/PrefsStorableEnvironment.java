...
public class PrefsStorableEnvironment extends StorableEnvironment {
     ...
     */
    public ISerializeInfo fSerialEnv;

    /**
     * Set of 'deleted' variables (to be removed from the backing store)
     */
    public Set<String> fDeletedVariables;
     ...
     */
    public Map<String, IEnvironmentVariable> fCachedSerialEnv = new HashMap<>();

    // State to track whether API users have changed these boolean values
    public boolean fAppendChanged = false;

    public boolean fAppendContributedChanged = false;
    ...
    private void checkBackingSerializeInfo() {
        ...
        try {
            if (//$NON-NLS-1$
            topNode.get(fSerialEnv.getPrefName(), "").length() != 0)
    ...
    private void migrateOldStylePrefs() {
        ...
        if (element != null) {
            ...
            //$NON-NLS-1$
            oldNode.put(fSerialEnv.getPrefName(), "");
            ...
            for (Map.Entry<String, IEnvironmentVariable> e : oldEnv.getMap().entrySet()) ((StorableEnvVar) e.getValue()).serialize(newNode.node(e.getKey()));
    ...
    void serialize() {
        ...
        if (fVariables != null) {
            for (Map.Entry<String, IEnvironmentVariable> e : fVariables.entrySet()) ((StorableEnvVar) e.getValue()).serialize(element.node(e.getKey()));
    ...
    Map<String, IEnvironmentVariable> getAllVariablesMap() {
        ...
        if (fDeletedVariables != null)
            for (String name : fDeletedVariables) vars.remove(name);
    ...
    public boolean isDirty() {
        return fAppendChanged || fAppendContributedChanged || (fVariables != null && !fVariables.isEmpty()) || (fDeletedVariables != null && !fDeletedVariables.isEmpty());
    }
