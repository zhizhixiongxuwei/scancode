...
public class StorableEnvironment {
    ...
    //$NON-NLS-1$
    static final public String ATTRIBUTE_APPEND = "append";

    //$NON-NLS-1$
    static final public String ATTRIBUTE_APPEND_CONTRIBUTED = "appendContributed";

    static final public boolean DEFAULT_APPEND = true;

    /**
     * The map of in-flight environment variables
     */
    public Map<String, IEnvironmentVariable> fVariables;
    ...
    public IEnvironmentVariable createVariable(String name, String value, int op, String delimiter) {
        ...
        if (//$NON-NLS-1$
        name == null || "".equals(name = name.trim()))
            return null;
    ...
    public IEnvironmentVariable checkVariable(String name, String value, int op, String delimiter) {
        ...
        if (var != null && checkStrings(var.getValue(), value) && var.getOperation() == op && checkStrings(var.getDelimiter(), delimiter))
            return var;
        return null;
    }
    ...
    public void createVriables(IEnvironmentVariable[] vars) {
        if (fIsReadOnly)
            throw ExceptionFactory.createIsReadOnlyException();
        for (int i = 0; i < vars.length; i++) createVariable(vars[i].getName(), vars[i].getValue(), vars[i].getOperation(), vars[i].getDelimiter());
    }
