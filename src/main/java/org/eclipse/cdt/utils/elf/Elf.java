...
public class Elf implements AutoCloseable {
    ...
    public class ELFhdr {
        ...
        /* entry point (Elf32_Addr) */
        public long e_phoff;

        /* Program hdr offset (Elf32_Off) */
        public long e_shoff;

        /* Section hdr offset (Elf32_Off) */
        public long e_flags;
        ...
        /* String table index (Elf32_Half) */
        protected ELFhdr() throws IOException {
            efile.seek(0);
            efile.readFully(e_ident);
            if (e_ident[ELFhdr.EI_MAG0] != 0x7f || e_ident[ELFhdr.EI_MAG1] != 'E' || e_ident[ELFhdr.EI_MAG2] != 'L' || e_ident[ELFhdr.EI_MAG3] != 'F')
                //$NON-NLS-1$
        ...
        public String toString() {
            try {
                if (section_strtab == null) {
                    // unsigned short
                    final int shstrndx = ehdr.e_shstrndx & 0xffff;
                ...
                while (section_strtab[(int) sh_name + str_size] != 0) str_size++;
    ...
    public PHdr[] getPHdrs() throws IOException {
        ...
        // interpret as unsigned short
        final int length = ehdr.e_phnum & 0xffff;
    ...
    public static boolean isElfHeader(byte[] e_ident) {
        return e_ident != null && e_ident.length >= 4 && e_ident[ELFhdr.EI_MAG0] == 0x7f && e_ident[ELFhdr.EI_MAG1] == 'E' && e_ident[ELFhdr.EI_MAG2] == 'L' && e_ident[ELFhdr.EI_MAG3] == 'F';
    }
    ...
    public Section[] getSections() throws IOException {
        if (sections == null) {
            ...
            // unsigned short
            final int length = ehdr.e_shnum & 0xffff;
            sections = new Section[length];
            for (int i = 0; i < length; i++) {
                // unsigned short
                efile.seek(ehdr.e_shoff + i * (ehdr.e_shentsize & 0xffff));
                ...
                switch(ehdr.e_ident[ELFhdr.EI_CLASS]) {
                    ...
                    default:
                        //$NON-NLS-1$
                        throw new ElfClassNoneException("Unknown ELF class " + ehdr.e_ident[ELFhdr.EI_CLASS]);
                }
    ...
    private class ElfSectionIterator implements Iterator<Symbol>, Closeable {
        ...
        public ElfSectionIterator(ERandomAccessFile eFile, byte b, Section sectionToRead, byte architecture) throws IOException {
            int numSyms = 1;
            ...
            nbSymbols = numSyms;
            //$NON-NLS-1$
            innerEfile = new ERandomAccessFile(eFile.getPath(), "r");
        ...
        public Symbol next() {
            ...
            try {
                ...
                switch(arch) {
                    ...
                    case ELFhdr.ELFCLASS64:
                        {
                            ...
                            if (symbol.st_size < 0) {
                                ...
                                "Maximal file offset is " + Long.toHexString(Long.MAX_VALUE) + " given offset is " + //$NON-NLS-1$
                                Long.toHexString(symbol.st_size));
                            }
                            break;
                        }
                    case ELFhdr.ELFCLASSNONE:
                    default:
                        //$NON-NLS-1$
                        throw new NoSuchElementException("Unknown ELF class " + arch);
                }
                return symbol;
            } catch (IOException e) {
