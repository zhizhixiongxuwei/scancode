...
public class CygPath {
    ...
    public boolean useOldCygPath = false;

    final public Process cygpath;

    final public BufferedReader stdout;

    final public BufferedWriter stdin;

    public boolean fSpaceIsSeparator = false;

    public CygPath(String command) throws IOException {
        if (!Platform.getOS().equals(Platform.OS_WIN32))
            // Don't run this on non-windows platforms
            throw //$NON-NLS-1$
            new IOException("Not Windows");
