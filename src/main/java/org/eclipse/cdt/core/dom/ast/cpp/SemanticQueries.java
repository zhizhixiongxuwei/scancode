...
public class SemanticQueries {
    ...
    //$NON-NLS-1$
    static final public String OPERATOR_EQ = "operator =";
    ...
    public static boolean isUnknownBuiltin(IProblemBinding binding, IASTNode node) {
        ...
        boolean isBuiltin = binding.getID() == IProblemBinding.SEMANTIC_NAME_NOT_FOUND && //$NON-NLS-1$
        CharArrayUtils.startsWith(name, "__builtin_");
        if (isBuiltin) {
            if (node != null) {
