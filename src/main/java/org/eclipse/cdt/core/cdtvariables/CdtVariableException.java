...
public class CdtVariableException extends CoreException {
     ...
     */
    static final public long serialVersionUID = 3976741380246681395L;
    ...
    //Object contextData
    {
        super(new CdtVariableStatus(code, message, exception, macroName, expression, referencedName));
    }
    ...
    //Object contextData
    {
        super(new CdtVariableStatus(code, macroName, expression, referencedName));
    }
    ...
    public ICdtVariableStatus[] getVariableStatuses() {
        ...
        if (status instanceof ICdtVariableStatus)
            ...
            if (num != children.length) {
                ...
                for (int i = 0; i < num; i++) tmp[i] = result[i];
                result = tmp;
            }
            return result;
        }
