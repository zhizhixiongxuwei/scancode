...
public class CommandLauncher implements ICommandLauncher {
    ...
    //$NON-NLS-1$
    static final public String PATH_ENV = "PATH";
    ...
    public Process execute(IPath commandPath, String[] args, String[] env, IPath workingDirectory, IProgressMonitor monitor) throws CoreException {
        ...
        if (fProcess != null && fProcess.isAlive()) {
            ...
            throw new ConcurrentModificationException(//$NON-NLS-1$
            NLS.bind("Process for command \"{0}\" is still executing.", command));
        }
        ...
        try {
            ...
            // to print original command on the console
            ...
            if (!isFound) {
                ...
                errorMessage = errorMessage + NEWLINE + NLS.bind(Messages.CommandLauncher_ProgramNotFoundInPath, command) + NEWLINE + //$NON-NLS-1$
                "PATH=[" + envPathValue + "]" + //$NON-NLS-1$
                NEWLINE;
            }
