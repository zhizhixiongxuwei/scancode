...
public class HashTable implements Cloneable {
    ...
    static final public int[] PRIMES = { 17, 29, 53, 97, 193, 389, 769, 1543, 3079, 6151, 12289, 24593, 49157, 98317, 196613, 393241, 786433, 1572869, 3145739, 6291469, 12582917, 25165843, 50331653, 100663319, 201326611, 402653189, 805306457, 1610612741 };

    static final public int MIN_HASH_SIZE = 9;

    /**
     * @deprecated Don't depend on this implementation detail. @noreference This field is not intended to be referenced by clients.
     */
    @Deprecated
    static final public int minHashSize = MIN_HASH_SIZE - 1;

    public int currEntry = -1;
    ...
    private static int getSuitableHashTableSize(int size) {
        size += (size + 2) / 3;
        if (size < 0)
            // Integer overflow. Return the max possible size.
        ...
        if (low == PRIMES.length)
            // Largest prime is not sufficient. Return the max possible size.
            return Integer.MAX_VALUE;
        return PRIMES[low];
    }
        ...
        if (j == 0) {
            ...
            for (int k; (k = nextTable[j]) != 0 && k != j + 1; j = k - 1) {
                if (--maxIterationsLeft < 0) {
                    throw new IllegalStateException(//$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                    "i = " + i + " hash = " + hash + " j = " + j + "\n  nextTable = " + //$NON-NLS-1$
                    Arrays.toString(nextTable) + "\n  hashTable = " + //$NON-NLS-1$
    ...
    public void dumpNexts() {
        ...
        for (int i = 0; i < nextTable.length; ++i) {
            if (nextTable[i] == 0)
                continue;
            System.out.print(i);
            for (int j = nextTable[i] - 1; j >= 0; j = nextTable[j] - 1) //$NON-NLS-1$
            System.out.print(" -> " + j);
            //$NON-NLS-1$
            System.out.println("");
        }
