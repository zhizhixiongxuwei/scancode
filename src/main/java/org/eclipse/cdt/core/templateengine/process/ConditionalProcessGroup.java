...
public class ConditionalProcessGroup {
    ...
    private static class Operator {
        ...
        //$NON-NLS-1$
        final static Operator EQUALS = new Operator("=");

        //$NON-NLS-1$
        final static Operator NOT_EQUALS = new Operator("!=");
    ...
    private void createProcessObjects(TemplateCore templateCore, List<Element> processElements) {
        ...
        for (int j = 0, l = processElements.size(); j < l; j++) {
            ...
            if (processElem.getNodeName().equals(TemplateDescriptor.PROCESS)) {
                String processId = //$NON-NLS-1$//$NON-NLS-2$
                id + "--> Process " + (j + 1) + " (" + processElem.getAttribute(Process.ELEM_TYPE) + //$NON-NLS-1$
                ")";
    ...
    public List<IStatus> process(IProgressMonitor monitor) throws ProcessFailureException {
        ...
        if (!isConditionValueTrue()) {
            ...
            statuses.add(new Status(IStatus.ERROR, getClass(), IStatus.INFO, Messages.getString("ConditionalProcessGroup.notExecuting") + id, //$NON-NLS-1$
            null));
            return statuses;
        }
    ...
    private String getUnexpandableMacroMessage() {
        if (macros != null) {
            ...
            for (String value : macros) {
                if (valueStore.get(value) == null) {
                    //$NON-NLS-1$
                    return Messages.getString("ConditionalProcessGroup.unexpandableMacro") + value;
                }
            }
        }
        return null;
