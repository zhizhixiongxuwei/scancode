...
public class Process {
    ...
    private void buildArgs(TemplateCore templateCore, Element element) {
        ...
        for (int i = 0; i < params.length; i++) {
            ...
            if (param.isExternal() && (child == null || !param.getName().equals(child.getAttribute(ProcessArgument.ELEM_NAME)))) {
        ...
        if (args != null) {
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                if ((macro = arg.getFirstNonExpandableMacro()) != null) {
                    return //$NON-NLS-1$
                    Messages.getString("Process.argument") + arg.getName() + Messages.getString("Process.expandableMacro") + //$NON-NLS-1$
                    macro;
                }
            }
        }
    ...
    private String getProcessMessage(int code, String msg) {
        switch(code) {
            case IStatus.ERROR:
                //$NON-NLS-1$
                return id + Messages.getString("Process.error") + msg;
            case IStatus.OK:
                //$NON-NLS-1$
                return id + Messages.getString("Process.success") + msg;
            default:
                //$NON-NLS-1$
                return id + Messages.getString("Process.info") + msg;
        }
    }
    ...
    public IStatus process(IProgressMonitor monitor) throws ProcessFailureException {
        if (processRunner == null) {
            //$NON-NLS-1$
            throw new ProcessFailureException(Messages.getString("Process.unknownProcess") + processType);
        }
        ...
        return new Status(IStatus.INFO, getClass(), IStatus.OK, //$NON-NLS-1$
        getProcessMessage(IStatus.OK, Messages.getString("Process.executedSuccessfully") + Arrays.asList(args)), null);
    }
