...
public abstract class ProcessRunner {
    ...
    public ProcessParameter[] params = new ProcessParameter[0];
    ...
    public String getArgumentsMismatchMessage(ProcessArgument[] args) {
        ...
        for (int i = 0; i < params.length; i++) {
            ...
            if (!arg.isOfParameterType(param)) {
                //$NON-NLS-1$
                return Messages.getString("ProcessRunner.argumentsMismatch") + arg.getName();
            }
        }
        return null;
    }
    ...
    protected final String getProcessMessage(String processId, int code, String msg) {
        switch(code) {
            case IStatus.ERROR:
                //$NON-NLS-1$
                return processId + Messages.getString("ProcessRunner.error") + msg;
            case IStatus.OK:
                //$NON-NLS-1$
                return processId + Messages.getString("ProcessRunner.success") + msg;
            default:
                //$NON-NLS-1$
                return processId + Messages.getString("ProcessRunner.info") + msg;
        }
    }
