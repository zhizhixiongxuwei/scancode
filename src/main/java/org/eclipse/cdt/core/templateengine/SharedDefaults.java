...
public class SharedDefaults extends HashMap<String, String> {
    ...
    static final public long serialVersionUID = 0000000000L;

    public Document document;

    public File parsedXML;

    public File backUpSharedXML;

    /**
     * HashMap's for persistence
     */
    public HashMap<String, String> sharedDefaultsMap;
    ...
    private void initSharedDefaults() {
        ...
        try {
            ...
            document = XmlProcessorFactoryCdt.createDocumentBuilderWithErrorOnDOCTYPE().parse(parsedXML.toURI().toURL().openStream());
        } catch (Exception exp) {
    ...
    private File createDefaultXMLFormat(File xmlFile) {
        ...
        //$NON-NLS-1$
        element.setAttribute(TemplateEngineHelper.ID, "");
        //$NON-NLS-1$
        element.setAttribute(TemplateEngineHelper.VALUE, "");
