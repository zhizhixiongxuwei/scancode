...
public class CDataSerializer {
    ...
    //$NON-NLS-1$
    static final public String SOURCE_ENTRIES = "sourceEntries";
    ...
    public CConfigurationData loadConfigurationData(CDataFactory factory, ICStorageElement el) throws CoreException {
        ...
        if (id == null)
            ...
            new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, UtilMessages.getString("CDataSerializer.20")));
        String name = el.getAttribute(NAME);
        CConfigurationData data = factory.createConfigurationdata(id, name, null, false);
        if (data == null)
            throw new CoreException(//$NON-NLS-1$
            new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, UtilMessages.getString("CDataSerializer.21")));
    ...
    public CFolderData loadFolderData(CConfigurationData data, CDataFactory factory, ICStorageElement el) throws CoreException {
        ...
        if (id == null)
            throw new CoreException(//$NON-NLS-1$
            new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, UtilMessages.getString("CDataSerializer.22")));
        ...
        if (path == null)
            throw new CoreException(//$NON-NLS-1$
            new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, UtilMessages.getString("CDataSerializer.23")));
        CFolderData foData = factory.createFolderData(data, null, id, false, path);
        if (foData == null) {
            throw new CoreException(//$NON-NLS-1$
            new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, UtilMessages.getString("CDataSerializer.24")));
        }
    ...
    public CFileData loadFileData(CConfigurationData data, CDataFactory factory, ICStorageElement el) throws CoreException {
        ...
        if (id == null)
            throw new CoreException(//$NON-NLS-1$
            new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, UtilMessages.getString("CDataSerializer.25")));
        String tmp = el.getAttribute(PATH);
        IPath path = null;
        if (tmp != null) {
            path = new Path(tmp);
        ...
        if (path == null)
            throw new CoreException(//$NON-NLS-1$
            new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, UtilMessages.getString("CDataSerializer.26")));
        CFileData fiData = factory.createFileData(data, null, null, id, false, path);
        if (fiData == null)
            throw new CoreException(//$NON-NLS-1$
            new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, UtilMessages.getString("CDataSerializer.27")));
        //		tmp = el.getAttribute(EXCLUDED);
        //		if(tmp != null){
        //			boolean b = Boolean.valueOf(tmp).booleanValue();
    ...
    public CBuildData loadBuildData(CConfigurationData data, CDataFactory factory, ICStorageElement el) throws CoreException {
        ...
        if (id == null)
            throw new CoreException(//$NON-NLS-1$
            new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, UtilMessages.getString("CDataSerializer.28")));
        String name = el.getAttribute(NAME);
        CBuildData bData = factory.createBuildData(data, null, id, name, false);
        if (bData == null)
            throw new CoreException(//$NON-NLS-1$
            new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, UtilMessages.getString("CDataSerializer.29")));
    ...
    public CTargetPlatformData loadTargetPlatformData(CConfigurationData data, CDataFactory factory, ICStorageElement el) throws CoreException {
        ...
        if (tpData == null)
            throw new CoreException(//$NON-NLS-1$
            new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, UtilMessages.getString("CDataSerializer.31")));
    ...
    public CLanguageData loadLanguageData(CConfigurationData data, CResourceData rcData, CDataFactory factory, ICStorageElement el) throws CoreException {
        ...
        if (id == null)
            throw new CoreException(//$NON-NLS-1$
            new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, UtilMessages.getString("CDataSerializer.32")));
        String name = el.getAttribute(NAME);
        ...
        if (lData == null)
            throw new CoreException(//$NON-NLS-1$
            new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, UtilMessages.getString("CDataSerializer.33")));
        ICStorageElement[] children = el.getChildren();
        String childName;
        ICStorageElement child;
        for (int i = 0; i < children.length; i++) {
