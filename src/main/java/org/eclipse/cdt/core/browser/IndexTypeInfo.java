...
public class IndexTypeInfo implements ITypeInfo, IFunctionInfo {
    ...
    final public IIndexFileLocation fileLocal;

    final public int elementType;

    final public IIndex index;
    ...
    private IndexTypeReference createReference(IIndexBinding binding, IIndexName indexName) throws CoreException {
        ...
        if (fullPath != null) {
            ...
            if (file != null) {
                return new IndexTypeReference(binding, file, file.getProject(), indexName.getNodeOffset(), indexName.getNodeLength());
            }
        } else {
            IPath path = URIUtil.toPath(ifl.getURI());
            if (path != null) {
                return new IndexTypeReference(binding, path, null, indexName.getNodeOffset(), indexName.getNodeLength());
            }
        }
        return null;
    }
