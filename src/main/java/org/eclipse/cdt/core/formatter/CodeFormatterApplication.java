...
public class CodeFormatterApplication implements IApplication {
    ...
    private void formatFile(File file, CodeFormatter codeFormatter) {
        ...
        try {
            ...
            Messages.//$NON-NLS-1$
            bind(//$NON-NLS-1$
            Messages.CaughtException, //$NON-NLS-1$
            "BadLocationException", e.getLocalizedMessage());
            System.err.println(Messages.bind(Messages.ExceptionSkip, errorMessage));
        }
    }
    ...
    private Properties readConfig(String filename) {
        ...
        try (BufferedInputStream stream = new BufferedInputStream(new FileInputStream(configFile))) {
            ...
            if (!configFile.exists() && !configFile.isAbsolute()) {
                errorMessage = Messages.bind(Messages.ConfigFileNotFoundErrorTryFullPath, new Object[] { //$NON-NLS-1$
                canonicalPath, //$NON-NLS-1$
    ...
    private boolean hasSuitableFileExtension(File file) {
        ...
        if (ct != null) {
            String id = ct.getId();
            if (CCorePlugin.CONTENT_TYPE_CSOURCE.equals(id) || CCorePlugin.CONTENT_TYPE_CHEADER.equals(id) || CCorePlugin.CONTENT_TYPE_CXXSOURCE.equals(id) || CCorePlugin.CONTENT_TYPE_CXXHEADER.equals(id)) {
                return true;
            }
        }
        return false;
