...
public class Scribe {
            ...
            if (endOffsetOfPreviousEdit == offset) {
                if (length != 0) {
                    if (replacementLength != 0) {
                        editsIndex--;
                        appendOptimizedReplaceEdit(previousOffset, previousLength + length, previousReplacement + replacement);
                        ...
                        loop: for (int i = previousOffset; i < previousOffset + previousReplacementLength; i++) {
                            if (scanner.source[i] != previousReplacement.charAt(i - previousOffset)) {
                                editsIndex--;
                                appendOptimizedReplaceEdit(previousOffset, previousReplacementLength, previousReplacement);
                                canBeRemoved = false;
                                break loop;
                            }
                        }
    ...
    public Alignment createAlignment(String name, int mode, int tieBreakRule, int count, int sourceRestart) {
        return createAlignment(name, mode, tieBreakRule, count, sourceRestart, preferences.continuation_indentation, false);
    }
            ...
            while (true) {
                ...
                switch(currentToken.type) {
                    ...
                    case Token.t_else:
                        ...
                        break;
                    default:
                        if (currentToken.isVisibilityModifier() && !preferences.indent_access_specifier_compare_to_type_header) {
    ...
    private void printBlockComment(boolean forceNewLine) {
        ...
        if ((currentTokenStartPosition > 0 && !preferences.format_block_comment) || (currentTokenStartPosition == 0 && !preferences.format_header_comment))
            inactiveState = true;
        try {
                ...
                if (inactivePos != null) {
                    ...
                    if (startOffset < endOffset) {
                        ...
                        try {
                             ...
                             */
                            if (inactivePos.isPreprocessorRegion() && editsIndex > 0 && lineSeparator.equals(edits[editsIndex - 1].replacement)) {
                                editsIndex--;
                            }
                ...
                case Token.tWHITESPACE:
                    ...
                    if (realTrailing && scanner.peekNextChar() == '/' && lines == 0) {
                        ...
                        if (canChangeTrailing) {
                            ...
                            // Get current token again to restore the scanner state.
                    ...
                    if (realTrailing) {
                        ...
                        if (lines == 0) {
                            ...
                            addReplaceEdit(scanner.getCurrentTokenStartPosition(), scanner.getCurrentTokenEndPosition(), preservedEmptyLines);
    ...
    private void printLineComment() {
        ...
        int nextCharacterStart = currentTokenStartPosition;
        boolean oldState = inactiveState;
        if ((currentTokenStartPosition > 0 && !preferences.format_line_comment) || (currentTokenStartPosition == 0 && !preferences.format_header_comment))
            inactiveState = true;
        try {
                ...
                if (onFirstColumn && preferences.never_indent_line_comments_on_first_column) {
                    ...
                    if (lastLineComment.contiguous) {
                        ...
                        if (tabLength == 0) {
                            ...
                            similarCommentsIndentation = relativeIndentation == 0 || currentCommentIndentation != 0 && lastLineComment.currentIndentation != 0;
                        }
            ...
            if (!lastLineComment.contiguous && commentIndentationLevel != indentationLevel && !(preferences.never_indent_line_comments_on_first_column && onFirstColumn)) {
                ...
                if (preferences.comment_preserve_white_space_between_code_and_line_comment && distance >= preferences.comment_min_distance_between_code_and_line_comment) {
            ...
            while (true) {
                ...
                if (!checkLineWrapping || indent <= pageWidth || whitespace == null || commentIndent - commentIndentationLevel <= preferences.comment_min_distance_between_code_and_line_comment) {
                    break;
                }
                ...
                if (computeIndentation(lastLineComment.leadingSpaces, commentIndentationLevel) - commentIndentationLevel < preferences.comment_min_distance_between_code_and_line_comment) {
            ...
            if (currentAlignment != null) {
                ...
                if (currentAlignment.name.equals(Alignment.BINARY_EXPRESSION) && currentAlignment.enclosing != null && currentAlignment.enclosing.equals(Alignment.BINARY_EXPRESSION) && indentationLevel < currentAlignment.breakIndentationLevel) {
                    indentationLevel = currentAlignment.breakIndentationLevel;
                }
            }
            scanner.resetTo(currentTokenEndPosition, scannerEndPosition);
    ...
    public void printNextToken(int expectedTokenType, boolean considerSpaceIfAny) {
        ...
        try {
            ...
            if (currentToken == null || expectedTokenType != currentToken.type) {
                ...
                pendingSpace = false;
                needSpace = true;
                throw new AbortFormatting(//$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
                "[" + (line + 1) + "/" + column + "] Unexpected token type, expecting:" + expectedTokenType + ", actual:" + //$NON-NLS-1$
                currentToken);
            }
    ...
    public void printNextToken(int[] expectedTokenTypes, boolean considerSpaceIfAny) {
        ...
        if (Arrays.binarySearch(expectedTokenTypes, currentToken.type) < 0) {
            ...
            throw new AbortFormatting(//$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
            "[" + (line + 1) + "/" + column + "] unexpected token type, expecting:[" + expectations.toString() + "], actual:" + //$NON-NLS-1$
            currentToken);
        }
        print(currentToken.getLength(), considerSpaceIfAny);
    }
    ...
    void redoAlignment(AlignmentException e) {
        if (e.relativeDepth > 0) {
            // if exception targets a distinct context
            // record fact that current context got traversed
            e.relativeDepth--;
            // pop currentLocation
            currentAlignment = currentAlignment.enclosing;
            // rethrow
            throw e;
        }
    ...
    public String toString() {
        StringBuilder buffer = new StringBuilder();
        //$NON-NLS-1$//$NON-NLS-2$
        buffer.append("(page width = ").append(pageWidth).append(") - (tabChar = ");
        switch(tabChar) {
            case DefaultCodeFormatterOptions.TAB:
                //$NON-NLS-1$
                buffer.append("TAB");
        ...
        //$NON-NLS-1$/
        buffer.append(") - (tabSize = ").append(tabLength).append(')').append(lineSeparator).append("(line = ").append(line).append(") - (column = ").append(//$NON-NLS-1$//$NON-NLS-2$
        column).append(") - (identationLevel = ").append(indentationLevel).append(//$NON-NLS-1$
        ')').append(lineSeparator).append("(needSpace = ").append(needSpace).append(//$NON-NLS-1$//$NON-NLS-2$
        ") - (lastNumberOfNewLines = ").append(lastNumberOfNewLines).append(") - (checkLineWrapping = ").append(checkLineWrapping).append(//$NON-NLS-1$
        ')').append(lineSeparator).append(//$NON-NLS-1$
            ...
            switch(currentToken.type) {
                ...
                default:
                    if (currentToken.getType() == Token.tIDENTIFIER) {
                        if (currentToken.getText().startsWith("__")) {
                            //$NON-NLS-1$
