...
public class CodeFormatterVisitor extends ASTVisitor implements ICPPASTVisitor, ICASTVisitor {
    ...
    static public boolean DEBUG = Boolean.parseBoolean(//$NON-NLS-1$
    Platform.getDebugOption("org.eclipse.cdt.core/debug/formatter"));
    ...
    private class TrailingSemicolonFormatter extends TrailingTokenFormatter {
        ...
        TrailingSemicolonFormatter(IASTNode node) {
            super(Token.tSEMI, getLastNodeCharacterPosition(node), fHasClauseInitStatement ? preferences.insert_space_before_semicolon_in_for : preferences.insert_space_before_semicolon, false);
        }
    }
        ...
        public void run() {
            ...
            if (token == Token.t_noexcept || token == Token.t_throw || token == Token.tIDENTIFIER) {
                if (node instanceof ICPPASTFunctionDeclarator) {
                    final IASTTypeId[] exceptionSpecification = ((ICPPASTFunctionDeclarator) node).getExceptionSpecification();
                    if (exceptionSpecification != null && token == Token.t_throw)
                        formatExceptionSpecification(exceptionSpecification);
                    final ICPPASTExpression noexceptExpression = ((ICPPASTFunctionDeclarator) node).getNoexceptExpression();
                ...
                if (peekNextToken() == Token.tIDENTIFIER) {
                    Alignment alignment = scribe.createAlignment(Alignment.TRAILING_TEXT, Alignment.M_COMPACT_SPLIT, 1, scribe.scanner.getCurrentPosition());
    ...
    final public Scanner localScanner;

    public List<InactivePosition> fInactivePreprocessorPositions;

    final public DefaultCodeFormatterOptions preferences;

    final public Scribe scribe;
    ...
    public TextEdit format(String string, IASTTranslationUnit unit) {
        ...
        fStatus = new //$NON-NLS-1$//$NON-NLS-2$
        MultiStatus(//$NON-NLS-1$//$NON-NLS-2$
        CCorePlugin.PLUGIN_ID, //$NON-NLS-1$//$NON-NLS-2$
        0, //$NON-NLS-1$//$NON-NLS-2$
        "Formatting problem(s) in '" + unit.getFilePath() + "'", null);
        try {
            unit.accept(this);
            ...
            try {
                ...
                for (; m < macroExpansions.length; m++) {
                    ...
                    if (isFunctionStyleMacroExpansion(macroExpansion) && macroOffset >= pos) {
                        ...
                        if (macroEndOffset <= declarationOffset || macroEndOffset >= declarationEndOffset || macroEndOffset == declarationEndOffset - 1 && isSemicolonAtPosition(macroEndOffset)) {
                            ...
                            scribe.setTailFormatter(new TrailingTokenFormatter(Token.tSEMI, macroEndOffset, preferences.insert_space_before_semicolon, false));
    ...
    public int visit(ICPPASTLambdaExpression node) {
        ...
        if (DefaultCodeFormatterConstants.END_OF_LINE.equals(preferences.brace_position_for_method_declaration) && bodyStmt instanceof IASTCompoundStatement && !startsWithMacroExpansion(bodyStmt)) {
            tailFormatter = new TrailingTokenFormatter(Token.tLBRACE, nodeOffset(bodyStmt), preferences.insert_space_before_opening_brace_in_method_declaration, false);
        ...
        Alignment alignment = scribe.createAlignment(Alignment.LAMBDA_EXPRESSION, preferences.alignment_for_lambda_expression, Alignment.R_INNERMOST, 1, getCurrentPosition());
        scribe.enterAlignment(alignment);
        boolean ok = false;
        do {
            try {
                ...
                if (bodyStmt instanceof IASTCompoundStatement) {
                    if (enterNode(bodyStmt)) {
                        ...
                        formatBlock((IASTCompoundStatement) bodyStmt, preferences.brace_position_for_method_declaration, preferences.insert_space_before_opening_brace_in_method_declaration, preferences.indent_statements_compare_to_body);
                        exitNode(bodyStmt);
                    }
    ...
    private int visit(ICPPASTLinkageSpecification node) {
        ...
        if (memberDecls.length == 1 && peekNextToken() != Token.tLBRACE) {
            ...
            formatOpeningBrace(preferences.brace_position_for_linkage_declaration, preferences.insert_space_before_opening_brace_in_linkage_declaration);
    ...
    private int visit(IASTFunctionDefinition node) {
        ...
        Runnable tailFormatter = null;
        IASTStatement bodyStmt = node.getBody();
        if (DefaultCodeFormatterConstants.END_OF_LINE.equals(preferences.brace_position_for_method_declaration) && !hasMemberInitializers(node) && !(node instanceof ICPPASTFunctionWithTryBlock) && bodyStmt instanceof IASTCompoundStatement && !startsWithMacroExpansion(bodyStmt)) {
            tailFormatter = new TrailingTokenFormatter(Token.tLBRACE, nodeOffset(bodyStmt), preferences.insert_space_before_opening_brace_in_method_declaration, false);
            scribe.setTailFormatter(tailFormatter);
        }
        declarator.accept(this);
        ...
        if (node instanceof ICPPASTFunctionDefinition) {
            ...
            if (constructorChain != null && constructorChain.length > 0) {
                ...
                if (preferences.insert_new_line_after_colon_in_constructor_initializer_list || preferences.insert_new_line_before_colon_in_constructor_initializer_list) {
    ...
    private int visit(ICPPASTFunctionDeclarator node) {
        ...
        formatList(parameters, options, true, node.takesVarArgs(), new FunctionDeclaratorTailFormatter(node, tailFormatter));
        return PROCESS_SKIP;
    }

    private void formatExceptionSpecification(final IASTTypeId[] exceptionSpecification) {
        if (exceptionSpecification.length > 0) {
            Alignment alignment = scribe.createAlignment(Alignment.EXCEPTION_SPECIFICATION, preferences.alignment_for_throws_clause_in_method_declaration, exceptionSpecification.length, getCurrentPosition());
            scribe.enterAlignment(alignment);
            boolean ok = false;
            do {
                try {
    ...
    private void formatExceptionSpecification(final ICPPASTExpression noexceptSpecification) {
        if (noexceptSpecification != null && noexceptSpecification != ICPPASTFunctionDeclarator.NOEXCEPT_DEFAULT) {
            Alignment alignment = scribe.createAlignment(Alignment.EXCEPTION_SPECIFICATION, preferences.alignment_for_throws_clause_in_method_declaration, 1, getCurrentPosition());
            scribe.enterAlignment(alignment);
            boolean ok = false;
            do {
                try {
    ...
    private boolean skipConstVolatileRestrict(boolean spaceBefore) {
        return skipTokenWhile(token -> token == Token.t_const || token == Token.t_volatile || token == Token.t_restrict, spaceBefore);
    }
    ...
    private int visit(IASTArrayDeclarator node) {
        ...
        if (arrayModifiers != null) {
            for (IASTArrayModifier arrayModifier : arrayModifiers) {
                ...
                boolean emptyBrackets = arrayModifier.getConstantExpression() == null && !(arrayModifier instanceof ICASTArrayModifier);
                if (!emptyBrackets) {
                ...
                boolean insertSpace = emptyBrackets ? preferences.insert_space_between_empty_brackets : preferences.insert_space_before_closing_bracket;
    ...
    private int visit(ICPPASTStructuredBindingDeclaration node) {
        ...
        if (refQualifier != null) {
            ...
            if (peekNextToken() == expectedToken) {
                scribe.printNextToken(expectedToken, preferences.insert_space_before_ref_qualifier_in_structured_binding);
            }
        }
        IASTInitializer initializer = node.getInitializer();
    ...
    private int visit(ICASTCompositeTypeSpecifier node) {
        ...
        if (name != null) {
            ...
            if (attributes.length > 0) {
                ...
                if (token == Token.tLBRACE || (name.getFileLocation() != null && nodeOffset(name) > nodeOffset(attributes[0]))) {
    ...
    private void formatList(List<?> elements, ListOptions options, boolean encloseInParen, boolean addEllipsis, Runnable tailFormatter, Runnable prefix) {
        ...
        if (encloseInParen) {
            boolean spaceBeforeClosingParen = elements.isEmpty() && !addEllipsis && options.captureDefault == CaptureDefault.UNSPECIFIED ? options.fSpaceBetweenEmptyParen : options.fSpaceBeforeClosingParen;
        ...
        if (!elements.isEmpty() || addEllipsis) {
            ...
            final int continuationIndentation = options.fContinuationIndentation >= 0 ? options.fContinuationIndentation : preferences.continuation_indentation;
            Alignment alignment = scribe.createAlignment(Alignment.LIST_ELEMENTS_PREFIX + //$NON-NLS-1$
            (elements.isEmpty() ? "ellipsis" : elements.get(0).getClass().getSimpleName()), options.fMode, options.fTieBreakRule, elementsLength + (addEllipsis ? 1 : 0), getCurrentPosition(), continuationIndentation, false);
            scribe.enterAlignment(alignment);
            boolean ok = false;
            do {
                try {
                    ...
                    for (i = 0; i < elementsLength; i++) {
                        ...
                        if (i < elementsLength - 1) {
                            scribe.setTailFormatter(new TrailingTokenFormatter(options.fSeparatorToken, findTokenAfterNodeOrTokenRange(options.fSeparatorToken, element), options.fSpaceBeforeSeparator, options.fSpaceAfterSeparator));
                        } else {
                            scribe.setTailFormatter(tailFormatter);
                        }
                        scribe.alignFragment(alignment, i);
    ...
    private int visit(IASTConditionalExpression node) {
        ...
        int numConditions = 0;
        for (IASTExpression expression = node; expression instanceof IASTConditionalExpression; expression = ((IASTConditionalExpression) expression).getNegativeResultExpression()) {
            numConditions++;
        }
        Runnable tailFormatter = scribe.takeTailFormatter();
        Alignment alignment = scribe.createAlignment(Alignment.CONDITIONAL_EXPRESSION_CHAIN, preferences.alignment_for_conditional_expression_chain, Alignment.R_OUTERMOST, numConditions, getCurrentPosition());
        scribe.enterAlignment(alignment);
        boolean ok = false;
        do {
            try {
    ...
    private void formatConditionalExpression(IASTConditionalExpression node, Runnable tailFormatter) {
        scribe.setTailFormatter(new TrailingTokenFormatter(Token.tQUESTION, node, preferences.insert_space_before_question_in_conditional, preferences.insert_space_after_question_in_conditional));
        ...
        Alignment alignment = scribe.createAlignment(Alignment.CONDITIONAL_EXPRESSION, preferences.alignment_for_conditional_expression, Alignment.R_OUTERMOST, negativeExpression instanceof IASTConditionalExpression ? 1 : 2, getCurrentPosition());
        scribe.enterAlignment(alignment);
        boolean ok = false;
        do {
            try {
                ...
                scribe.setTailFormatter(new TrailingTokenFormatter(Token.tCOLON, node, preferences.insert_space_before_colon_in_conditional, preferences.insert_space_after_colon_in_conditional));
    ...
    private int visit(IASTEqualsInitializer node) {
        if (node.getPropertyInParent() == IASTInitializerList.NESTED_INITIALIZER) {
            ...
            Alignment alignment = scribe.createAlignment(Alignment.DECLARATION_INITIALIZER, preferences.alignment_for_assignment, Alignment.R_INNERMOST, 1, getCurrentPosition());
            Runnable tailFormatter = scribe.getTailFormatter();
            scribe.enterAlignment(alignment);
            // Inherit tail formatter from the enclosing alignment
            scribe.setTailFormatter(tailFormatter);
            boolean ok = false;
            do {
                try {
    ...
    private int visit(ICASTDesignatedInitializer node) {
        ...
        Alignment expressionAlignment = scribe.createAlignment(Alignment.DESIGNATED_INITIALIZER, preferences.alignment_for_assignment, 1, getCurrentPosition());
    ...
    private int visit(IASTBinaryExpression node) {
        ...
        Alignment alignment = scribe.createAlignment(Alignment.BINARY_EXPRESSION, preferences.alignment_for_binary_expression, Alignment.R_OUTERMOST, operands.length, getCurrentPosition());
        scribe.enterAlignment(alignment);
        boolean ok = false;
        do {
            try {
    ...
    private int formatAssignment(IASTBinaryExpression node) {
        ...
        Alignment expressionAlignment = scribe.createAlignment(Alignment.ASSIGNMENT_EXPRESSION, preferences.alignment_for_assignment, Alignment.R_INNERMOST, 1, getCurrentPosition());
        scribe.enterAlignment(expressionAlignment);
        boolean ok = false;
        do {
            try {
    ...
    private int formatOverloadedLeftShiftChain(IASTBinaryExpression binaryExpression) {
        ...
        Alignment alignment = scribe.createAlignment(Alignment.OVERLOADED_LEFT_SHIFT_CHAIN, preferences.alignment_for_overloaded_left_shift_chain, Alignment.R_OUTERMOST, elements.size(), getCurrentPosition(), preferences.continuation_indentation, false);
        scribe.enterAlignment(alignment);
        boolean ok = false;
        do {
            try {
    ...
    private boolean isOverloadedLeftShift(IASTBinaryExpression node) {
        return node.getOperator() == IASTBinaryExpression.op_shiftLeft && node instanceof ICPPASTBinaryExpression && ((ICPPASTBinaryExpression) node).getOverload() != null;
    }
    ...
    private int visit(IASTFieldReference node) {
        ...
        if (fieldName != null) {
            Alignment alignment = scribe.createAlignment(Alignment.FIELD_REFERENCE, preferences.alignment_for_member_access, Alignment.R_OUTERMOST, 1, getCurrentPosition());
            scribe.enterAlignment(alignment);
            boolean ok = false;
            do {
                try {
    ...
    private int visit(IASTForStatement node) {
        ...
        if (!doNodesHaveSameOffset(node, initializerStmt)) {
            ...
            if (DefaultCodeFormatterConstants.END_OF_LINE.equals(preferences.brace_position_for_block) && body instanceof IASTCompoundStatement && !startsWithMacroExpansion(body)) {
                tailFormatter = new TrailingTokenFormatter(Token.tLBRACE, nodeOffset(body), preferences.insert_space_before_opening_brace_in_block, false);
            }
            tailFormatter = new ClosingParensesisTailFormatter(preferences.insert_space_before_closing_paren_in_for, tailFormatter);
        }
        ...
        Alignment alignment = scribe.createAlignment(Alignment.FOR, Alignment.M_COMPACT_SPLIT, Alignment.R_OUTERMOST, 2, getCurrentPosition());
        scribe.enterAlignment(alignment);
        boolean ok = false;
        do {
            try {
        ...
        if (body instanceof IASTCompoundStatement && !startsWithMacroExpansion(body)) {
            if (enterNode(body)) {
                ...
                formatBlock((IASTCompoundStatement) body, preferences.brace_position_for_block, preferences.insert_space_before_opening_brace_in_block, preferences.indent_statements_compare_to_block);
                exitNode(body);
            }
        } else {
    ...
    private int visit(IASTIfStatement node) {
        ...
        try {
            ...
            if (condition == null || !doNodesHaveSameOffset(node, condition)) {
                ...
                Runnable tailFormatter = null;
                if (DefaultCodeFormatterConstants.END_OF_LINE.equals(preferences.brace_position_for_block) && thenStatement instanceof IASTCompoundStatement && !startsWithMacroExpansion(thenStatement)) {
                    tailFormatter = new TrailingTokenFormatter(Token.tLBRACE, nodeOffset(thenStatement), preferences.insert_space_before_opening_brace_in_block, false);
                }
                tailFormatter = new ClosingParensesisTailFormatter(preferences.insert_space_before_closing_paren_in_if, tailFormatter);
                scribe.setTailFormatter(tailFormatter);
        ...
        if (thenStatement != null) {
            if (condition != null && doNodeLocationsOverlap(condition, thenStatement)) {
                ...
                if (elseStatement == null && preferences.keep_simple_if_on_one_line) {
                    Alignment compactIfAlignment = scribe.createAlignment(Alignment.COMPACT_IF, preferences.alignment_for_compact_if, Alignment.R_OUTERMOST, 1, getCurrentPosition(), 1, false);
            ...
            for (int i = 0; i < statementsLength; i++) {
                ...
                if (statement instanceof IASTCaseStatement || statement instanceof IASTDefaultStatement) {
                    ...
                    if (wasACase) {
                        ...
                        try {
                            enterNode(statement);
                            formatBlock((IASTCompoundStatement) statement, bracePosition, preferences.insert_space_after_colon_in_case, preferences.indent_statements_compare_to_block);
                            exitNode(statement);
                        } catch (ASTProblemException e) {
                        ...
                        try {
                            enterNode(statement);
                            formatBlock((IASTCompoundStatement) statement, bracePosition, preferences.insert_space_before_opening_brace_in_block, preferences.indent_statements_compare_to_block);
                            exitNode(statement);
                        } catch (ASTProblemException e) {
                            if (i < statementsLength - 1) {
                                final IASTStatement nextStatement = statements.get(i + 1);
    ...
    private boolean enterNode(IASTNode node) {
        ...
        if (nodeLocation != null) {
            ...
            if (currentPosition > nodeEndOffset)
                // We have already passed the end of the node.
                return false;
        }
        scribe.enterNode();
        if (node instanceof IASTProblemHolder)
        ...
        if (locations.length == 0) {
            ...
            if (locations.length <= 2 && (node instanceof IASTStatement || node instanceof IASTExpression)) {
                ...
                IASTFileLocation macroLocation = macroExpansion.getFileLocation();
                int macroOffset = macroLocation.getNodeOffset();
                if (macroOffset >= currentPosition && !scribe.shouldSkip(macroOffset) && (nodeEndOffset == macroOffset + macroLocation.getNodeLength() || locations.length == 2 && isSemicolonLocation(locations[1])) && isFunctionStyleMacroExpansion(macroExpansion)) {
                    if (locations.length == 2 && isSemicolonLocation(locations[1])) {
                        scribe.setTailFormatter(new TrailingTokenFormatter(Token.tSEMI, locations[1].getNodeOffset(), preferences.insert_space_before_semicolon, false));
                    }
                    formatFunctionStyleMacroExpansion(macroExpansion);
                    return false;
                }
    ...
    private boolean endsWithMacroExpansion(IASTNode node) {
        if (fInsideMacroArguments)
            return false;
        IASTNodeLocation[] locations = node.getNodeLocations();
        if (!(node instanceof IASTProblemHolder) && locations.length != 0 && locations[locations.length - 1] instanceof IASTMacroExpansionLocation) {
            return true;
        }
        return false;
    }
    ...
    private void formatOpenedBlock(IASTCompoundStatement block, String block_brace_position, boolean indentStatements) {
        final boolean startsWithStatementLikeMacro = startsWithMacroExpansion(block) && looksLikeStatement((IASTMacroExpansionLocation) block.getNodeLocations()[0]);
    ...
    private void formatLeftCurlyBrace(final int line, final String bracePosition) {
        ...
        try {
            ...
            scribe.printComment();
            if (DefaultCodeFormatterConstants.NEXT_LINE_ON_WRAP.equals(bracePosition) && (scribe.line > line || scribe.column >= preferences.page_width)) {
                scribe.startNewLine();
            }
        } finally {
    ...
    private void formatStatements(final List<IASTStatement> statements, boolean insertNewLineAfterLastStatement) {
        ...
        if (statementsLength > 1) {
            ...
            for (int i = 1; i < statementsLength - 1; i++) {
                final IASTStatement statement = statements.get(i);
                if ((!(statement instanceof IASTNullStatement) || isEmptyStatement(statement)) && !doNodeLocationsOverlap(statement, statements.get(i - 1))) {
                    scribe.startNewLine();
                }
            ...
            if (!(statement instanceof IASTNullStatement) && (!startsWithMacroExpansion(statement) || !looksLikeStatement((IASTMacroExpansionLocation) statement.getNodeLocations()[0]))) {
                scribe.startNewLine();
            }
            statement.accept(this);
        } else {
    ...
    private boolean isSemicolonLocation(IASTNodeLocation location) {
        return location instanceof IASTFileLocation && location.getNodeLength() == 1 && peekTokenAtPosition(location.getNodeOffset()) == Token.tSEMI && localScanner.getCurrentTokenEndPosition() + 1 == location.getNodeOffset() + location.getNodeLength();
    }
