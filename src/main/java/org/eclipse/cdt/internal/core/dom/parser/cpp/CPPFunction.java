...
public class CPPFunction extends PlatformObject implements ICPPFunction, ICPPInternalFunction {
    ...
    public IASTDeclarator[] declarations;

    public ICPPASTFunctionDeclarator definition;

    public ICPPFunctionType declaredType;

    public ICPPFunctionType type;
        ...
        if (size > 0) {
            for (int i = 0; i < size; i++) {
                ...
                if (binding instanceof ICPPParameter) {
                    ...
                    result[i] = new CPPParameter.CPPParameterProblem(param, IProblemBinding.SEMANTIC_INVALID_TYPE, name.toCharArray());
                }
            }
    ...
    public ICPPFunctionType getDeclaredType() {
        if (declaredType == null) {
            IType t = CPPVisitor.createType((definition != null) ? definition : declarations[0], CPPVisitor.DO_NOT_RESOLVE_PLACEHOLDERS);
    ...
    static public boolean hasStorageClass(ICPPInternalFunction function, int storage) {
        ...
        do {
            if (dtor != null) {
                ...
                while (!(parent instanceof IASTDeclaration)) parent = parent.getParent();
                IASTDeclSpecifier declSpec = null;
