...
public abstract class APathEntry extends PathEntry {
    ...
    public IPath baseRef;

    //$NON-NLS-1$
    final static public char[][] UNINIT_PATTERNS = new char[][] { "Non-initialized yet".toCharArray() };
    ...
    public String toString() {
        ...
        if (basePath != null && !basePath.isEmpty()) {
            //$NON-NLS-1$
            sb.append(" base-path:").append(basePath.toString());
        }
        if (baseRef != null && !baseRef.isEmpty()) {
            //$NON-NLS-1$
            sb.append(" base-ref:").append(baseRef.toString());
        }
        return sb.toString();
    }
}
