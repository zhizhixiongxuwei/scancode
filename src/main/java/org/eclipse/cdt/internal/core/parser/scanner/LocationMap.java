...
public class LocationMap implements ILocationResolver {
    ...
    final public LexerOptions fLexerOptions;

    public String fTranslationUnitPath;

    public IASTTranslationUnit fTranslationUnit;

    public ArrayList<ASTPreprocessorNode> fDirectives = new ArrayList<>();

    public ArrayList<ASTProblem> fProblems = new ArrayList<>();
    ...
    public ILocationCtx pushInclusion(int startOffset, int nameOffset, int nameEndOffset, int endOffset, AbstractCharArray buffer, String filename, char[] name, boolean userInclude, boolean heuristic, boolean isSource) {
        ...
        final ASTInclusionStatement inclusionStatement = new ASTInclusionStatement(fTranslationUnit, startNumber, nameNumber, nameEndNumber, endNumber, name, filename, userInclude, true, heuristic, exported, null);
    ...
    public ASTInclusionStatement encounterPoundInclude(int startOffset, int nameOffset, int nameEndOffset, int endOffset, char[] name, String filename, boolean userInclude, boolean active, boolean heuristic, IFileNomination nominationDelegate) {
        ...
        final ASTInclusionStatement inc = new ASTInclusionStatement(fTranslationUnit, startOffset, nameOffset, nameEndOffset, endOffset, name, filename, userInclude, active, heuristic, exported, nominationDelegate);
        fDirectives.add(inc);
        return inc;
    }
    ...
    public IASTImageLocation getImageLocation(int sequenceNumber, int length) {
        ...
        if (loc instanceof IASTFileLocation) {
            ...
            return new ASTImageLocation(IASTImageLocation.REGULAR_CODE, floc.getFileName(), floc.getNodeOffset(), floc.getNodeLength());
        }
