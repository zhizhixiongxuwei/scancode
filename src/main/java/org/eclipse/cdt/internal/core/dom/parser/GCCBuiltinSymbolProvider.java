...
public class GCCBuiltinSymbolProvider implements IBuiltinBindingsProvider {
     ...
     */
    public static final ASTNodeProperty BUILTIN_GCC_SYMBOL = new ASTNodeProperty(//$NON-NLS-1$
    "GCCBuiltinSymbolProvider.BUILTIN_GCC_SYMBOL - built-in GCC symbol");

    static final public Map<String, char[]> CHAR_ARRAYS = new HashMap<>();

    public IBinding[] fBindings;

    public IScope fScope;

    final public boolean fCpp;
    ...
    private void addGnuBuiltins() {
        ...
        for (String op : new String[] { "add", "sub", "mul" }) {
            ...
            function("bool", "__builtin_u" + op + "l_overflow", "unsigned long int", "unsigned long int", "unsigned long int*");
            function("bool", "__builtin_u" + op + "ll_overflow", "unsigned long long int", "unsigned long long int", "unsigned long long int*");
        }
        ...
        //TODO: This should technically be a constant integer expression
        ...
        // not in the manual
        function("void*", "__builtin_memchr", "const void*", "int", "size_t");
    ...
    private void variable(String type, String name) {
        IBinding b = fCpp ? new CPPBuiltinVariable(toType(type), toCharArray(name), fScope) : new CBuiltinVariable(toType(type), toCharArray(name), fScope);
        fBindingList.add(b);
    }

    private void typedef(String type, String name) {
        IBinding b = fCpp ? new CPPImplicitTypedef(toType(type), toCharArray(name), fScope) : new CImplicitTypedef(toType(type), toCharArray(name), fScope);
        fBindingList.add(b);
    }
    ...
    private void function(String returnType, String name, ICPPExecution exec, String... parameterTypes) {
        ...
        IBinding b = fCpp ? new CPPBuiltinImplicitFunction(toCharArray(name), fScope, (ICPPFunctionType) ft, (ICPPParameter[]) theParms, varargs, exec) : new CImplicitFunction(toCharArray(name), fScope, ft, theParms, varargs);
        fBindingList.add(b);
    }
    ...
    private IType createType(final String type) {
        ...
        if (tstr.equals("void")) {
            ...
            t = fCpp ? new CPPPointerType(new CPPFunctionType(rt, IType.EMPTY_TYPE_ARRAY, null)) : new CPointerType(new CFunctionType(rt, IType.EMPTY_TYPE_ARRAY), 0);
            ...
            t = fCpp ? new CPPPointerType(new CPPBasicType(Kind.eVoid, q)) : new CPointerType(new CBasicType(Kind.eVoid, q), 0);
        } else {
            throw new IllegalArgumentException(type);
        }
        if (isConst || isVolatile) {
            return fCpp ? new CPPQualifierType(t, isConst, isVolatile) : new CQualifierType(t, isConst, isVolatile, false);
        }
        return t;
    }
