...
public class CPPASTTemplateIDAmbiguity extends ASTAmbiguousNode implements IASTAmbiguousExpression, ICPPASTExpression {
    ...
    final public BinaryOperator fEndOperator;

    final public BranchPoint fVariants;

    public IASTNode[] fNodes;

    final public AbstractGNUSourceCodeParser fParser;
            ...
            if (templateName.getTranslationUnit() != null) {
                ...
                if (b instanceof IProblemBinding) {
                    if (!containsFunctionTemplate(((IProblemBinding) b).getCandidateBindings()))
                        return -1;
                    count++;
                } else if (b instanceof ICPPSpecialization || b instanceof ICPPTemplateDefinition || b instanceof ICPPAliasTemplateInstance || b instanceof ICPPConstructor || (b instanceof IFunction && b instanceof ICPPUnknownBinding)) {
                    count++;
                } else {
                    return -1;
                }
