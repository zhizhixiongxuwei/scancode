...
public class MultiConfigDescription extends MultiItemsHolder implements ICMultiConfigDescription {
    ...
    public ICConfigurationDescription[] fCfgs = null;
    ...
    @Override
    public ICExternalSetting createExternalSetting(String[] languageIDs, String[] contentTypeIds, String[] extensions, ICSettingEntry[] entries) throws WriteAccessException {
        if (DEBUG)
            //$NON-NLS-1$
    ...
    public String[][] getErrorParserIDs() {
        ...
        for (int i = 0; i < fCfgs.length; i++) out[i] = fCfgs[i].getBuildSetting().getErrorParserIDs();
        return out;
    }

    @Override
    public void setErrorParserIDs(String[] ids) {
        for (int i = 0; i < fCfgs.length; i++) fCfgs[i].getBuildSetting().setErrorParserIDs(ids);
    }
    ...
    public ICFileDescription[] getFileDescriptions() {
        ...
        for (int i = 0; i < fCfgs.length; i++) lst.addAll(Arrays.asList(fCfgs[i].getFileDescriptions()));
    ...
    public ICFolderDescription[] getFolderDescriptions() {
        ...
        for (int i = 0; i < fCfgs.length; i++) lst.addAll(Arrays.asList(fCfgs[i].getFolderDescriptions()));
    ...
    public ICProjectDescription getProjectDescription() {
        ...
        if (pd == null)
            return null;
        for (int i = 1; i < fCfgs.length; i++) if (!pd.equals(fCfgs[i].getProjectDescription()))
            // Different projects !
            return null;
        return pd;
    }
    ...
    public ICResourceDescription[] getResourceDescriptions() {
        ArrayList<ICResourceDescription> lst = new ArrayList<>();
        for (int i = 0; i < fCfgs.length; i++) lst.addAll(Arrays.asList(fCfgs[i].getResourceDescriptions()));
    ...
    public ICFolderDescription getRootFolderDescription() {
        ...
        for (int i = 0; i < fCfgs.length; i++) rds[i] = fCfgs[i].getRootFolderDescription();
    ...
    public boolean isActive() {
        for (int i = 0; i < fCfgs.length; i++) if (fCfgs[i].isActive())
            return true;
        return false;
    }
    ...
    public boolean isModified() {
        for (int i = 0; i < fCfgs.length; i++) if (fCfgs[i].isModified())
            return true;
        return false;
    }
    ...
    public void remove(ICConfigExtensionReference ext) throws CoreException {
        for (int i = 0; i < fCfgs.length; i++) fCfgs[i].remove(ext);
    }

    @Override
    public void remove(String extensionPoint) throws CoreException {
        for (int i = 0; i < fCfgs.length; i++) fCfgs[i].remove(extensionPoint);
    }

    @Override
    public void removeExternalSetting(ICExternalSetting setting) throws WriteAccessException {
        for (int i = 0; i < fCfgs.length; i++) fCfgs[i].removeExternalSetting(setting);
    }

    @Override
    public void removeExternalSettings() throws WriteAccessException {
        for (int i = 0; i < fCfgs.length; i++) fCfgs[i].removeExternalSettings();
    }
    ...
    public void setActive() throws WriteAccessException {
        for (int i = 0; i < fCfgs.length; i++) fCfgs[i].setActive();
    }
    ...
    public void setExternalSettingsProviderIds(String[] ids) {
        for (int i = 0; i < fCfgs.length; i++) fCfgs[i].setExternalSettingsProviderIds(ids);
    }
    ...
    public void setReferenceInfo(Map<String, String> refs) throws WriteAccessException {
        for (int i = 0; i < fCfgs.length; i++) fCfgs[i].setReferenceInfo(refs);
    }

    @Override
    public void setSessionProperty(QualifiedName name, Object value) {
        for (int i = 0; i < fCfgs.length; i++) fCfgs[i].setSessionProperty(name, value);
    }

    @Override
    public void setSourceEntries(ICSourceEntry[] entries) throws CoreException, WriteAccessException {
        for (int i = 0; i < fCfgs.length; i++) fCfgs[i].setSourceEntries(entries);
    }

    @Override
    public void updateExternalSettingsProviders(String[] ids) throws WriteAccessException {
        for (int i = 0; i < fCfgs.length; i++) fCfgs[i].updateExternalSettingsProviders(ids);
    }
    ...
    public ICSettingContainer getParent() {
        ...
        if (p == null)
            return null;
        for (int i = 1; i < fCfgs.length; i++) if (!p.equals(fCfgs[i].getParent()))
            return null;
        return p;
    }
    ...
    public int getType() {
        ...
        for (int i = 1; i < fCfgs.length; i++) if (t != fCfgs[i].getType())
            return 0;
        return t;
    }
    ...
    public boolean isReadOnly() {
        for (int i = 0; i < fCfgs.length; i++) if (!fCfgs[i].isReadOnly())
            return false;
        return true;
    }
    ...
    public void setReadOnly(boolean readOnly, boolean keepModify) {
        for (ICConfigurationDescription cfg : fCfgs) cfg.setReadOnly(readOnly, keepModify);
    }

    @Override
    public boolean isValid() {
        for (int i = 0; i < fCfgs.length; i++) if (!fCfgs[i].isValid())
            return false;
        return true;
    }
    ...
    public void removeStorage(String id) throws CoreException {
        for (int i = 0; i < fCfgs.length; i++) fCfgs[i].removeStorage(id);
    }
}
