...
public abstract class ASTAmbiguousNode extends ASTNode {
    ...
    private void logAmbiguousNodeError() {
        CCorePlugin.log(new ParserException(//$NON-NLS-1$
        "Encountered an ambiguous node \"" + getRawSignature() + "\" at " + //$NON-NLS-1$
        getFileLocation().getFileName() + ", line " + //$NON-NLS-1$
        getFileLocation().getStartingLineNumber() + " while parsing " + //$NON-NLS-1$
        getTranslationUnit().getContainingFilename()));
    }
}
