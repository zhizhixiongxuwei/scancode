...
public class EvalBinary extends CPPDependentEvaluation {
    ...
    final public ICPPEvaluation fArg1;

    final public ICPPEvaluation fArg2;

    public ICPPFunction fOverload = CPPFunction.UNINITIALIZED_FUNCTION;
    ...
    public IType getType() {
        if (fType == null) {
            if (isTypeDependent()) {
                ...
                if (overload != null) {
                    fType = ExpressionTypes.restoreTypedefs(ExpressionTypes.typeFromFunctionCall(overload), fArg1.getType(), fArg2.getType());
                } else {
                    fType = computeType();
                }
            }
    ...
    public IValue getValue() {
        ...
        if (overload != null) {
            ...
            if (parameterTypes.length >= 2) {
                ...
                CCorePlugin.log(IStatus.ERROR, //$NON-NLS-1$
                "Unexpected overload for binary operator " + fOperator + ": '" + overload.getName() + //$NON-NLS-1$//$NON-NLS-2$
                "'");
            }
    ...
    private ICPPFunction computeOverload() {
        ...
        if (fOperator == op_arrayAccess) {
            ...
            if (type instanceof ICPPClassType) {
                return CPPSemantics.findOverloadedBinaryOperator(getTemplateDefinitionScope(), OverloadableOperator.BRACKET, fArg1, fArg2);
            }
        } else {
    ...
    private static IType computeTypeForAssignment(IType targetType, ValueCategory targetCategory, IType sourceType, ValueCategory sourceCategory) {
        ...
        try {
            ...
            cost = Conversions.checkImplicitConversionSequence(targetT, sourceType, XVALUE, UDCMode.ALLOWED, Context.ORDINARY, false);
    ...
    public String toString() {
        //$NON-NLS-1$
        return fArg1.toString() + " <op> " + fArg2.toString();
    }
