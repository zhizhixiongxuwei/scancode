...
public class SetCProjectDescriptionOperation extends CModelOperation {
     ...
     */
    final public AbstractCProjectDescriptionStorage fPrjDescStorage;

    final public CProjectDescription fSetDescription;

    final public int fFlags;
    ...
    protected void executeOperation() throws CModelException {
        ...
        if (fSetDescription != null) {
            ...
            if (!fSetDescription.isValid() && (!mngr.isEmptyCreatingDescriptionAllowed() || !creating))
                throw new CModelException(ExceptionFactory.createCoreException(//$NON-NLS-1$
        ...
        for (ICElementDelta d : cElementDeltas) addDelta(d);
        if (fSetDescription != null)
    ...
    private void setEnvStates(CProjectDescription pd, boolean[] data) {
        ...
        for (int i = 0; i < cfs.length; i++) {
            if (data.length <= i) {
                ...
                log(//$NON-NLS-1$
                "Error: setEnvStates: different number of configurations as there are envDatas...", new Exception());
                break;
            }
            if (!data[i])
                // write only TRUE values
                continue;
            if (cfs[i] instanceof IInternalCCfgInfo) {
                try {
                    CConfigurationSpecSettings ss = ((IInternalCCfgInfo) cfs[i]).getSpecSettings();
