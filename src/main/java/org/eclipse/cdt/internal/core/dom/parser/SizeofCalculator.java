...
public class SizeofCalculator {
    ...
    static final public SizeofCalculator defaultInstance = new SizeofCalculator();

    static final public SizeAndAlignment SIZE_1 = new SizeAndAlignment(1, 1);
    ...
    public static SizeAndAlignment getSizeAndAlignment(IType type) {
        ...
        SizeofCalculator calc = point == null ? getDefault() : ((ASTTranslationUnit) point.getTranslationUnit()).getSizeofCalculator();
    ...
    public SizeofCalculator(IASTTranslationUnit ast) {
        ...
        //$NON-NLS-1$
        sizeof_pointer = getSize(sizeofMacros, "__SIZEOF_POINTER__", maxAlignment);
        //$NON-NLS-1$
        sizeof_int = getSize(sizeofMacros, "__SIZEOF_INT__", maxAlignment);
        //$NON-NLS-1$
        sizeof_long = getSize(sizeofMacros, "__SIZEOF_LONG__", maxAlignment);
        //$NON-NLS-1$
        sizeof_long_long = getSize(sizeofMacros, "__SIZEOF_LONG_LONG__", maxAlignment);
        //$NON-NLS-1$
        sizeof_int128 = getSize(sizeofMacros, "__SIZEOF_INT128__", maxAlignment);
        //$NON-NLS-1$
        sizeof_short = getSize(sizeofMacros, "__SIZEOF_SHORT__", maxAlignment);
        //$NON-NLS-1$
        SizeAndAlignment size = getSize(sizeofMacros, "__SIZEOF_BOOL__", maxAlignment);
        // __SIZEOF_BOOL__ is not defined by GCC but sizeof(bool) is needed for template resolution.
        if (size == null)
            size = SIZE_1;
        sizeof_bool = size;
        //$NON-NLS-1$
        sizeof_wchar_t = getSize(sizeofMacros, "__SIZEOF_WCHAR_T__", maxAlignment);
        //$NON-NLS-1$
        sizeof_float = getSize(sizeofMacros, "__SIZEOF_FLOAT__", maxAlignment);
        sizeof_complex_float = getSizeOfPair(sizeof_float);
        //$NON-NLS-1$
        sizeof_double = getSize(sizeofMacros, "__SIZEOF_DOUBLE__", maxAlignment);
        sizeof_complex_double = getSizeOfPair(sizeof_double);
        //$NON-NLS-1$
        sizeof_long_double = getSize(sizeofMacros, "__SIZEOF_LONG_DOUBLE__", maxAlignment);
        sizeof_complex_long_double = getSizeOfPair(sizeof_long_double);
        // GCC does not define __SIZEOF_FLOAT128__
        sizeof_float128 = size_16;
        sizeof_complex_float128 = getSizeOfPair(sizeof_float128);
        // GCC does not define __SIZEOF_DECIMAL32__
        sizeof_decimal32 = size_4;
        // GCC does not define __SIZEOF_DECIMAL64__
        sizeof_decimal64 = size_8;
        // GCC does not define __SIZEOF_DECIMAL128__
    ...
    private SizeAndAlignment sizeAndAlignment(IBasicType type) {
        ...
        switch(kind) {
            ...
            case eChar8:
                return SIZE_1;
            case eInt:
                return type.isShort() ? sizeof_short : type.isLong() ? sizeof_long : type.isLongLong() ? sizeof_long_long : sizeof_int;
            case eInt128:
                return sizeof_int128;
            case eFloat:
                return type.isComplex() ? sizeof_complex_float : sizeof_float;
            case eDouble:
                return type.isComplex() ? (type.isLong() ? sizeof_complex_long_double : sizeof_complex_double) : (type.isLong() ? sizeof_long_double : sizeof_double);
            case eFloat128:
    ...
    private SizeAndAlignment sizeAndAlignment(ICompositeType type) {
        ...
        if (type instanceof ICPPClassType) {
            ...
            try {
                ...
                for (ICPPBase base : classType.getBases()) {
                    if (base.isVirtual())
                        // Don't know how to calculate size when there are virtual bases.
                        return null;
        ...
        if (// a structure cannot have size 0
        size == 0)
            size = 1;
    ...
    private SizeAndAlignment getSizeOfPair(SizeAndAlignment sizeAndAlignment) {
        return sizeAndAlignment == null ? null : new SizeAndAlignment(sizeAndAlignment.size * 2, sizeAndAlignment.alignment);
    }
}
