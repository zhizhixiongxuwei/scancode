...
public class EvalMemberAccess extends CPPDependentEvaluation {
    ...
    final public IType fOwnerType;

    final public IBinding fMember;

    final public ValueCategory fOwnerValueCategory;

    final public boolean fIsPointerDeref;
    ...
    public ICPPEvaluation fOwnerEval;
    ...
    public EvalMemberAccess(IType ownerType, ValueCategory ownerValueCat, IBinding member, boolean isPointerDeref, IASTNode pointOfDefinition) {
        this(ownerType, ownerValueCat, member, isPointerDeref, findEnclosingTemplate(pointOfDefinition));
    }

    public EvalMemberAccess(IType ownerType, ValueCategory ownerValueCat, IBinding member, boolean isPointerDeref, IBinding templateDefinition) {
        this(ownerType, ownerValueCat, member, null, isPointerDeref, templateDefinition);
    }

    public EvalMemberAccess(IType ownerType, ValueCategory ownerValueCat, IBinding member, ICPPEvaluation ownerEval, boolean isDeref, IASTNode pointOfDefinition) {
        this(ownerType, ownerValueCat, member, ownerEval, isDeref, findEnclosingTemplate(pointOfDefinition));
    }

    public EvalMemberAccess(IType ownerType, ValueCategory ownerValueCat, IBinding member, ICPPEvaluation ownerEval, boolean isDeref, IBinding templateDefinition) {
    ...
    public boolean isEquivalentTo(ICPPEvaluation other) {
        ...
        return fOwnerType.isSameType(o.fOwnerType) && fMember == o.fMember && fOwnerValueCategory == o.fOwnerValueCategory && fIsPointerDeref == o.fIsPointerDeref;
    }
    ...
    public static IType getFieldOwnerType(IType fieldOwnerExpressionType, boolean isDeref, Collection<ICPPFunction> functionBindings, boolean returnDependent) {
        ...
        for (int j = 0; j < 20; j++) {
            ...
            ICPPFunction op = CPPSemantics.findOverloadedOperator(null, args, classType, OverloadableOperator.ARROW, LookupMode.NO_GLOBALS);
            if (op == null)
                break;
        ...
        if (CPPTemplates.isDependentType(type)) {
            return returnDependent ? // The type resulting from dereferecing 'type'
            new TypeOfDependentExpression(new EvalUnary(IASTUnaryExpression.op_star, new EvalFixed(type, LVALUE, IntegralValue.UNKNOWN), null, CPPSemantics.getCurrentLookupPoint())) : null;
        }
    ...
    public ICPPEvaluation instantiate(InstantiationContext context, int maxDepth) {
        ...
        return new EvalMemberAccess(ownerType, fOwnerValueCategory, member, ownerEval, fIsPointerDeref, getTemplateDefinition());
    }
    ...
    public ICPPEvaluation computeForFunctionCall(ActivationRecord record, ConstexprEvaluationContext context) {
        ...
        if (fMember instanceof ICPPField) {
            ...
            if (field.getType() instanceof IArrayType) {
                EvalPointer evalPointer = new EvalPointer(record, new EvalCompositeAccess(new EvalCompositeAccess(evaluatedOwner, fieldPos), 0), getTemplateDefinition());
                return evalPointer;
            }
        }
        EvalReference evalRef = new EvalReference(record, new EvalCompositeAccess(evaluatedOwner, fieldPos), getTemplateDefinition());
        return evalRef;
    }
