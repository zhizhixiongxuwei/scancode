...
public class FunctionCost {
    ...
    public FunctionCost(ICPPFunction fn, Cost cost) {
        ...
        // no udc will be performed
        fValueCategories = null;
    }
        ...
        for (int i = 0; i < fCosts.length; i++) {
            ...
            switch(cost.isDeferredUDC()) {
                ...
                case COPY_INIT_OF_CLASS:
                    udcCost = Conversions.copyInitializationOfClass(fValueCategories[i], cost.source, (ICPPClassType) cost.target, false, noNarrowing);
                    break;
                case INIT_BY_CONVERSION:
                    IType uqSource = getNestedType(cost.source, TDEF | REF | CVTYPE);
                    udcCost = Conversions.initializationByConversion(fValueCategories[i], cost.source, (ICPPClassType) uqSource, cost.target, false, allowsContextualBooleanConversion(), noNarrowing);
                    break;
                case LIST_INIT_OF_CLASS:
                    udcCost = Conversions.listInitializationOfClass(((InitializerListType) cost.source).getEvaluation(), (ICPPClassType) cost.target, false, false);
                    break;
                case DIRECT_LIST_INIT_OF_CLASS:
                    udcCost = Conversions.listInitializationOfClass(((InitializerListType) cost.source).getEvaluation(), (ICPPClassType) cost.target, true, false);
                    break;
                default:
                    return false;
            }
    ...
    private boolean allowsContextualBooleanConversion() {
        ...
        return Arrays.equals(functionName, OverloadableOperator.AND.toCharArray()) || Arrays.equals(functionName, OverloadableOperator.OR.toCharArray()) || Arrays.equals(functionName, OverloadableOperator.NOT.toCharArray());
    }
