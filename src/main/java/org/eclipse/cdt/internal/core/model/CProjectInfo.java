        ...
        try {
            if (res instanceof IContainer) {
                ...
                for (int i = 0; i < resources.length; ++i) {
                    ...
                    boolean found = false;
                    for (int j = 0; j < sourceRoots.length; ++j) if (sourceRoots[j].isOnSourceEntry(child)) {
                        found = true;
                        break;
                    }
                    if (found) {
                        switch(child.getType()) {
                            case IResource.FILE:
                                ...
                                if (CoreModel.isValidTranslationUnitName(cproject.getProject(), child.getName()) || (cproject.isOnOutputEntry(child) && (CModelManager.getDefault().createBinaryFile((IFile) child) != null)))
                                    continue;
                                break;
