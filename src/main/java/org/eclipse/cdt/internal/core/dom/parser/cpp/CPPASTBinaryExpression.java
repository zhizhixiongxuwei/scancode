...
public class CPPASTBinaryExpression extends ASTNode implements ICPPASTBinaryExpression, IASTAmbiguityParent {
    ...
    public int fOperator;

    public ICPPASTExpression fOperand1;

    public ICPPASTInitializerClause fOperand2;

    public ICPPEvaluation fEvaluation;

    public IASTImplicitName[] fImplicitNames;
    ...
    public CPPASTBinaryExpression copy(CopyStyle style) {
        CPPASTBinaryExpression copy = new CPPASTBinaryExpression(fOperator, fOperand1 == null ? null : fOperand1.copy(style), fOperand2 == null ? null : fOperand2.copy(style));
    ...
    private static boolean acceptWithoutRecursion(IASTBinaryExpression bexpr, ASTVisitor action) {
        ...
        while (stack != null) {
            ...
            if (stack.fState == 1) {
                if (action.shouldVisitImplicitNames && !acceptByNodes(((IASTImplicitNameOwner) expr).getImplicitNames(), action)) {
                    return false;
                }
                stack.fState = 2;
