...
public class EvalUnary extends CPPDependentEvaluation {
    ...
    static final public ICPPEvaluation ZERO_EVAL = new EvalFixed(CPPSemantics.INT_TYPE, PRVALUE, IntegralValue.create(0));

    final public int fOperator;

    final public ICPPEvaluation fArgument;

    final public IBinding fAddressOfQualifiedNameBinding;
    ...
    private ICPPFunction computeOverload() {
        ...
        return CPPSemantics.findOverloadedOperator(getTemplateDefinitionScope(), args, type, op, LookupMode.LIMITED_GLOBALS);
    }
    ...
    private ICPPEvaluation createOperatorOverloadEvaluation(ICPPFunction overload, ICPPEvaluation arg) {
        ...
        if (overload instanceof ICPPMethod) {
            EvalMemberAccess opAccess = new EvalMemberAccess(arg.getType(), ValueCategory.LVALUE, overload, arg, false, templateDefinition);
    ...
    public int determinePackSize(ICPPTemplateParameterMap tpMap) {
        if (fOperator == op_integerPack) {
            ICPPEvaluation instantiatedArg = fArgument.instantiate(new InstantiationContext(tpMap), IntegralValue.MAX_RECURSION_DEPTH);
