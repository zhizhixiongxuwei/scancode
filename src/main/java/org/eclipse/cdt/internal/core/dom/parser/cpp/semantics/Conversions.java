...
public class Conversions {
    ...
    public enum Context {
        ...
        ORDINARY, IMPLICIT_OBJECT_FOR_METHOD_WITHOUT_REF_QUALIFIER, IMPLICIT_OBJECT_FOR_METHOD_WITH_REF_QUALIFIER, FIRST_PARAM_OF_DIRECT_COPY_CTOR, REQUIRE_DIRECT_BINDING
    }

    //$NON-NLS-1$
    static final public char[] INITIALIZER_LIST_NAME = "initializer_list".toCharArray();

    //$NON-NLS-1$
    static final public char[] STD_NAME = "std".toCharArray();
     ...
     * @throws DOMException
     */
    public static Cost checkImplicitConversionSequence(IType target, IType exprType, ValueCategory valueCat, UDCMode udc, Context ctx, boolean noNarrowing) throws DOMException {
        ...
        if (target instanceof ICPPReferenceType) {
            ...
            if (valueCat == ValueCategory.XVALUE || (valueCat == ValueCategory.PRVALUE && (T2 instanceof ICPPClassType || T2 instanceof IArrayType)) || (valueCat == ValueCategory.LVALUE && T2 instanceof ICPPFunctionType)) {
                Cost cost = isReferenceCompatible(cv1T1, cv2T2, isImpliedObject);
                if (cost != null) {
                    cost.setReferenceBinding(refBindingType);
                    return cost;
     ...
     */
    private static Cost initializationByConversionForDirectReference(final IType cv1T1, final IType cv2T2, final ICPPClassType T2, boolean needLValue, boolean illFormedIfLValue, Context ctx) throws DOMException {
        ...
        if (fcns.length > 0 && !(fcns[0] instanceof IProblemBinding)) {
            for (final ICPPMethod op : fcns) {
                ...
                // expression type to implicit object type
                Cost udcCost = isReferenceCompatible(getNestedType(implicitParameterType, TDEF | REF), cv2T2, true);
                if (udcCost != null) {
                    ...
                    if (cmp <= 0) {
                        // converted to target
                        Cost cost = isReferenceCompatible(cv1T1, getNestedType(t, TDEF | REF), false);
                        if (cost != null) {
    ...
    private static Cost nonReferenceConversion(ValueCategory valueCat, IType source, IType target, UDCMode udc, boolean noNarrowing) throws DOMException {
        ...
        if (uqTarget instanceof ICPPClassType) {
            ...
            if (udc == UDCMode.FORBIDDEN)
                return Cost.NO_CONVERSION;
            return copyInitializationOfClass(valueCat, source, (ICPPClassType) uqTarget, udc == UDCMode.DEFER, noNarrowing);
        }
        if (uqSource instanceof ICPPClassType) {
            if (udc == UDCMode.FORBIDDEN)
                return Cost.NO_CONVERSION;
            return initializationByConversion(valueCat, source, (ICPPClassType) uqSource, target, udc == UDCMode.DEFER, false, noNarrowing);
        }
    ...
    static Cost listInitializationSequenceHelper(EvalInitList arg, IType target, UDCMode udc, boolean isDirect) throws DOMException {
        ...
        if (listType != null) {
            ...
            for (ICPPEvaluation clause : arg.getClauses()) {
                Cost cost = checkImplicitConversionSequence(listType, clause.getType(), clause.getValueCategory(), UDCMode.ALLOWED, Context.ORDINARY, false);
                if (!cost.converts())
                    return cost;
        ...
        if (args.length == 1) {
            ...
            if (!firstArg.isInitializerList()) {
                Cost cost = checkImplicitConversionSequence(target, firstArg.getType(), firstArg.getValueCategory(), udc, Context.ORDINARY, false);
                if (cost.isNarrowingConversion()) {
                    return Cost.NO_CONVERSION;
                }
                return cost;
    ...
    static Cost listInitializationOfClass(EvalInitList arg, ICPPClassType t, boolean isDirect, boolean deferUDC) throws DOMException {
        ...
        } else if (result instanceof IProblemBinding && ((IProblemBinding) result).getID() == IProblemBinding.SEMANTIC_AMBIGUOUS_LOOKUP) {
     ...
     */
    static final Cost copyInitializationOfClass(ValueCategory valueCat, IType source, ICPPClassType t, boolean deferUDC, boolean noNarrowing) throws DOMException {
        if (deferUDC) {
        ...
        ctors = CPPTemplates.instantiateForFunctionCall(ctors, null, Collections.singletonList(source), Collections.singletonList(valueCat), false);
            ...
            if (!ctor.isExplicit()) {
                ...
                if (ptypes.length == 0) {
                    ...
                    if (ctor.getRequiredArgumentCount() > 1)
                        continue;
                    Cost cost = checkImplicitConversionSequence(ptype, source, valueCat, UDCMode.FORBIDDEN, Context.ORDINARY, noNarrowing);
     ...
     */
    static Cost initializationByConversion(ValueCategory valueCat, IType source, ICPPClassType uqSource, IType target, boolean deferUDC, boolean allowExplicitConversion, boolean noNarrowing) throws DOMException {
        if (deferUDC) {
        ...
        for (final ICPPFunction f : ops) {
            if (f instanceof ICPPMethod && !(f instanceof IProblemBinding)) {
                ...
                Cost c2 = checkImplicitConversionSequence(target, uqReturnType, valueCategoryFromReturnType(uqReturnType), UDCMode.FORBIDDEN, Context.ORDINARY, noNarrowing);
                if (c2.converts()) {
    ...
    private static final boolean qualificationConversion(Cost cost) {
        ...
        while (true) {
            ...
            if (s instanceof IPointerType && t instanceof IPointerType) {
                // is t more qualified than s?
                final int cmp = compareQualifications(t, s);
        ...
        // is t more qualified than s?
        int cmp = compareQualifications(t, s);
        if (cmp < 0 || (cmp > 0 && !constInEveryCV2k)) {
            return false;
        }
        adjustments |= (cmp << shift);
    ...
    public static IType copyPointer(final IPointerType p1, IType target, final boolean isConst, final boolean isVolatile) {
            ...
            switch(basicTypeKind) {
                ...
                case eInt:
                    ...
                    if (sizeToType.size <= 8) {
                        ...
                        //$NON-NLS-1$
                        biVal = biVal.add(new BigInteger("18446744073709551616"));
                        return biVal;
                    }
