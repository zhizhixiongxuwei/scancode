...
public abstract class PDOMIndexerTask extends AbstractIndexerTask implements IPDOMIndexerTask {
    ...
    static final public String TRUE = Boolean.TRUE.toString();

    public AbstractPDOMIndexer fIndexer;

    public boolean fWriteInfoToLog;
    ...
    protected PDOMIndexerTask(ITranslationUnit[] forceFiles, ITranslationUnit[] updateFiles, ITranslationUnit[] removeFiles, AbstractPDOMIndexer indexer, boolean isFastIndexer) {
        ...
        if (checkProperty(IndexerPreferences.KEY_INDEX_ALL_FILES)) {
            ...
            if (i1 == i2) {
                ...
                strategy = i1 == CProject.hasCCNature(getCProject().getProject()) ? UnusedHeaderStrategy.useCPP : UnusedHeaderStrategy.useC;
            }
        ...
        String privatePattern = CCorePreferenceConstants.getPreference(CCorePreferenceConstants.INCLUDE_PRIVATE_PATTERN, project, null);
    ...
    private Set<String> getStringSet(String key) {
        ...
        if (prefSetting != null && !prefSetting.isEmpty()) {
            //$NON-NLS-1$
            return new HashSet<>(Arrays.asList(prefSetting.split(",")));
        }
    ...
    protected final IncludeExportPatterns getIncludeExportPatterns() {
        ICProject project = getCProject();
        String exportPattern = CCorePreferenceConstants.getPreference(CCorePreferenceConstants.INCLUDE_EXPORT_PATTERN, project, null);
        String beginExportsPattern = CCorePreferenceConstants.getPreference(CCorePreferenceConstants.INCLUDE_BEGIN_EXPORTS_PATTERN, project, null);
        String endExportsPattern = CCorePreferenceConstants.getPreference(CCorePreferenceConstants.INCLUDE_END_EXPORTS_PATTERN, project, null);
    ...
    protected void traceEnd(long start, IWritableIndex index, boolean wasCancelled) {
        ...
        if (fWriteInfoToLog && !wasCancelled && index != null) {
            ...
            final String msg = NLS.bind(Messages.PDOMIndexerTask_indexerInfo, new Object[] { getCProject().getElementName(), nfGroup.format(info.fCompletedSources), nfGroup.format(info.fCompletedHeaders), nfTime.format(totalTime / 1000.), nfGroup.format(fStatistics.fDeclarationCount), nfGroup.format(fStatistics.fReferenceCount), nfGroup.format(fStatistics.fUnresolvedIncludesCount), nfGroup.format(fStatistics.fPreprocessorProblemCount + fStatistics.fSyntaxProblemsCount), nfGroup.format(fStatistics.fProblemBindingCount), nfPercent.format(problemPct) });
        ...
        if (checkDebugOption(IPDOMIndexerTask.TRACE_STATISTICS, TRUE)) {
            ...
            "C/C++ Indexer: Project '" + getCProject().getElementName() + "' (" + info.fCompletedSources + //$NON-NLS-1$//$NON-NLS-2$
            ...
            System.out.println(//$NON-NLS-1$
            indent + " Options: " + "indexer='" + //$NON-NLS-1$
            kind + "', parseAllFiles=" + //$NON-NLS-1$
            indexFilesWithoutConfiguration() + ", unusedHeaders=" + //$NON-NLS-1$
            getIndexHeadersWithoutContext() + ", skipReferences=" + //$NON-NLS-1$
            skipRefs + ", skipImplicitReferences=" + //$NON-NLS-1$
            skipImplRefs + ", skipTypeReferences=" + //$NON-NLS-1$
            skipTypeRefs + ", skipMacroReferences=" + //$NON-NLS-1$
            skipMacroRefs + //$NON-NLS-1$
            ".");
            //$NON-NLS-1$ //$NON-NLS-2$
            System.out.println(indent + " Database: " + dbSize + " bytes");
            System.out.println(//$NON-NLS-1$
            indent + " Timings: " + totalTime + //$NON-NLS-1$
            ...
            " index update.");
            System.out.println(//$NON-NLS-1$
            indent + " Errors: " + fStatistics.fErrorCount + //$NON-NLS-1$
            " internal, " + fStatistics.fUnresolvedIncludesCount + //$NON-NLS-1$
            " include, " + fStatistics.fPreprocessorProblemCount + //$NON-NLS-1$
            " scanner, " + fStatistics.fSyntaxProblemsCount + //$NON-NLS-1$
            " syntax errors.");
            if (fStatistics.fTooManyTokensCount > 0)
                System.out.println(//$NON-NLS-1$
                indent + " Tokens: " + fStatistics.fTooManyTokensCount + //$NON-NLS-1$
                " TUs with too many tokens.");
            NumberFormat nfPercent = NumberFormat.getPercentInstance();
            nfPercent.setMaximumFractionDigits(2);
            nfPercent.setMinimumFractionDigits(2);
            System.out.println(//$NON-NLS-1$
            indent + " Names: " + fStatistics.fDeclarationCount + //$NON-NLS-1$
            " declarations, " + fStatistics.fReferenceCount + //$NON-NLS-1$
            " references, " + fStatistics.fProblemBindingCount + "(" + nfPercent.format(problemPct) + //$NON-NLS-1$ //$NON-NLS-2$
            ") unresolved.");
            ...
            System.out.println(//$NON-NLS-1$
            indent + " Cache[" + ChunkCache.getSharedInstance().getMaxSize() / 1024 / 1024 + //$NON-NLS-1$
            "MB]: " + +hits + //$NON-NLS-1$
            " hits, " + misses + "(" + nfPercent.format(missPct) + //$NON-NLS-1$ //$NON-NLS-2$
            ") misses.");
    ...
    public synchronized boolean acceptUrgentTask(IPDOMIndexerTask urgentTask) {
        ...
        if (fIndexer == null || ti == null || fIndexer.getProject().getProject() != ti.getProject().getProject()) {
            return false;
        }
