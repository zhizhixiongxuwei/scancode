...
public class WritableCIndex extends CIndex implements IWritableIndex {
    ...
    public boolean fIsWriteLocked;

    public Object fThread;
    ...
    @Override
    public void setFileContent(IIndexFragmentFile file, int linkageID, IncludeInformation[] includes, IASTPreprocessorStatement[] macros, IASTName[][] names, ASTFilePathResolver resolver, YieldableIndexLock lock) throws CoreException, FailedToReAcquireLockException {
    ...
    public void acquireReadLock() throws InterruptedException {
        checkThread();
        //$NON-NLS-1$
        assert !fIsWriteLocked : "Read locks are not allowed while write-locked.";
    ...
    public void acquireWriteLock(IProgressMonitor monitor) throws InterruptedException {
        checkThread();
        //$NON-NLS-1$
        assert !fIsWriteLocked : "Multiple write locks is not allowed";
    ...
    public void releaseWriteLock(boolean flush) {
        checkThread();
        //$NON-NLS-1$
        assert fIsWriteLocked : "No write lock to be released";
    ...
    public void clearResultCache() {
        //$NON-NLS-1$
        assert fIsWriteLocked : "Need to hold a write lock to clear result caches";
