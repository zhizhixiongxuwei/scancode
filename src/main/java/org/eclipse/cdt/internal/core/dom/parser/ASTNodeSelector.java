...
public class ASTNodeSelector implements IASTNodeSelector {
    ...
    final public ASTTranslationUnit fTu;

    final public ILocationResolver fLocationResolver;

    public String fFilePath;

    final public boolean fIsValid;
    ...
    private <T extends IASTNode> T findNode(int offsetInFile, int lengthInFile, Relation relation, Class<T> requiredClass, boolean searchInExpansion) {
        if (!fIsValid) {
            return null;
        }
        if (lengthInFile < 0) {
        ...
        final ASTNodeSpecification<T> nodeSpec = new ASTNodeSpecification<>(relation, requiredClass, offsetInFile, lengthInFile);
