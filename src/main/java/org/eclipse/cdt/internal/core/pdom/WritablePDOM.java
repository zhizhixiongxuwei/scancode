...
public class WritablePDOM extends PDOM implements IWritableIndexFragment {
    ...
    public boolean fClearedBecauseOfVersionMismatch = false;

    public boolean fCreatedFromScratch = false;

    public ASTFilePathResolver fPathResolver;

    public PDOMFile fileBeingUpdated;

    public PDOMFile uncommittedFile;
    ...
    @Override
    public void addFileContent(IIndexFragmentFile sourceFile, IncludeInformation[] includes, IASTPreprocessorStatement[] macros, IASTName[][] names, ASTFilePathResolver pathResolver, YieldableIndexLock lock) throws CoreException, FailedToReAcquireLockException {
    ...
    public void setProperty(String propertyName, String value) throws CoreException {
        if (IIndexFragment.PROPERTY_FRAGMENT_FORMAT_ID.equals(propertyName) || IIndexFragment.PROPERTY_FRAGMENT_FORMAT_VERSION.equals(propertyName)) {
            //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public boolean hasLastingDefinition(PDOMBinding binding) throws CoreException {
        ...
        for (PDOMName name = binding.getFirstDefinition(); name != null; name = name.getNextInBinding()) {
            if (!fileBeingUpdated.getPDOM().equals(name.getPDOM()) || fileBeingUpdated.getRecord() != name.getFileRecord()) {
                return true;
            }
        }
        return false;
    ...
    protected boolean isCommitted(PDOMName name) throws CoreException {
        return uncommittedFile == null || !uncommittedFile.getPDOM().equals(name.getPDOM()) || uncommittedFile.getRecord() != name.getFileRecord();
    }
