...
public class MultiMacroExpansionExplorer extends MacroExpansionExplorer {
    ...
    final public char[] fSource;

    final public int[] fBoundaries;

    final public SingleMacroExpansionExplorer[] fDelegates;

    final public String fFilePath;

    final public Map<IMacroBinding, IASTFileLocation> fMacroLocations;
    ...
    public MultiMacroExpansionExplorer(IASTTranslationUnit tu, IASTFileLocation loc) {
        ...
        for (IASTPreprocessorMacroExpansion expansion : expansions) {
            ...
            if (ref != null) {
                ...
                fBoundaries[++bidx] = to;
                fDelegates[++didx] = new SingleMacroExpansionExplorer(new String(fSource, from, to - from), refs.toArray(new IASTName[refs.size()]), fMacroLocations, fFilePath, refLoc.getStartingLineNumber(), isPPCond, tu.getAdapter(LexerOptions.class));
            }
        }
    ...
    private void addLocations(IASTPreprocessorMacroDefinition[] defs, final Map<IMacroBinding, IASTFileLocation> result) {
