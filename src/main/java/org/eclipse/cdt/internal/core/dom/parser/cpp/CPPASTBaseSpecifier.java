...
public class CPPASTBaseSpecifier extends ASTNode implements ICPPASTBaseSpecifier, ICPPASTCompletionContext {
    ...
    public ICPPASTNameSpecifier nameSpecifier;

    public boolean fIsPackExpansion;
    ...
    public IASTName getName() {
        ...
        "Cannot call getName() on base-specifier whose name-specifier " + //$NON-NLS-1$
        "is not a name. Use getNameSpecifier() instead.");
    }
        ...
        for (int i = 0; i < bindings.length; i++) {
            ...
            if (binding instanceof IType) {
                ...
                while (type instanceof ITypedef || type instanceof ICPPAliasTemplate) {
                    type = type instanceof ITypedef ? ((ITypedef) type).getType() : ((ICPPAliasTemplate) type).getType();
                }
                if (type instanceof ICPPClassType) {
                    int key = ((ICPPClassType) type).getKey();
                    if ((key == ICPPClassType.k_class || key == ICPPClassType.k_struct || type instanceof ICPPDeferredClassInstance || type instanceof ICPPUnknownMemberClass) && (classType == null || !type.isSameType(classType))) {
                        if (i != j)
                            bindings[j] = binding;
                        j++;
                    }
