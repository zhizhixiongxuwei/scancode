...
public class CPPClosureType extends PlatformObject implements ICPPClassType, ICPPInternalBinding {
    ...
    final public ICPPASTLambdaExpression fLambdaExpression;

    public IType[] fParameterTypes;

    public ICPPParameter[] fParameters;

    public ICPPMethod[] fMethods;

    public ClassScope fScope;
    ...
    private ICPPMethod[] createMethods() {
        boolean needConversionOperator = fLambdaExpression.getCaptureDefault() == CaptureDefault.UNSPECIFIED && fLambdaExpression.getCaptures().length == 0;
        ...
        CPPImplicitConstructor ctor = new CPPImplicitConstructor(scope, CharArrayUtils.EMPTY, ICPPParameter.EMPTY_CPPPARAMETER_ARRAY, fLambdaExpression);
        ...
        ft = new CPPFunctionType(returnType, parameterTypes, getNoexceptEvaluation(), !isMutable(), false, false, false, false);
        ...
        if (isGeneric()) {
            m = new CPPImplicitMethodTemplate(getInventedTemplateParameterList(), scope, operatorParensName, ft, params, constExpr) {
        ...
        if (needConversionOperator) {
            ...
            ft = new CPPFunctionType(conversionTarget, IType.EMPTY_TYPE_ARRAY, CPPASTFunctionDeclarator.NOEXCEPT_TRUE, /* (CWG DR 1722) */
            ...
            if (isGeneric()) {
                ...
                m = new CPPImplicitMethodTemplate(templateParamClones, scope, conversionOperatorName, ft, params, constExpr) {
            ...
            for (ICPPASTParameterDeclaration param : params) {
                ...
                if (declSpec instanceof IASTSimpleDeclSpecifier) {
                    if (((IASTSimpleDeclSpecifier) declSpec).getType() == IASTSimpleDeclSpecifier.t_auto) {
                        ...
                        result = ArrayUtil.append(result, new CPPImplicitTemplateTypeParameter(fLambdaExpression, position, isPack));
                        position++;
                    }
                }
            }
    ...
    public int getVisibility(IBinding member) {
        ...
        member.getName() + " is not a member of closure type '" + fLambdaExpression.getRawSignature() + //$NON-NLS-1$
        "'");
    }
