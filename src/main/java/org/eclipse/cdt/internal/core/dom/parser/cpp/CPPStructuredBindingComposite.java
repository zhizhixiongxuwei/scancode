...
public class CPPStructuredBindingComposite extends CPPImplicitVariable {
    ...
    //$NON-NLS-1$
    static final public char[] GET_NAME = "get".toCharArray();

    static final public IStructuredBindingNameEvaluationStrategy INCOMPLETE_INITIALIZER = new IStructuredBindingNameEvaluationStrategy() {
    ...
    public IStructuredBindingNameEvaluationStrategy evaluationStrategy = null;

    public ICPPASTStructuredBindingDeclaration declaration;
    ...
    private class ArrayElement extends IStructuredBindingNameEvaluationStrategy {
        ...
        @Override
        public ICPPEvaluation getEvaluation(int nameIndex) {
            EvalFixed indexEvaluation = new EvalFixed(CPPBasicType.UNSIGNED_INT, ValueCategory.PRVALUE, IntegralValue.create(nameIndex));
    ...
    private class TupleElement extends IStructuredBindingNameEvaluationStrategy {
        ...
        public ICPPEvaluation getEvaluation(int nameIndex) {
            EvalFixed indexEvaluation = new EvalFixed(CPPBasicType.UNSIGNED_INT, ValueCategory.PRVALUE, IntegralValue.create(nameIndex));
            IBinding resolvedFunction = resolveGetFunction(initializerType, initializer, initializerEvaluation, indexEvaluation);
            if (resolvedFunction instanceof ICPPMethod) {
                ICPPEvaluation eExpressionEvaluation = new EvalMemberAccess(initializerType, initializerEvaluation.getValueCategory(), resolvedFunction, initializerEvaluation, false, initializer);
                ...
                EvalBinding functionEvaluation = new EvalBinding(resolvedFunction, ((ICPPFunction) resolvedFunction).getType(), initializer);
                return new EvalFunctionCall(new ICPPEvaluation[] { functionEvaluation, initializerEvaluation }, null, initializer);
            }
            return EvalFixed.INCOMPLETE;
        }
    ...
    private class MemberElement extends IStructuredBindingNameEvaluationStrategy {
        ...
        @Override
        public ICPPEvaluation getEvaluation(int nameIndex) {
            IField[] nonStaticFields = Arrays.stream(ClassTypeHelper.getFields(initializerType)).filter(IS_STATIC_VARIABLE.negate()).toArray(IField[]::new);
            ...
            if (nameIndex >= 0 && nameIndex < nonStaticFields.length) {
                ...
                return new EvalMemberAccess(initializerType, initializerEvaluation.getValueCategory(), boundField, initializerEvaluation, false, initializer);
            }
            return EvalFixed.INCOMPLETE;
        }
    }
