...
public class GNUCSourceParser extends AbstractGNUSourceCodeParser {
    ...
    static final public int DEFAULT_POINTEROPS_LIST_SIZE = 4;

    static final public int DEFAULT_PARAMETERS_LIST_SIZE = 4;

    final public boolean supportGCCStyleDesignators;
    ...
    public GNUCSourceParser(IScanner scanner, ParserMode parserMode, IParserLogService logService, ICParserExtensionConfiguration config) {
        this(scanner, parserMode, logService, config, null);
    }

    public GNUCSourceParser(IScanner scanner, ParserMode parserMode, IParserLogService logService, ICParserExtensionConfiguration config, IIndex index) {
        super(scanner, logService, parserMode, CNodeFactory.getDefault(), config.supportStatementsInExpressions(), config.supportTypeofUnaryExpressions(), config.supportAlignOfUnaryExpression(), config.supportKnRC(), config.supportAttributeSpecifiers(), config.supportDeclspecSpecifiers(), config.getBuiltinBindingsProvider());
    ...
    private IASTDeclaration simpleDeclaration(final DeclarationOptions declOption) throws BacktrackException, EndOfFileException {
        ...
        for (IASTDeclarator declarator : declarators) simpleDeclaration.addDeclarator(declarator);
    ...
    private IASTDeclaration functionDefinition(int firstOffset, IASTDeclSpecifier declSpec, IASTDeclarator[] declarators) throws BacktrackException, EndOfFileException {
    ...
    private IASTExpression expression(final ExprKind kind) throws EndOfFileException, BacktrackException {
        ...
        loop: while (true) {
            ...
            switch(lt1) {
                case IToken.tQUESTION:
                    ...
                    if (LT(2) == IToken.tCOLON) {
                        ...
                        // Consume operator
                        consume();
                        // Next cast expression is just null
                        lastExpression = null;
                        continue;
                    }
                    // assignment expressions will be subsumed by the conditional expression
            ...
            // consume operator
            consume();
            // next cast expression
            lastExpression = castExpression(CastExprCtx.eDirectlyInBExpr, null);
        }
    ...
    protected IASTExpression unaryExpression(CastExprCtx ctx, ITemplateIdStrategy strat) throws EndOfFileException, BacktrackException {
        switch(LT(1)) {
            ...
            case IToken.t_sizeof:
                return parseTypeidInParenthesisOrUnaryExpression(false, consume().getOffset(), IASTTypeIdExpression.op_sizeof, IASTUnaryExpression.op_sizeof, ctx, strat);
            case IToken.t__Alignof:
            case IGCCToken.t___alignof__:
                return parseTypeidInParenthesisOrUnaryExpression(false, consume().getOffset(), IASTTypeIdExpression.op_alignof, IASTUnaryExpression.op_alignOf, ctx, strat);
            default:
        ...
        for (; ; ) {
            switch(LT(1)) {
                case IToken.tLBRACKET:
                    ...
                    ((ASTNode) s).setOffsetAndLength(((ASTNode) firstExpression).getOffset(), last - ((ASTNode) firstExpression).getOffset());
                    firstExpression = s;
                    break;
                case IToken.tLPAREN:
                    // function call
                ...
                case IToken.tINCR:
                    ...
                    firstExpression = buildUnaryExpression(IASTUnaryExpression.op_postFixIncr, firstExpression, ((ASTNode) firstExpression).getOffset(), offset);
                    break;
                case IToken.tDECR:
                    offset = consume().getEndOffset();
                    firstExpression = buildUnaryExpression(IASTUnaryExpression.op_postFixDecr, firstExpression, ((ASTNode) firstExpression).getOffset(), offset);
                    break;
                case IToken.tDOT:
                    // member access
                    IToken dot = consume();
                    ...
                    if (name == null)
                        throwBacktrack(((ASTNode) firstExpression).getOffset(), ((ASTNode) firstExpression).getLength() + dot.getLength());
                    IASTFieldReference result = getNodeFactory().newFieldReference(name, firstExpression);
                    result.setIsPointerDereference(false);
                    ((ASTNode) result).setOffsetAndLength(((ASTNode) firstExpression).getOffset(), calculateEndOffset(name) - ((ASTNode) firstExpression).getOffset());
                ...
                case IToken.tARROW:
                    ...
                    name = identifier();
                    if (name == null)
                        throwBacktrack(((ASTNode) firstExpression).getOffset(), ((ASTNode) firstExpression).getLength() + arrow.getLength());
    ...
    protected IASTExpression primaryExpression(CastExprCtx ctx, ITemplateIdStrategy strat) throws EndOfFileException, BacktrackException {
        ...
        switch(LT(1)) {
            ...
            case IToken.tLPAREN:
                ...
                t = consume();
                // instead of expression(), to keep the stack smaller
                IASTExpression lhs = expression(ExprKind.eExpression);
                int finalOffset = 0;
                switch(LT(1)) {
                    case IToken.tRPAREN:
                    case IToken.tEOC:
     ...
     */
    protected void consumePointerOperators(List<IASTPointerOperator> pointerOps) throws EndOfFileException, BacktrackException {
        for (; ; ) {
    ...
    private final static int INLINE = 0x1, CONST = 0x2, RESTRICT = 0x4, VOLATILE = 0x8, SHORT = 0x10, UNSIGNED = 0x20, SIGNED = 0x40, COMPLEX = 0x80, IMAGINARY = 0x100;

    @Override
    protected Decl declSpecifierSeq(final DeclarationOptions declOption, ITemplateIdStrategy strat) throws BacktrackException, EndOfFileException {
            ...
            declSpecifiers: for (; ; ) {
                ...
                switch(lt1) {
                    ...
                    case IToken.t__Alignas:
                        ...
                        break;
                    case // if __attribute__ is after the declSpec
                    ...
                    case IGCCToken.t_typeof:
                        ...
                        typeofExpression = parseTypeidInParenthesisOrUnaryExpression(false, LA(1).getOffset(), IASTTypeIdExpression.op_typeof, -1, CastExprCtx.eNotInBExpr, null);
                        encounteredTypename = true;
    ...
    private ICASTTypedefNameSpecifier buildNamedTypeSpecifier(IASTName name, int storageClass, int options, int offset, int endOffset) {
    ...
    private ICASTSimpleDeclSpecifier buildSimpleDeclSpec(int storageClass, int simpleType, int options, int isLong, IASTExpression typeofExpression, int offset, int endOffset) {
    ...
    protected ICASTCompositeTypeSpecifier structOrUnionSpecifier() throws BacktrackException, EndOfFileException {
        ...
        switch(LT(1)) {
            ...
            default:
                throwBacktrack(LA(1));
                // line never reached, hint for the parser.
                return null;
        }
    ...
    @Override
    protected IASTDeclarator initDeclarator(IASTDeclSpecifier declspec, final DeclarationOptions option) throws EndOfFileException, BacktrackException, FoundAggregateInitializer {
    ...
    protected IASTDeclarator declarator(IASTDeclSpecifier declSpec, DeclarationOptions option) throws EndOfFileException, BacktrackException {
        ...
        if (lt1 == IToken.tLPAREN) {
            ...
            if (option.fAllowAbstract) {
                ...
                try {
                    cand1 = declarator(pointerOps, attributes, getNodeFactory().newName(), null, startingOffset, endOffset, option);
                    if (option.fRequireAbstract)
                        return cand1;
                    cand1End = LA(1);
                } catch (BacktrackException e) {
            ...
            try {
                ...
                final IASTDeclarator cand2 = declarator(pointerOps, attributes, null, nested, startingOffset, endOffset, option);
    ...
    private IASTDeclarator declarator(final List<IASTPointerOperator> pointerOps, List<IASTAttributeSpecifier> attributes, final IASTName declaratorName, final IASTDeclarator nestedDeclarator, final int startingOffset, int endOffset, final DeclarationOptions option) throws EndOfFileException, BacktrackException {
        ...
        loop: while (true) {
            ...
            switch(lt1) {
                case IToken.tLPAREN:
                    result = functionDeclarator(isAbstract(declaratorName, nestedDeclarator) ? DeclarationOptions.PARAMETER : DeclarationOptions.C_PARAMETER_NON_ABSTRACT);
                ...
                case IToken.tCOLON:
                    ...
                    setDeclaratorID(result, declaratorName, nestedDeclarator);
                    break loop;
                case // if __attribute__ is after a declarator
                IGCCToken.t__attribute__:
                    if (!supportAttributeSpecifiers)
                        throwBacktrack(LA(1));
        ...
        if (lt1 != 0 && LT(1) == IToken.t_asm) {
            // asm labels bug 226121
            consume();
            ...
            for (; ; ) {
                if (LT(1) == IToken.tCOMMA) {
                    ...
                    if (previousWasIdentifier) {
                        backup(mark);
                        // i.e. KnR C won't have int f(typedef x)
                        return 0;
                        // char
                        // x; {}
                    }
                    ...
                    backup(mark);
                    // i.e. KnR C won't have int f(char) char x; {}
                    return 0;
                }
            }
    ...
    private IASTProblemDeclaration createKnRCProblemDeclaration(int offset, int length) throws EndOfFileException {
        ...
        while (LT(1) != IToken.tLBRACE) {
            next = consume();
            if (next == previous || next.getType() == IToken.tEOC) {
                // infinite loop detected
                break;
            }
            previous = next;
    ...
    protected IASTStatement parseIfStatement() throws EndOfFileException, BacktrackException {
        ...
        if_loop: while (true) {
            ...
            if (// shouldn't be possible but failure in condition() makes it so
            condition != null) {
    ...
    protected IASTStatement parseSwitchStatement() throws EndOfFileException, BacktrackException {
        ...
        ((ASTNode) switch_statement).setOffsetAndLength(startOffset, (switch_body != null ? calculateEndOffset(switch_body) : LA(1).getEndOffset()) - startOffset);
    ...
    @Override
    protected IASTTypeId typeIdWithOptionalTrailingEllipsis(DeclarationOptions option) throws EndOfFileException, BacktrackException {
        // No pack expansions in C.
        return typeId(option);
    }

    @Override
    protected IASTAlignmentSpecifier createAmbiguousAlignmentSpecifier(IASTAlignmentSpecifier expression, IASTAlignmentSpecifier typeId) {
        return new CASTAmbiguousAlignmentSpecifier(expression, typeId);
    }

    @Override
    protected boolean maybeDeclaresNonStaticMemberOfSameClass(final DeclarationOptions option, final IASTDeclSpecifier declSpec, final IASTDeclarator dtor) {
        return false;
    }
}
