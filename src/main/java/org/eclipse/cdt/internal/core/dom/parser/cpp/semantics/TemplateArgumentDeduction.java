...
public class TemplateArgumentDeduction {
     ...
     * 14.8.2.1
     */
    static ICPPTemplateArgument[] deduceForFunctionCall(ICPPFunctionTemplate template, ICPPTemplateArgument[] tmplArgs, List<IType> fnArgs, List<ValueCategory> argIsLValue, CPPTemplateParameterMap map) throws DOMException {
     ...
     */
    static boolean deduceFromFunctionArgs(ICPPFunctionTemplate template, List<IType> fnArgs, List<ValueCategory> argCats, CPPTemplateParameterMap map) {
        try {
            ...
            argLoop: for (int j = 0; j < fnArgs.size(); j++) {
                ...
                if (CPPTemplates.isDependentType(par)) {
                    ...
                    // adjustParameterType preserves typedefs
                    ...
                    if (arg instanceof InitializerListType) {
                        ...
                        if (par instanceof IArrayType) {
                            ...
                            IValue as = IntegralValue.create(((InitializerListType) arg).getEvaluation().getClauses().length);
    ...
    private static boolean deduceFromFunctionArg(IType par, IType arg, ValueCategory valueCat, TemplateArgumentDeduction deduct) throws DOMException {
        ...
        if (cvPar == cvArg || (isReferenceTypeParameter && cvPar.isAtLeastAsQualifiedAs(cvArg))) {
            ...
            if (!(pcheck instanceof ICPPTemplateParameter)) {
                ...
                while (pcheck instanceof ITypedef) pcheck = ((ITypedef) pcheck).getType();
    ...
    private static void addReferencedTemplateParameters(IType t, Set<Integer> usedTemplateParIds) {
        while (true) {
            if (t instanceof ICPPTemplateParameter) {
                ...
                for (IType par : ft.getParameterTypes()) addReferencedTemplateParameters(par, usedTemplateParIds);
                t = ft.getReturnType();
     ...
     */
    public static boolean addExplicitArguments(ICPPFunctionTemplate template, final ICPPTemplateParameter[] tmplParams, ICPPTemplateArgument[] tmplArgs, CPPTemplateParameterMap map) {
    ...
    private static ICPPTemplateArgument[] createArguments(CPPTemplateParameterMap map, final ICPPTemplateParameter[] tmplParams) {
    ...
    public static boolean fromTemplateArguments(final ICPPTemplateParameter[] pars, final ICPPTemplateArgument[] p, final ICPPTemplateArgument[] a, CPPTemplateParameterMap map) throws DOMException {
        ...
        for (int j = 0; j < p.length; j++) {
            if (p[j].isPackExpansion()) {
                ...
                if (j != p.length - 1) {
                    // A pack expansion must be the last argument to the specialization.
                    return false;
                }
    ...
    final public CPPTemplateParameterMap fExplicitArgs;

    public CPPTemplateParameterMap fDeducedArgs;

    public Set<Integer> fTemplateParameterPacks;

    public int fPackOffset;

    final public int fPackSize;
    ...
    private boolean fromTemplateArgument(ICPPTemplateArgument p, ICPPTemplateArgument a) throws DOMException {
        ...
        if (p.isNonTypeValue()) {
            ...
            tval = CPPTemplates.instantiateValue(tval, InstantiationContext.forDeduction(fDeducedArgs), IntegralValue.MAX_RECURSION_DEPTH);
            return tval.equals(sval);
        }
        // Try to deduce from the original argument type, but if it fails, fall back to the simplified
        // argument type.
        return fromType(p.getTypeValue(), a.getOriginalTypeValue(), false, true) || (a.getTypeValue() != a.getOriginalTypeValue() && fromType(p.getTypeValue(), a.getTypeValue(), false, true));
    }
    ...
    private boolean fromType(IType p, IType a, boolean allowCVQConversion, boolean verifyNonDeduced) throws DOMException {
        ...
        while (p != null) {
            while (a instanceof ITypedef) a = ((ITypedef) a).getType();
            while (p instanceof ITypedef) p = ((ITypedef) p).getType();
            if (p instanceof IBasicType) {
                ...
                if (a == null)
                    return false;
                return deduce(((ICPPTemplateParameter) p).getParameterID(), new CPPTemplateTypeArgument(a, ExpressionTypes.restoreTypedefs(a, originalArgType)));
                ...
                if (!verifyNonDeduced)
                    // An unknown type may match anything.
                    return true;
                ...
                CPPTemplates.BindingOrType bindingType = CPPTemplates.resolveUnknownBindingOrType((ICPPUnknownBinding) p, context);
