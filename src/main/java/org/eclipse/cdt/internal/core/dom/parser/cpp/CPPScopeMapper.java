...
public class CPPScopeMapper {
    ...
    final public HashMap<IIndexScope, IScope> fMappedScopes = new HashMap<>();

    final public HashMap<String, NamespaceScopeWrapper> fNamespaceWrappers = new HashMap<>();

    final public Map<String, List<UsingDirectiveWrapper>> fPerName = new HashMap<>();

    final public CPPASTTranslationUnit fTu;

    public CharArrayMap<IASTName[]> fClasses;
    ...
    private IScope wrapNamespaceScope(ICPPNamespaceScope scope) {
        try {
            ...
            // index scopes don't throw dom-exceptions
            assert false;
            return null;
        }
    }
