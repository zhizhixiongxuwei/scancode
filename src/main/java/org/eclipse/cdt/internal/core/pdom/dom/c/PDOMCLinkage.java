 ...
 */
public class PDOMCLinkage extends PDOMLinkage implements IIndexCBindingConstants {
    ...
    public ICPPEvaluation unmarshalEvaluation(ITypeMarshalBuffer buffer) throws CoreException {
        ...
        CCorePlugin.createStatus("Cannot unmarshal an evaluation, first byte=" + buffer.getByte()));
    }

    @Override
    public ICPPExecution unmarshalExecution(ITypeMarshalBuffer buffer) throws CoreException {
        throw new CoreException(//$NON-NLS-1$
        CCorePlugin.createStatus("Cannot unmarshal an execution, first byte=" + buffer.getByte()));
    }
}
