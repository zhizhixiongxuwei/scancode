...
public class ExecRangeBasedFor implements ICPPExecution {
    ...
    final public ExecSimpleDeclaration declarationExec;

    final public ICPPEvaluation initClauseEval;

    final public ICPPFunction begin;

    final public ICPPFunction end;

    final public ICPPExecution bodyExec;
    ...
    private static ICPPEvaluation deref(ICPPEvaluation ptr, boolean isRef, ActivationRecord record, ConstexprEvaluationContext context) {
        ICPPEvaluation derefEval = new EvalUnary(ICPPASTUnaryExpression.op_star, ptr, null, ptr.getTemplateDefinition()).computeForFunctionCall(record, context);
        if (isRef) {
            return derefEval;
        } else {
    ...
    private static ICPPEvaluation inc(ActivationRecord record, ICPPEvaluation ptr, ConstexprEvaluationContext context) {
        return new EvalUnary(IASTUnaryExpression.op_prefixIncr, ptr, null, ptr.getTemplateDefinition()).computeForFunctionCall(record, context);
    }
    ...
    private static ICPPEvaluation callFunction(ICPPClassType classType, ICPPFunction func, ICPPEvaluation rangeEval, ActivationRecord record, ConstexprEvaluationContext context) {
        ...
        if (func instanceof ICPPMethod) {
            EvalMemberAccess memberAccess = new EvalMemberAccess(classType, ValueCategory.LVALUE, func, rangeEval, false, templateDefinition);
    ...
    public ICPPExecution instantiate(InstantiationContext context, int maxDepth) {
        ...
        if (newDeclarationExec == declarationExec && newInitClauseEval == initClauseEval && newBegin == begin && newEnd == end && newBodyExec == bodyExec) {
            return this;
        }
