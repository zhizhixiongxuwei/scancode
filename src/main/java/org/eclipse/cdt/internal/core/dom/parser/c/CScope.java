...
public class CScope implements ICScope, IASTInternalScope {
    ...
    static final public IndexFilter[] INDEX_FILTERS = { new // namespace type tag
    IndexFilter() {
    ...
    protected static class CollectNamesAction extends ASTVisitor {
        ...
        public int visit(IASTName n) {
            ...
            if (prop == IASTElaboratedTypeSpecifier.TYPE_NAME || prop == IASTCompositeTypeSpecifier.TYPE_NAME || prop == IASTDeclarator.DECLARATOR_NAME) {
    ...
    public final IBinding[] getBindings(ScopeLookupData lookup) {
        ...
        if (physicalNode instanceof IASTTranslationUnit) {
            ...
            if (index != null) {
                try {
                    IBinding[] bindings = lookup.isPrefixLookup() ? index.findBindingsForContentAssist(lookup.getLookupKey(), true, INDEX_FILTERS[NAMESPACE_TYPE_BOTH], null) : index.findBindings(lookup.getLookupKey(), INDEX_FILTERS[NAMESPACE_TYPE_BOTH], null);
        ...
        if (nodes != null) {
            ...
            while (node != null) {
                ...
                if (idx > -1 && ++idx < nodes.length) {
                    ...
                    node = null;
                    if (nodes[0].getPropertyInParent() == ICASTKnRFunctionDeclarator.FUNCTION_PARAMETER || nodes[0].getPropertyInParent() == IASTStandardFunctionDeclarator.FUNCTION_PARAMETER) {
                        ...
                        if (scopeNode instanceof IASTCompoundStatement) {
                            ...
                            while (n instanceof IASTDeclarator) n = n.getParent();
