...
public abstract class MultiResourceDescription extends MultiItemsHolder implements ICMultiResourceDescription {
    ...
    public ICConfigurationDescription fCfg = null;
    ...
    public boolean canExclude(boolean exclude) {
        for (int i = 0; i < fRess.length; i++) if (!fRess[i].canExclude(exclude))
            return false;
        return true;
    }
    ...
    public boolean isExcluded() {
        for (int i = 0; i < fRess.length; i++) if (!fRess[i].isExcluded())
            return false;
        return true;
    }
    ...
    public void setExcluded(boolean excluded) throws WriteAccessException {
        for (int i = 0; i < fRess.length; i++) fRess[i].setExcluded(excluded);
    }

    @Override
    public void setPath(IPath path) throws WriteAccessException {
        for (int i = 0; i < fRess.length; i++) fRess[i].setPath(path);
    }
    ...
    public ICConfigurationDescription getConfiguration() {
        if (fCfg == null) {
            ...
            for (int i = 0; i < fRess.length; i++) cfgs[i] = fRess[i].getConfiguration();
    ...
    public ICSettingContainer getParent() {
        ...
        if (sc == null)
            return null;
        for (int i = 1; i < fRess.length; i++) if (!sc.equals(fRess[i].getParent()))
            return null;
        return sc;
    }
    ...
    public int getType() {
        ...
        for (int i = 1; i < fRess.length; i++) if (t != fRess[i].getType())
            return 0;
        return t;
    }
    ...
    public boolean isReadOnly() {
        for (int i = 0; i < fRess.length; i++) if (!fRess[i].isReadOnly())
            return false;
        return true;
    }
    ...
    public boolean isValid() {
        for (int i = 0; i < fRess.length; i++) if (!fRess[i].isValid())
            return false;
        return true;
    }
