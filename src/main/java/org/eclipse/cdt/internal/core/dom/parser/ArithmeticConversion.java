...
public abstract class ArithmeticConversion {
    ...
    static final public int DOMAIN_FLAGS = IBasicType.IS_IMAGINARY | IBasicType.IS_COMPLEX;
    ...
    private boolean isLongDouble(IType type) {
        if (type instanceof IBasicType) {
            final IBasicType bt = (IBasicType) type;
            return bt.isLong() && bt.getKind() == Kind.eDouble || bt.getKind() == Kind.eFloat128 || bt.getKind() == Kind.eDecimal128;
        }
        return false;
    }
