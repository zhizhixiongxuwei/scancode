...
public abstract class AbstractIndexerTask extends PDOMWriter {
    ...
    static final public int MAX_ERRORS = 500;
    ...
    //$NON-NLS-1$
    static final public Pattern HEADERNAME_PATTERN = Pattern.compile("@headername\\{(?<header>[^\\}]+)\\}");
    ...
    public int fUpdateFlags = IIndexManager.UPDATE_ALL;

    public UnusedHeaderStrategy fIndexHeadersWithoutContext = UnusedHeaderStrategy.useDefaultLanguage;

    public boolean fIndexFilesWithoutConfiguration = true;
    ...
    public synchronized boolean acceptUrgentTask(IPDOMIndexerTask urgentTask) {
        ...
        if (task.fIsFastIndexer != fIsFastIndexer || task.fIndexFilesWithoutConfiguration != fIndexFilesWithoutConfiguration || (fIndexFilesWithoutConfiguration && task.fIndexHeadersWithoutContext != fIndexHeadersWithoutContext) || fTaskCompleted) {
            // Reject the urgent work since this task is not capable of doing it, or it's too late.
            return false;
        }
        if (task.fFilesToUpdate.length > (fFilesToUpdate != null ? fFilesToUpdate.length : getProgressInformation().fRequestedFilesCount)) {
    ...
    public final void runTask(IProgressMonitor monitor) throws InterruptedException {
        try {
            ...
            fASTOptions = ILanguage.OPTION_NO_IMAGE_LOCATIONS | ILanguage.OPTION_SKIP_TRIVIAL_EXPRESSIONS_IN_AGGREGATE_INITIALIZERS;
    ...
    private void extractFiles(HashMap<Integer, List<IIndexFileLocation>> files, List<IIndexFragmentFile> filesToRemove, IProgressMonitor monitor) throws CoreException {
        ...
        for (final Object tu : fFilesToUpdate) {
            ...
            if (regularContent || indexedUnconditionally) {
                ...
                final UpdateKind updateKind = isSourceUnit ? UpdateKind.REQUIRED_SOURCE : regularContent && both ? UpdateKind.REQUIRED_HEADER : UpdateKind.ONE_LINKAGE_HEADER;
                    ...
                    for (AbstractLanguage lang : langs) {
                        ...
                        for (int i = 0; i < indexFiles.length; i++) {
                            ...
                            if (ifile != null && ifile.getLinkageID() == linkageID && ifile.hasContent()) {
                                ...
                                // Take the file.
                                indexFiles[i] = null;
                                boolean update = force || (forceUnresolvedIncludes && ifile.hasUnresolvedInclude()) || isModified(checkTimestamps, checkFileContentsHash, ifl, tu, ifile);
    ...
    private boolean isModified(boolean checkTimestamps, boolean checkFileContentsHash, IIndexFileLocation ifl, Object tu, IIndexFragmentFile file) throws CoreException {
        if (checkTimestamps) {
            if (fResolver.getLastModified(ifl) != file.getTimestamp() || computeFileSizeAndEncodingHashcode(ifl) != file.getSizeAndEncodingHashcode()) {
                ...
                if (locTask == null || locTask.isCompleted()) {
                    ...
                    for (FileVersionTask versionTask : locTask.fVersionTasks) {
                        if (versionTask.fOutdated) {
                            if (hasUrgentTasks())
                                return;
                            parseVersionInContext(linkageID, map, ifl, versionTask, locTask.fTu, new LinkedHashSet<IIndexFile>(), progress.split(1));
                        }
                    }
                }
            }
    ...
    private void parseVersionInContext(int linkageID, LinkageTask map, IIndexFileLocation ifl, final FileVersionTask versionTask, Object tu, LinkedHashSet<IIndexFile> safeGuard, IProgressMonitor monitor) throws CoreException, InterruptedException {
            ...
            while (!done) {
                ...
                if (d != null) {
                    ...
                    if (dependencies.add(d.fIndexFile)) {
                        if (parseFile(d.fTu, language, d.fIndexFile.getLocation(), scannerInfo, new FileContext(ctxFile, d.fIndexFile), progress.split(1)) == null) {
                            done = false;
                        }
                    }
                }
    ...
    private IIndexFragmentFile findContextFile(int linkageID, LinkageTask map, final FileVersionTask versionTask, LinkedHashSet<IIndexFile> safeGuard, IProgressMonitor monitor) throws CoreException, InterruptedException {
        ...
        while (true) {
            ...
            if (ctxTask != null) {
                ...
                if (ctxVersionTask != null && ctxVersionTask.fOutdated) {
                    ...
                    ctxVersionTask.fOutdated || !versionTask.fOutdated) {
                        // Our file was parsed.
                        return null;
                    }
    ...
    private DependsOnOutdatedFileException parseFile(Object tu, AbstractLanguage lang, IIndexFileLocation ifl, IScannerInfo scanInfo, FileContext ctx, IProgressMonitor monitor) throws CoreException, InterruptedException {
        ...
        try {
            if (fShowActivity) {
                //$NON-NLS-1$
                trace("Indexer: parsing " + path.toOSString());
            }
            progress.subTask(getMessage(MessageKind.parsingFileTask, path.lastSegment(), path.removeLastSegments(1).toString()));
            FileContent codeReader = fResolver.getCodeReader(tu);
            ...
            if (ast == null) {
                ...
                // The cache was cleared while writing to the index.
                resultCacheCleared = true;
            }
            if (fShowActivity) {
                long time = System.currentTimeMillis() - start;
                //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                trace("Indexer: processed " + path.toOSString() + " [" + time + " ms]");
            }
    ...
    private final IASTTranslationUnit createAST(AbstractLanguage language, FileContent codeReader, IScannerInfo scanInfo, int options, FileContext ctx, IProgressMonitor monitor) throws CoreException {
        if (codeReader == null) {
            return null;
        }
        if (fTranslationUnitSizeLimit > 0 && fResolver.getFileSize(codeReader.getFileLocation()) > fTranslationUnitSizeLimit) {
            if (fShowActivity) {
                //$NON-NLS-1$
                trace("Indexer: Skipping large file " + codeReader.getFileLocation());
            }
            return null;
        }
        ...
        if (fCodeReaderFactory == null) {
            ...
            if (fIsFastIndexer) {
                IndexBasedFileContentProvider ibfcp = new IndexBasedFileContentProvider(fIndex, fResolver, language.getLinkageID(), fileContentProvider, this);
    ...
    private void writeToIndex(final int linkageID, IASTTranslationUnit ast, FileContent codeReader, FileContext ctx, IProgressMonitor monitor) throws CoreException, InterruptedException {
        ...
        ISignificantMacros significantMacros = ast.isHeaderUnit() ? ast.getSignificantMacros() : ISignificantMacros.NONE;
    ...
    private void collectOrderedFileKeys(final int linkageID, IASTInclusionNode inclusion, Set<FileContentKey> enteredFiles, List<FileInAST> orderedFileKeys) throws CoreException {
    ...
    private Map<String, String> extractReplacementHeaders(IASTComment[] comments, IProgressMonitor monitor) {
        ...
        for (int i = 0; i < comments.length; i++) {
            ...
            if (commentChars.length <= 2)
                continue;
            if (carryoverLocation == null || !location.getFileName().equals(carryoverLocation.getFileName()) || location.getStartingLineNumber() != carryoverLocation.getEndingLineNumber() + 1) {
            ...
            if (matcher.find()) {
                ...
                if (header == null) {
                    ...
                    header = header.replace(" or ", ",").replace(" ", "");
                }
            ...
            if (fPragmaPrivatePattern != null) {
                ...
                if (matcher.find()) {
                    if (!isWhitespace(text, 0, matcher.start()))
                        // Extraneous text before the pragma.
                        continue;
    ...
    public final IndexFileContent getFileContent(int linkageID, IIndexFileLocation ifl, IIndexFile file) throws CoreException, DependsOnOutdatedFileException {
        LinkageTask map = findRequestMap(linkageID);
        if (map != null) {
