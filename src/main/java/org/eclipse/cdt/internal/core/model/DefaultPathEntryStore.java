...
public class DefaultPathEntryStore implements IPathEntryStore, ICDescriptorListener {
    ...
    //$NON-NLS-1$
    static public String PATH_ENTRY = "pathentry";

    //$NON-NLS-1$
    static public String PATH_ENTRY_ID = "org.eclipse.cdt.core.pathentry";

    //$NON-NLS-1$
    static public String ATTRIBUTE_KIND = "kind";

    //$NON-NLS-1$
    static public String ATTRIBUTE_PATH = "path";

    //$NON-NLS-1$
    static public String ATTRIBUTE_BASE_PATH = "base-path";
    ...
    public IPathEntry[] getRawPathEntries() throws CoreException {
        ...
        if (cdesc != null) {
            ...
            for (ICStorageElement childNode : entry.getChildrenByName(PATH_ENTRY)) pathEntries.add(decodePathEntry(fProject, childNode));
    ...
    static IPathEntry decodePathEntry(IProject project, ICStorageElement element) throws CModelException {
        ...
        element.hasAttribute(ATTRIBUTE_BASE_PATH) ? element.getAttribute(ATTRIBUTE_BASE_PATH) : "");
        // get the base ref
        IPath baseRef = new Path(//$NON-NLS-1$
        element.hasAttribute(ATTRIBUTE_BASE_REF) ? element.getAttribute(ATTRIBUTE_BASE_REF) : "");
        ...
        switch(kind) {
            ...
            case IPathEntry.CDT_LIBRARY:
                {
                    ...
                    IPath sourceAttachmentPath = element.hasAttribute(ATTRIBUTE_SOURCEPATH) ? new Path(element.getAttribute(ATTRIBUTE_SOURCEPATH)) : null;
                    IPath sourceAttachmentRootPath = element.hasAttribute(ATTRIBUTE_ROOTPATH) ? new Path(element.getAttribute(ATTRIBUTE_ROOTPATH)) : null;
                    IPath sourceAttachmentPrefixMapping = element.hasAttribute(ATTRIBUTE_PREFIXMAPPING) ? new Path(element.getAttribute(ATTRIBUTE_PREFIXMAPPING)) : null;
                    if (!baseRef.isEmpty()) {
                        return CoreModel.newLibraryRefEntry(path, baseRef, libraryPath);
                    }
                    return CoreModel.newLibraryEntry(path, basePath, libraryPath, sourceAttachmentPath, sourceAttachmentRootPath, sourceAttachmentPrefixMapping, isExported);
                }
            ...
            default:
                {
                    //$NON-NLS-1$ //$NON-NLS-2$
                    ICModelStatus status = new CModelStatus(IStatus.ERROR, "PathEntry: unknown kind (" + kindAttr + ")");
    ...
    public void descriptorChanged(CDescriptorEvent event) {
        if (event.getType() == CDescriptorEvent.CDTPROJECT_CHANGED) /*|| event.getType() == CDescriptorEvent.CDTPROJECT_ADDED*/
        {
