...
public class PDOMTagSynchronizer implements IBTreeVisitor {
    ...
    final public Long searchRecord;

    final public Map<String, ITag> newTags;

    final public List<Long> toRemove = new ArrayList<>();

    final public List<Long> toInsert = new ArrayList<>();
    ...
    public boolean visit(long existing_record) throws CoreException {
        ...
        if (newTag == null) {
            ...
            "Unable to modify data of tag record " + existing_record + " from taggerId " + //$NON-NLS-1$
            taggerId);
