...
public class ASTCache {
     ...
     */
    static final public boolean DEBUG = Boolean.parseBoolean(//$NON-NLS-1$
    Platform.getDebugOption("org.eclipse.cdt.core/debug/ASTCache"));

    //$NON-NLS-1$
    static final public String DEBUG_PREFIX = "[ASTCache] ";

    /**
     * Fast parse mode (use PDOM)
     */
    public static int PARSE_MODE = ITranslationUnit.AST_SKIP_ALL_HEADERS | ITranslationUnit.AST_CONFIGURE_USING_SOURCE_CONTEXT | ITranslationUnit.AST_SKIP_TRIVIAL_EXPRESSIONS_IN_AGGREGATE_INITIALIZERS | ITranslationUnit.AST_PARSE_INACTIVE_CODE;
    ...
    final public int fParseMode;

    final public Object fCacheMutex = new Object();
    ...
    private IASTTranslationUnit getAST(ITranslationUnit tUnit, IIndex index, boolean wait, IProgressMonitor progressMonitor) {
        ...
        while (true) {
            ...
            synchronized (fCacheMutex) {
                ...
                if (isActiveElement) {
                    if (fAST != null) {
                        ...
                        if (fLastWriteOnIndex < index.getLastWriteAccess()) {
                            ...
                            if (DEBUG)
                                ...
                                DEBUG_PREFIX + getThreadName() + "returning cached AST:" + toString(fAST) + " for: " + //$NON-NLS-1$
                    ...
                    if (!wait) {
                        ...
                        if (DEBUG)
                            System.out.println(//$NON-NLS-1$
                            DEBUG_PREFIX + getThreadName() + "returning null (WAIT_NO) for: " + tUnit.getElementName());
                        return null;
                    }
                }
                ...
                if (isActiveElement && isReconciling(tUnit)) {
                    try {
                        ...
                        if (DEBUG)
                            System.out.println(//$NON-NLS-1$
                            DEBUG_PREFIX + getThreadName() + "waiting for AST for: " + tUnit.getElementName());
                        fCacheMutex.wait();
                        // Check whether active element is still valid
                        if (fAST != null) {
                            if (DEBUG)
                                System.out.println(//$NON-NLS-1$
                                DEBUG_PREFIX + getThreadName() + "...got AST for: " + tUnit.getElementName());
                            return fAST;
                        }
                        // try again
                        continue;
                    } catch (InterruptedException e) {
                        // thread has been interrupted don't compute AST
                        return null;
                    }
                } else if (!wait) {
                    return null;
            ...
            if (DEBUG)
                //$NON-NLS-1$
                System.err.println(DEBUG_PREFIX + getThreadName() + "creating AST for " + tUnit.getElementName());
            ...
            try {
                ...
                if (progressMonitor != null && progressMonitor.isCanceled())
                    ...
                    //$NON-NLS-1$
                    System.err.println(DEBUG_PREFIX + getThreadName() + "created AST for: " + tUnit.getElementName());
            } finally {
                if (isActiveElement) {
                    if (fAST != null) {
                        if (DEBUG)
                            System.out.println(//$NON-NLS-1$
                            DEBUG_PREFIX + getThreadName() + "Ignore created AST for " + tUnit.getElementName() + //$NON-NLS-1$
                            "- AST from reconciler is newer");
    ...
    public IStatus runOnAST(ITranslationUnit tUnit, boolean wait, IProgressMonitor monitor, ASTRunnable astRunnable) {
        IIndex index;
        try {
            index = CCorePlugin.getIndexManager().getIndex(tUnit.getCProject(), IIndexManager.ADD_EXTENSION_FRAGMENTS_EDITOR);
    ...
    private void cache(IASTTranslationUnit ast, ITranslationUnit tUnit) {
        ...
        if (fActiveTU != null && !fActiveTU.equals(tUnit)) {
            ...
            DEBUG && tUnit != null)
                //$NON-NLS-1$
                System.out.println(DEBUG_PREFIX + getThreadName() + "don't cache AST for inactive: " + toString(tUnit));
            return;
        }
        if (// don't report call from disposeAST()
        DEBUG && (tUnit != null || ast != null))
            System.out.println(//$NON-NLS-1$ //$NON-NLS-2$
            DEBUG_PREFIX + getThreadName() + "caching AST: " + toString(ast) + " for: " + toString(tUnit));
        if (fAST != null)
            disposeAST();
        fAST = ast;
    ...
    public void disposeAST() {
        synchronized (fCacheMutex) {
            ...
            if (DEBUG)
                System.out.println(//$NON-NLS-1$//$NON-NLS-2$
                DEBUG_PREFIX + getThreadName() + "disposing AST: " + toString(fAST) + " for: " + toString(fActiveTU));
            fAST = null;
            cache(null, null);
        }
    }
    ...
    public IASTTranslationUnit createAST(final ITranslationUnit tUnit, final IIndex index, final IProgressMonitor progressMonitor) {
        ...
        SafeRunner.run(new ISafeRunnable() {
            ...
            public void handleException(Throwable ex) {
                IStatus status = new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, IStatus.OK, "Error in CDT Core during AST creation", //$NON-NLS-1$
                ex);
    ...
    public void setActiveElement(ITranslationUnit tUnit) {
        ...
        if (DEBUG)
            //$NON-NLS-1$
            System.out.println(DEBUG_PREFIX + getThreadName() + "active element is: " + toString(tUnit));
    }
    ...
    public void aboutToBeReconciled(ITranslationUnit tUnit) {
        ...
        synchronized (fCacheMutex) {
            ...
            if (DEBUG)
                //$NON-NLS-1$
                System.out.println(DEBUG_PREFIX + getThreadName() + "about to reconcile: " + toString(tUnit));
            fIsReconciling = true;
            cache(null, tUnit);
        }
    }
    ...
    public void reconciled(IASTTranslationUnit ast, ITranslationUnit tUnit) {
        synchronized (fCacheMutex) {
            ...
            if (DEBUG)
                System.out.println(//$NON-NLS-1$ //$NON-NLS-2$
                DEBUG_PREFIX + getThreadName() + "reconciled: " + toString(tUnit) + ", AST: " + toString(ast));
            fIsReconciling = false;
            cache(ast, tUnit);
        }
    }
