...
public class CASTDeclarator extends ASTAttributeOwner implements IASTDeclarator, IASTAmbiguityParent {
    ...
    public IASTDeclarator nestedDeclarator;

    public IASTPointerOperator[] pointerOps;

    public int pointerOpsPos = -1;
    ...
    public boolean accept(ASTVisitor action) {
        ...
        if (getPropertyInParent() != IASTTypeId.ABSTRACT_DECLARATOR && nestedDeclarator == null) {
            if (getParent() instanceof IASTDeclarator) {
                ...
                while (outermostDeclarator.getParent() instanceof IASTDeclarator) outermostDeclarator = (IASTDeclarator) outermostDeclarator.getParent();
                if (outermostDeclarator.getPropertyInParent() != IASTTypeId.ABSTRACT_DECLARATOR && name != null && !name.accept(action)) {
                    return false;
                }
    ...
    public int getRoleForName(IASTName n) {
        if (n == this.name) {
            ...
            if (getParent instanceof IASTDeclarator) {
                ...
                while (t instanceof IASTDeclarator) t = t.getParent();
