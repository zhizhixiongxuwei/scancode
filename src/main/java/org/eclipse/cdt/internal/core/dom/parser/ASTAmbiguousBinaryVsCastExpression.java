...
public abstract class ASTAmbiguousBinaryVsCastExpression extends ASTAmbiguousNode implements IASTAmbiguousExpression {
    ...
    final public IASTBinaryExpression fBinaryExpression;

    final public IASTCastExpression fCastExpression;
    ...
    private IASTExpression joinExpressions(IASTExpression l, IASTExpression middle, IASTBinaryExpression r) {
        while (true) {
            if (l == null) {
                ...
                setEnd(l, middle);
                // middle becomes cast-expr, can be put into r (a binary-expr)
                middle = l;
                ...
                setEnd(l, middle);
                // middle becomes unary-expr, can be put into r (a binary-expr)
                middle = l;
                l = (IASTExpression) l.getParent();
            } else {
                if (r == null || getPrecendence((IASTBinaryExpression) l) >= getPrecendence(r)) {
                    ...
                    setEnd(l, middle);
                    // middle becomes binary, can be put into r because precedence is greater or equal.
                    middle = l;
                    l = (IASTExpression) l.getParent();
                } else {
                    r.setOperand1(middle);
                    setStart(r, middle);
                    // middle becomes binary, can be put into r because precedence is greater.
                    middle = r;
                    r = (IASTBinaryExpression) r.getParent();
                }
            }
