...
public class AccessContext {
    ...
    static final public int v_private = ICPPMember.v_private;

    static final public int v_protected = ICPPMember.v_protected;
    ...
    // Depends on the binding for which we check the access.
    ...
    public boolean isAccessible(IBinding binding, int bindingVisibility) {
        ...
        if (!(owner instanceof ICPPClassType)) {
            // The binding is not a class member.
            return true;
        }
        if (!initialize()) {
            // Assume visibility if anything goes wrong.
            return true;
        }
    ...
    private boolean isAccessible(IBinding binding, int bindingVisibility, ICPPClassType owner, ICPPClassType derivedClass, int accessLevel, int depth) {
        ...
        if (bases != null) {
            for (ICPPBase base : bases) {
                ...
                if (isAccessible(binding, bindingVisibility, owner, (ICPPClassType) baseBinding, accessLevel == v_private ? v_protected : accessLevel, depth + 1)) {
                    return true;
                }
            }
        }
    ...
    private ICPPClassType getNamingClass(ICPPClassType accessOwner) {
        ...
        if (classType != null && isUnqualifiedLookup) {
            ...
            while (owner instanceof ICPPClassType && !derivesFrom(classType, accessOwner, name, CPPSemantics.MAX_INHERITANCE_DEPTH)) {
    ...
    private static IBinding[] getContext(IASTName name) {
        ...
        for (IBinding binding = CPPVisitor.findEnclosingFunctionOrClass(name); binding != null; binding = binding.getOwner()) {
            if (binding instanceof ICPPMethod || // Definition of an undeclared method.
            binding instanceof IProblemBinding && ((IProblemBinding) binding).getID() == IProblemBinding.SEMANTIC_MEMBER_DECLARATION_NOT_FOUND) {
                continue;
            }
