...
public class AbstractCPPClassSpecializationScope implements ICPPClassSpecializationScope {
    ...
    final public ICPPClassSpecialization specialClass;

    // The following fields are used by the PDOM bindings and need to be volatile.
    volatile public ICPPBase[] fBases;

    volatile public ICPPMethod[] ownInheritedConstructors;

    final public ThreadLocal<Boolean> fComputingBases = new ThreadLocal<>() {
    ...
    public IBinding getBinding(IASTName name, boolean resolve, IIndexFileSet fileSet) {
        ...
        if (CharArrayUtils.equals(c, specialClass.getNameCharArray()) && !CPPClassScope.shallReturnConstructors(name, false)) {
            return specialClass;
        }
    ...
    final public IBinding[] getBindings(ScopeLookupData lookup) {
        ...
        int n = 0;
        for (IBinding binding : bindings) {
            if (binding == specialized || (binding instanceof ICPPClassType && areSameTypesModuloPartialSpecialization(specialized, (IType) binding))) {
                ...
                if (bases.length == 0) {
                    ...
                    for (ICPPBase base : bases) {
                        ...
                        if (baseType instanceof ICPPParameterPackType) {
                            IType[] specClasses = CPPTemplates.instantiateTypes(new IType[] { baseType }, new InstantiationContext(tpmap, specialClass));
    ...
    private ICPPMethod[] getOwnInheritedConstructors(ICPPConstructor[] existingConstructors) {
        if (ownInheritedConstructors == null) {
            ...
            ICPPMethod[] constructors = ClassTypeHelper.getInheritedConstructors(this, getBases(), existingConstructorParamTypes);
