...
public class CVisitor extends ASTQueries {
    ...
    static final public CBasicType UNSIGNED_LONG_INT = new CBasicType(Kind.eInt, IBasicType.IS_LONG | IBasicType.IS_UNSIGNED);
    ...
    public static class CollectProblemsAction extends ASTVisitor {
        ...
        private void addProblem(IASTProblem problem) {
            if (problems.length == numFound) {
                // if the found array is full, then double the array
                IASTProblem[] old = problems;
                problems = new IASTProblem[old.length * 2];
                for (int j = 0; j < old.length; ++j) problems[j] = old[j];
            }
            problems[numFound++] = problem;
        }

        private IASTProblem[] removeNullFromProblems() {
            if (problems[problems.length - 1] != null) {
                // if the last element in the list is not null then return the list
                return problems;
            } else if (problems[0] == null) {
                // if the first element in the list is null, then return empty list
                return new IASTProblem[0];
            }
            IASTProblem[] results = new IASTProblem[numFound];
            for (int i = 0; i < results.length; i++) results[i] = problems[i];
            return results;
        }
    ...
    public static class CollectDeclarationsAction extends ASTVisitor {
        ...
        private void addName(IASTName name) {
            if (declsFound.length == numFound) {
                // if the found array is full, then double the array
                IASTName[] old = declsFound;
                declsFound = new IASTName[old.length * 2];
                for (int j = 0; j < old.length; ++j) declsFound[j] = old[j];
            }
            declsFound[numFound++] = name;
        }

        private IASTName[] removeNullFromNames() {
            if (declsFound[declsFound.length - 1] != null) {
                // if the last element in the list is not null then return the list
                return declsFound;
            } else if (declsFound[0] == null) {
                // if the first element in the list is null, then return empty list
                return new IASTName[0];
            }
            IASTName[] results = new IASTName[numFound];
            for (int i = 0; i < results.length; i++) results[i] = declsFound[i];
            return results;
        }
        ...
        public int visit(IASTDeclarator declarator) {
            ...
            while (parent != null && !(parent instanceof IASTDeclaration || parent instanceof IASTParameterDeclaration)) parent = parent.getParent();
    ...
    public static class CollectReferencesAction extends ASTVisitor {
        ...
        public CollectReferencesAction(IBinding binding) {
            ...
            if (binding instanceof ILabel) {
                kind = KIND_LABEL;
            } else if (binding instanceof ICompositeType || binding instanceof ITypedef || binding instanceof IEnumeration) {
                kind = KIND_TYPE;
            } else {
                kind = KIND_OBJ_FN;
            }
            ...
            switch(kind) {
                ...
                case KIND_TYPE:
                    if (prop == IASTNamedTypeSpecifier.NAME) {
                        ...
                        if (!(p instanceof IASTSimpleDeclaration) || ((IASTSimpleDeclaration) p).getDeclarators().length > 0) {
                            break;
                        }
                    }
                    return PROCESS_CONTINUE;
                case KIND_OBJ_FN:
                    if (prop == IASTIdExpression.ID_NAME || prop == IASTFieldReference.FIELD_NAME || prop == ICASTFieldDesignator.FIELD_NAME) {
                        break;
                    }
                    return PROCESS_CONTINUE;
            }
    ...
    static final public ASTNodeProperty STRING_LOOKUP_PROPERTY = new ASTNodeProperty(//$NON-NLS-1$
    "CVisitor.STRING_LOOKUP_PROPERTY - STRING_LOOKUP");

    static final public ASTNodeProperty STRING_LOOKUP_TAGS_PROPERTY = new ASTNodeProperty(//$NON-NLS-1$
    "CVisitor.STRING_LOOKUP_TAGS_PROPERTY - STRING_LOOKUP");

    //$NON-NLS-1$
    static final public String SIZE_T = "size_t";
    ...
    private static IBinding createBinding(ICASTElaboratedTypeSpecifier elabTypeSpec) {
        ...
        if (parent instanceof IASTDeclaration) {
            ...
            if (parent instanceof IASTSimpleDeclaration && ((IASTSimpleDeclaration) parent).getDeclarators().length == 0) {
                ...
                try {
                    while (scope instanceof ICCompositeTypeScope) scope = scope.getParent();
                    binding = scope.getBinding(name, false);
                } catch (DOMException e) {
                }
    ...
    static IType getPtrDiffType(IASTBinaryExpression expr) {
        ...
        for (IBinding b : bs) {
            if (b instanceof IType) {
                if (!(b instanceof ICInternalBinding) || CVisitor.declaredBefore(((ICInternalBinding) b).getPhysicalNode(), expr)) {
                    return (IType) b;
                }
            }
        }
    ...
    private static IBinding createBinding(IASTDeclarator declarator, IASTName name) {
        ...
        if (declarator instanceof ICASTKnRFunctionDeclarator) {
            if (CharArrayUtils.equals(declarator.getName().toCharArray(), name.toCharArray())) {
                ...
                // createBinding for one of the ICASTKnRFunctionDeclarator's parameterNames
                    ...
                    if (CVisitor.unwrapTypedefs(t1) instanceof IFunctionType) {
                        ...
                        if (binding != null && !(binding instanceof IIndexBinding) && name.isActive()) {
                            if (binding instanceof IParameter) {
                                ...
                                if (t1 != null && t2 != null && (areArraysOfTheSameElementType(t1, t2) || t1.isSameType(t2))) {
    ...
    protected static IBinding resolveBinding(IASTNode node) {
        if (node instanceof IASTFunctionDefinition) {
            ...
            if (binding instanceof IType && !(binding instanceof IProblemBinding)) {
                return new ProblemBinding(node, IProblemBinding.SEMANTIC_INVALID_TYPE, binding.getNameCharArray(), new IBinding[] { binding });
            }
            return binding;
            ...
            if ((blockItem instanceof IASTSimpleDeclaration || (blockItem instanceof IASTDeclarationStatement && ((IASTDeclarationStatement) blockItem).getDeclaration() instanceof IASTSimpleDeclaration))) {
    ...
    private static IASTNode getContainingBlockItem(IASTNode node) {
        for (IASTNode parent = node.getParent(); parent != null; parent = parent.getParent()) {
            ...
            parent instanceof IASTCompoundStatement || parent instanceof IASTTranslationUnit || parent instanceof IASTForStatement || parent instanceof IASTFunctionDeclarator) {
                return node;
            }
            node = parent;
        }
    ...
    public static IType createType(IASTDeclarator declarator, int flags) {
        ...
        if (isParameter) {
            ...
            if (paramType instanceof IArrayType) {
                // the index does not yet return ICArrayTypes
                ...
                for (IASTAttribute attribute : attributes) {
                    ...
                    if (CharArrayUtils.equals(name, "__mode__") || CharArrayUtils.equals(name, "mode")) {
                        ...
                        if (CharArrayUtils.equals(mode, "__QI__") || CharArrayUtils.equals(mode, "QI")) {
                            //$NON-NLS-1$ //$NON-NLS-2$
                            type = new CBasicType(IBasicType.Kind.eChar, basicType.isUnsigned() ? IBasicType.IS_UNSIGNED : IBasicType.IS_SIGNED);
                        } else if (CharArrayUtils.equals(mode, "__HI__") || CharArrayUtils.equals(mode, "HI")) {
                            //$NON-NLS-1$ //$NON-NLS-2$
                            type = new CBasicType(IBasicType.Kind.eInt, IBasicType.IS_SHORT | getSignModifiers(basicType));
                            ...
                            //$NON-NLS-1$ //$NON-NLS-2$
                            type = new CBasicType(IBasicType.Kind.eInt, IBasicType.IS_LONG | getSignModifiers(basicType));
                        }
                    }
                }
            }
    ...
    private static IType setupPointerChain(IASTPointerOperator[] ptrs, IType lastType) {
        ...
        if (ptrs != null && ptrs.length > 0) {
            ...
            if (ptrs.length == 1) {
                ...
                pointerType.setQualifiers((((ICASTPointer) ptrs[0]).isConst() ? CPointerType.IS_CONST : 0) | (((ICASTPointer) ptrs[0]).isRestrict() ? CPointerType.IS_RESTRICT : 0) | (((ICASTPointer) ptrs[0]).isVolatile() ? CPointerType.IS_VOLATILE : 0));
            } else {
                CPointerType tempType = new CPointerType();
                pointerType.setType(tempType);
                pointerType.setQualifiers((((ICASTPointer) ptrs[ptrs.length - 1]).isConst() ? CPointerType.IS_CONST : 0) | (((ICASTPointer) ptrs[ptrs.length - 1]).isRestrict() ? CPointerType.IS_RESTRICT : 0) | (((ICASTPointer) ptrs[ptrs.length - 1]).isVolatile() ? CPointerType.IS_VOLATILE : 0));
                int i = ptrs.length - 2;
                for (; i > 0; i--) {
                    tempType.setType(new CPointerType());
                    tempType.setQualifiers((((ICASTPointer) ptrs[i]).isConst() ? CPointerType.IS_CONST : 0) | (((ICASTPointer) ptrs[i]).isRestrict() ? CPointerType.IS_RESTRICT : 0) | (((ICASTPointer) ptrs[i]).isVolatile() ? CPointerType.IS_VOLATILE : 0));
    ...
    private static IBinding[] findBindingForContentAssist(ICASTFieldDesignator fd, boolean isPrefix) {
        ...
        if (blockItem instanceof IASTDeclarationStatement && ((IASTDeclarationStatement) blockItem).getDeclaration() instanceof IASTSimpleDeclaration) {
            ...
            while (declarationNode != null && !(declarationNode instanceof IASTSimpleDeclaration)) declarationNode = declarationNode.getParent();
        }
    ...
    static public boolean declaredBefore(IASTNode nodeA, IASTNode nodeB) {
        ...
        if (nodeA instanceof ASTNode) {
            ...
            } else // point of declaration for an enumerator is immediately after it enumerator-definition
            if (prop == IASTEnumerator.ENUMERATOR_NAME) {
