...
public class AggregateInitialization {
    ...
    private Cost checkElement(IType type, IValue initialValue, Cost worstCost) throws DOMException {
        ...
        if (initFromStringLiteral(nestedType, initializer)) {
            ...
            // will error in case we cannot determine the size
        ...
        Cost costWithoutElision = Conversions.checkImplicitConversionSequence(nestedType, initializer.getType(), initializer.getValueCategory(), UDCMode.ALLOWED, Context.ORDINARY, false);
        ...
        } else if (fInitializers[fIndex].isInitializerList() || !isAggregate(nestedType)) {
            // cannot elide braces
            // doesn't convert
            return costWithoutElision;
        } else {
            // braces are elided: need to check on subaggregates
    ...
    private static boolean isAggregate(IType type) {
        return (type instanceof ICPPClassType && TypeTraits.isAggregateClass((ICPPClassType) type) && !isSelfAggregate((ICPPClassType) type, null)) || type instanceof IArrayType;
    }
    ...
    private static ICPPBasicType getBasicTypeFromArray(IType type) {
        if (type instanceof IArrayType) {
            IType nested = SemanticUtil.getNestedType(((IArrayType) type).getType(), SemanticUtil.ALLCVQ | SemanticUtil.TDEF);
