...
public class MacroExpander {
    ...
    static final public int ORIGIN = OffsetLimitReachedException.ORIGIN_MACRO_EXPANSION;

    static final public TokenList EMPTY_TOKEN_LIST = new TokenList();
    ...
    public static final class ExpansionBoundary extends Token {
        ...
        public String toString() {
            //$NON-NLS-1$
            return "{" + (fIsStart ? '+' : '-') + fMacro.getName() + '}';
        }
    ...
    final public ILexerLog fLog;

    final public MacroDefinitionParser fDefinitionParser;

    final public CharArrayMap<PreprocessorMacro> fDictionary;
     ...
     */
    public TokenList expand(ITokenSequence lexer, final int ppOptions, PreprocessorMacro macro, Token identifier, boolean completionMode, ScannerContext scannerContext) throws OffsetLimitReachedException {
            ...
            switch(t.getType()) {
                ...
                case IToken.tIDENTIFIER:
                    ...
                    if (protect || (tracker != null && tracker.isDone())) {
                        ...
                        t.setType(CPreprocessor.tEXPANDED_IDENTIFIER);
                        result.append(t);
                    } else {
                        ...
                        TokenList replacement = new TokenList();
                        // start expansion
                        addSpacemarker(l, t, replacement);
                        ...
                        // end expansion
                        addSpacemarker(last, input.first(), replacement);
        ...
        for (Token t = replacement.first(); t != null; l = t, t = n) {
            ...
            switch(t.getType()) {
                    ...
                    if (idx < args.length) {
                        ...
                        if (isKind(n, IToken.tPOUNDPOUND)) {
                            ...
                            if (pasteArg1 != null) {
                                ...
                                // start token paste
                                addSpacemarker(result.last(), pasteArg1, result);
                            }
                        } else {
                ...
                case IToken.tPOUND:
                    ...
                    if (isKind(n, IToken.tPOUNDPOUND)) {
                        // start token paste, same as start stringify
                        pasteArg1 = generated;
                    } else {
                    ...
                    if (n != null) {
                        ...
                        if (generated != null) {
                            if (pasteNext && rest == null) {
                                // no need to mark spaces, done ahead
                                pasteArg1 = generated;
                            } else {
                                result.append(generated);
                                // end token paste
                                addSpacemarker(spaceDef0, spaceDef1, result);
                            }
                        }
                        if (rest != null) {
                            if (pasteNext) {
                ...
                case IToken.tCOMMA:
                    if (isKind(n, IToken.tPOUNDPOUND)) {
                        ...
                        if (isKind(nn, CPreprocessor.tMACRO_PARAMETER)) {
                            ...
                            if (idx == args.length - 1 && macro.hasVarArgs() != FunctionStyleMacro.NO_VAARGS && !isKind(nn.getNext(), IToken.tPOUNDPOUND)) {
            ...
            switch(t.getType()) {
                ...
                case IToken.tPOUNDPOUND:
                    if (isKind(n, CPreprocessor.tMACRO_PARAMETER)) {
                        idx = ((TokenParameterReference) n).getIndex();
                        // gcc-extension
                        if (isKind(l, IToken.tCOMMA) && macro.hasVarArgs() != FunctionStyleMacro.NO_VAARGS && idx == macro.getParameterPlaceholderList().length - 1 && !isKind(n.getNext(), IToken.tPOUNDPOUND)) {
                            result.set(2 * idx + 1);
                        } else {
                            result.set(2 * idx);
                        }
                    ...
                    if (pasteArg1 != null) {
                        ...
                        if (t != null) {
                            if (isKind(n, IToken.tPOUNDPOUND)) {
                                ...
                                result.append(t);
                                // end token paste
                                addSpacemarker(pasteArg2, n, result);
                            }
                        }
                    }
                    break;
