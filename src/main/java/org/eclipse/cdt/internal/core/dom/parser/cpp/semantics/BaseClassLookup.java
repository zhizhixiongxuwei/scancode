...
public class BaseClassLookup {
        ...
        if (baseClassScope == null) {
            ...
            try {
                ...
                if (data.qualified) {
                    ...
                    if (lookupName != null && lookupName.getPropertyInParent() == ICPPASTQualifiedName.SEGMENT_NAME && lookupName.getRoleOfName(false) == IASTNameOwner.r_definition) {
            ...
            if (grandBases != null && grandBases.length > 0) {
                ...
                for (int i = 0; i < grandBases.length; i++) {
                    ...
                    if (!(grandBaseScope instanceof ICPPClassScope))
                        continue;
                    BaseClassLookup baseInfo = lookupInBaseClass(data, (ICPPClassScope) grandBaseScope, grandBase.isVirtual(), root, infoMap, depth);
                    if (baseInfo != null)
    ...
    private IBinding[] collectResult(LookupData data, boolean asVirtualBase, IBinding[] result) {
        ...
        if (possibleAmbiguity) {
            ...
            if (result.length > 1) {
                data.problem = new ProblemBinding(data.getLookupName(), IProblemBinding.SEMANTIC_AMBIGUOUS_LOOKUP, result);
            }
        }
        for (int i = 0; i < fChildren.size(); i++) {
            BaseClassLookup child = fChildren.get(i);
    ...
    private IBinding[] collapseInjectedClassNames(LookupData data, IBinding[] result) {
        ...
        for (IBinding binding : result) {
            if (binding instanceof ICPPClassType && binding instanceof ICPPTemplateInstance) {
                ICPPTemplateDefinition specialized = (ICPPTemplateDefinition) ((ICPPTemplateInstance) binding).getSpecializedBinding();
                if (template == null) {
