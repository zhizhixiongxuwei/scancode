...
final public class ImplicitsAnalysis {
    ...
    public int getImplicitsToDeclareCount() {
        return (!hasDestructor ? 1 : 0) + (!hasConstructor ? 1 : 0) + (!hasCopyConstructor ? 1 : 0) + (!hasCopyAssignmentOperator ? 1 : 0);
    }
    ...
    private void analyzeMembers(ICPPASTCompositeTypeSpecifier compositeTypeSpecifier) {
        ...
        for (IASTDeclaration member : members) {
            ...
            if (spec instanceof IASTSimpleDeclSpecifier && ((IASTSimpleDeclSpecifier) spec).getType() == IASTSimpleDeclSpecifier.t_unspecified) {
                if (CharArrayUtils.equals(declName, name)) {
                    hasConstructor = true;
                    IASTParameterDeclaration[] params = ((ICPPASTFunctionDeclarator) dcltor).getParameters();
                    if (params.length != 0 && hasTypeReferenceToClassType(params[0]) && parametersHaveInitializers(params, 1)) {
