...
public final class CompositeValue implements IValue {
    ...
    // Initialized in the TranslationUnit.
    public static boolean sDEBUG;

    final public ICPPEvaluation evaluation;

    final public ICPPEvaluation[] values;
    ...
    static final public ThreadLocal<Set<ICPPClassType>> fCreateInProgress = new ThreadLocal<>() {
    ...
    public static IValue create(ICPPClassType classType, int nestingLevel) {
        ...
        try {
            if (sDEBUG && nestingLevel > 0) {
                ...
                "CompositeValue.create(" + ASTTypeUtil.getType(classType) + ", " + nestingLevel + ")");
                System.out.flush();
            }
    ...
    public void setSubValue(int position, ICPPEvaluation newValue) {
        if (position >= 0 && position < values.length) {
            ...
            CCorePlugin.log(IStatus.WARNING, //$NON-NLS-1$
            "Out-of-bounds access to composite value: " + position + " (length is " + values.length + //$NON-NLS-1$//$NON-NLS-2$
            ")");
        }
    }
