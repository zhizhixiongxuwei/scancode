...
public class PDOMFile implements IIndexFragmentFile {
    ...
    final public PDOMLinkage fLinkage;

    final public long record;

    // No need to make volatile, all fields of IndexFileLocation are final.
    public IIndexFileLocation location;

    // No need to make volatile, all fields of SignificantMacros are either final or atomically updated.
    public ISignificantMacros sigMacros;

    static final public int FIRST_NAME = 0;
    ...
    // size 3
    private static final int LINKAGE_ID = LOCATION_REPRESENTATION + Database.PTR_SIZE;

    // size 1
    private static final int FLAGS = LINKAGE_ID + 3;

    // long
    private static final int TIME_STAMP = FLAGS + 1;

    // long
    private static final int SOURCE_READ_TIME = TIME_STAMP + 8;

    // long
    private static final int CONTENT_HASH = SOURCE_READ_TIME + 8;
    ...
    // 9*PTR_SIZE + 3+1+8+8+8+4 = 68
    private static final int RECORD_SIZE = REPLACEMENT_HEADER + Database.PTR_SIZE;
    ...
    public PDOMFile(PDOMLinkage linkage, IIndexFileLocation location, int linkageID, ISignificantMacros macros) throws CoreException {
        ...
        if (locationString == null)
            ...
            CCorePlugin.createStatus(Messages.getString("PDOMFile.toInternalProblem") + location.getURI()));
    ...
    public void setLocation(IIndexFileLocation location) throws CoreException {
        String locationString = fLinkage.getPDOM().getLocationConverter().toInternalFormat(location);
        if (locationString == null) {
            throw new CoreException(CCorePlugin.createStatus(//$NON-NLS-1$
            Messages.getString("PDOMFile.toInternalProblem") + location.getURI()));
        }
    ...
    public void addNames(IASTName[][] names, YieldableIndexLock lock) throws CoreException, FailedToReAcquireLockException {
    ...
    private IIndexFragmentName createPDOMName(PDOMLinkage linkage, IASTName name, PDOMName caller) throws CoreException {
        ...
        try {
            ...
            if (pdomBinding != null) {
                final PDOMName result = new PDOMName(fLinkage, name, this, pdomBinding, caller, false);
                ...
                if (b instanceof IProblemBinding) {
                    ...
                    for (IBinding candidate : ((IProblemBinding) b).getCandidateBindings()) {
                        ...
                        if (pdomBinding != null) {
                            final PDOMName pdomName = new PDOMName(fLinkage, name, this, pdomBinding, caller, true);
    ...
    private IIndexFragmentName createPDOMMacroReferenceName(PDOMLinkage linkage, IASTName name, PDOMName caller) throws CoreException {
    ...
    public String toDebugString() {
        ...
        try {
            ...
            //$NON-NLS-1$
            buf.append(" (incomplete due to ").append(e.getClass().getName()).append(')');
        }
        return buf.toString();
    }
}
