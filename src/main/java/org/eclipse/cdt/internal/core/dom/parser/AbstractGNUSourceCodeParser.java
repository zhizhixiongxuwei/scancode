...
public abstract class AbstractGNUSourceCodeParser implements ISourceCodeParser {
    ...
    static final public ASTVisitor MARK_INACTIVE = new ASTGenericVisitor(true) {
    ...
    static final public int DEFAULT_DESIGNATOR_LIST_SIZE = 4;

    final public AbstractParserLogService log;

    final public IScanner scanner;

    final public ParserMode mode;
    ...
    private final IToken fetchToken(boolean skipInactive) throws EndOfFileException {
        try {
            ...
            if (olre.getOriginator() == OffsetLimitReachedException.ORIGIN_INACTIVE_CODE) {
                ...
                // declaredBefore().
                IASTName completionName = nodeFactory.newInactiveCompletionName(completionToken.getCharImage(), getTranslationUnit());
    ...
    protected void logThrowable(String methodName, Throwable e) {
        if (e != null) {
            if (log.isTracing()) {
                ...
                String.//$NON-NLS-1$
                format(//$NON-NLS-1$
                "Parser: Unexpected throwable in %s:%s::%s. w/%s", methodName, e.getClass().getName(), e.getMessage(), scanner);
    ...
    protected void logException(String methodName, Exception e) {
        if (!(e instanceof EndOfFileException) && e != null) {
            if (log.isTracing()) {
                ...
                String.//$NON-NLS-1$
                format(//$NON-NLS-1$
                "Parser: Unexpected exception in %s:%s::%s. w/%s", methodName, e.getClass().getName(), e.getMessage(), scanner);
                log.traceLog(message);
            }
            log.traceException(e);
        }
    ...
    public IASTTranslationUnit parse() {
        ...
        if (log.isTracing()) {
            ...
            String.//$NON-NLS-1$
            format(//$NON-NLS-1$
            "Parsed %s: %d ms%s. Ambiguity resolution: %d ms", name, t1 - t0, parsePassed ? "" : " - parse failure", //$NON-NLS-1$//$NON-NLS-2$
            System.currentTimeMillis() - t1);
            log.traceLog(message);
        }
        nullifyTranslationUnit();
        // Make the AST immutable.
        ast.freeze();
        return ast;
    }
    ...
    protected IASTCompoundStatement compoundStatement() throws EndOfFileException, BacktrackException {
        ...
        while (true) {
            ...
            try {
                ...
                // Don't hold on to the token while parsing namespaces, class bodies, etc.
                next = null;
                IASTStatement stmt;
    ...
    private IASTExpression buildExpression(IASTExpression left, BinaryOperator operator) {
        ...
        if (am != null) {
            if (unaryOp != 0) {
                result = createCastVsBinaryExpressionAmbiguity((IASTBinaryExpression) result, am.getTypeIdForCast(), unaryOp, am.getUnaryOperatorOffset());
            } else {
                assert false;
            }
        }
    ...
    private void addDeclaration(final IASTDeclarationListOwner parent, IASTDeclaration declaration, final boolean active) {
        if (!active) {
    ...
    protected abstract IASTDeclarator initDeclarator(IASTDeclSpecifier declSpec, DeclarationOptions option) throws EndOfFileException, BacktrackException, FoundAggregateInitializer;
    ...
    protected int enumBody(final IASTEnumerationSpecifier result) throws EndOfFileException, BacktrackException {
        ...
        // IToken.tLBRACE
        int endOffset = consume(IToken.tLBRACE).getEndOffset();
     ...
     */
    protected abstract Decl declSpecifierSeq(DeclarationOptions option, ITemplateIdStrategy strat) throws BacktrackException, EndOfFileException;
    ...
    protected boolean isAtStartOfStructuredBinding(IASTDeclSpecifier declSpecifier) {
        ...
        return LTcatchEOF(expectedBracketOffset) == IToken.tLBRACKET && LTcatchEOF(expectedBracketOffset + 1) == IToken.tIDENTIFIER;
    }

    protected abstract boolean maybeDeclaresNonStaticMemberOfSameClass(final DeclarationOptions option, final IASTDeclSpecifier declSpec, final IASTDeclarator dtor);
     ...
     */
    protected Decl declSpecifierSequence_initDeclarator(final DeclarationOptions option, boolean acceptCompoundWithoutDtor, ITemplateIdStrategy strat) throws EndOfFileException, FoundAggregateInitializer, BacktrackException {
    ...
    protected abstract IASTInitializer optionalInitializer(IASTDeclarator dtor, DeclarationOptions options) throws EndOfFileException, BacktrackException;
    ...
    protected IASTStatement parseDeclarationOrExpressionStatement(List<IASTAttributeSpecifier> attributeSpecifiers) throws EndOfFileException, BacktrackException {
        ...
        try {
            ...
            if (expressionStatement == null || (!foundSemicolon && isProblemDecl && node.contains(expressionStatement))) {
                if (isProblemDecl) {
    ...
    protected IASTStatement parseWhileStatement() throws EndOfFileException, BacktrackException {
        ...
        ((ASTNode) while_statement).setOffsetAndLength(startOffset, (while_body != null ? calculateEndOffset(while_body) : LA(1).getEndOffset()) - startOffset);
    ...
    protected void reconcileLengths(IASTIfStatement result) {
        ...
        while (current.getElseClause() instanceof IASTIfStatement) current = (IASTIfStatement) current.getElseClause();
        while (current != null) {
    ...
    protected IASTStatement parseCaseStatement() throws EndOfFileException, BacktrackException {
        ...
        if (lt1 == IToken.tELLIPSIS) {
            ...
            caseExpression = buildBinaryExpression(IASTBinaryExpression.op_ellipses, caseExpression, upperBoundExpression, calculateEndOffset(upperBoundExpression));
            lt1 = LT(1);
        }
        int lastOffset = 0;
        switch(lt1) {
    ...
    protected abstract IASTAlignmentSpecifier createAmbiguousAlignmentSpecifier(IASTAlignmentSpecifier expression, IASTAlignmentSpecifier typeId);
