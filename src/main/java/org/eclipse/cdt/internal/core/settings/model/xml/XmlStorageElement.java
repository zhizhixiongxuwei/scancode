...
public class XmlStorageElement implements ICStorageElement {
    ...
    static final public String[] emptyStringList = new String[0];
    ...
    public ICStorageElement fParent;

    public List<ICStorageElement> fChildList = new ArrayList<>();
    ...
    protected ICStorageElement[] getChildren(Class<XmlStorageElement> clazz, boolean load) {
        synchronized (fLock) {
            if (load)
                createChildren();
            ICStorageElement[] children = (ICStorageElement[]) java.lang.reflect.Array.newInstance(clazz, fChildList.size());
    ...
    public ICStorageElement[] getChildrenByName(String name) {
        synchronized (fLock) {
            ...
            for (ICStorageElement child : fChildList) if (name.equals(child.getName()))
                children.add(child);
