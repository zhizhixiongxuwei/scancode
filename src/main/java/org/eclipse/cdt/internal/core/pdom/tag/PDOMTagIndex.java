...
public class PDOMTagIndex {
    ...
    final public long ptr;

    public long rootRecord;

    public PDOMStringSet taggerIds;
    ...
    private long getIdRecord(String taggerId, boolean createIfNeeded) {
        ...
        if (db == null || taggerId == null || taggerId.isEmpty() || (taggerIds == null && !createIfNeeded)) {
            return 0L;
        }
        try {
    ...
    private boolean setTags(long binding_record, Iterable<ITag> tags) {
        ...
        for (ITag tag : tags) {
            ...
            if (dupTag != null)
                ...
                "Duplicate incoming tag for record " + binding_record + " from taggerId " + //$NON-NLS-1$
                tag.getTaggerId());
        }
        BTree btree = null;
        try {
