...
public class PDOMCPPFunction extends PDOMCPPBinding implements ICPPFunction, IPDOMOverloader, ICPPComputableFunction {
    ...
    private static final int FUNCTION_BODY = REQUIRED_ARG_COUNT + 2;

    /**
     * Offset of the function's declared type.
     */
    // Database.TYPE_SIZE
    private static final int DECLARED_TYPE = FUNCTION_BODY + Database.EXECUTION_SIZE;
    ...
    // No need for volatile, all fields of ICPPFunctionTypes are final.
    ...
    public void initData(ICPPFunctionType ftype, ICPPFunctionType declaredType, ICPPParameter[] params, IType[] exceptionSpec, ICPPExecution functionBody) {
        try {
            setType(ftype);
