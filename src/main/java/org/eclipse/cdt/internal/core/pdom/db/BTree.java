...
public class BTree {
    ...
    static final public int DELMODE_NORMAL = 0;

    static final public int DELMODE_DELETE_MINIMUM = 1;

    static final public int DELMODE_DELETE_MAXIMUM = 2;

    final public Database db;

    final public long rootPointer;
    ...
    private class BTNode {
        ...
        BTNode(long node) throws CoreException {
            ...
            int i = 0;
            while (i < MAX_RECORDS && getRecord(chunk, node, i) != 0) i++;
            keyCount = i;
        }
    ...
    private long deleteImp(long key, long nodeRecord, int mode) throws CoreException, BTreeKeyNotFoundException {
        ...
        if (mode == DELMODE_NORMAL)
            for (int i = 0; i < node.keyCount; i++) if (getRecord(node.chunk, node.node, i) == key) {
                keyIndexInNode = i;
                break;
            }
        if (getChild(node.chunk, node.node, 0) == 0) {
            ...
            if (keyIndexInNode != -1) {
                ...
                throw new BTreeKeyNotFoundException(//$NON-NLS-1$
                MessageFormat.//$NON-NLS-1$
                format(//$NON-NLS-1$
                Messages.getString("BTree.DeletionOnAbsentKey"), new Object[] { Long.valueOf(key), Integer.valueOf(mode) }));
            }
        } else {
            if (keyIndexInNode != -1) {
                ...
                switch(mode) {
                    case DELMODE_NORMAL:
                        ...
                        for (int i = 0; i < node.keyCount; i++) if (cmp.compare(getRecord(node.chunk, node.node, i), key) > 0) {
                            subtreeIndex = i;
                            break;
                        }
                        break;
                    ...
                    case DELMODE_DELETE_MAXIMUM:
                        ...
                        break;
                    default:
                        throw new CoreException(new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, IStatus.OK, Messages.getString("BTree.UnknownMode"), //$NON-NLS-1$
                        null));
                }
                BTNode child = node.getChild(subtreeIndex);
                if (child == null) {
                    throw new CoreException(new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, IStatus.OK, Messages.getString("BTree.IntegrityError"), //$NON-NLS-1$
                    null));
                }
    ...
    private void nodeContentCopy(BTNode src, int srcPos, BTNode dst, int dstPos, int length) {
        for (int i = length - 1; i >= 0; i--) {
            // this order is important when src == dst!
    ...
    private class InvariantsChecker implements IBTreeVisitor {
        ...
        @Override
        public void postVisit(long node) throws CoreException {
            depth--;
        }
        ...
        public void preVisit(long node) throws CoreException {
            ...
            if (indexFirstBlankKey != indexLastNonBlankKey + 1) {
                ...
                if (!full && !empty) {
                    ...
                    MessageFormat.//$NON-NLS-1$
                    format(//$NON-NLS-1$
                    Messages.getString("BTree.IntegrityErrorA"), new Object[] { Long.valueOf(node), Integer.valueOf(indexFirstBlankKey), Integer.valueOf(indexLastNonBlankKey) });
                }
            }
            ...
            if (childCount != 0 && childCount != keyCount + 1) {
                ...
                MessageFormat.//$NON-NLS-1$
                format(//$NON-NLS-1$
                Messages.getString("BTree.IntegrityErrorB"), new Object[] { Long.valueOf(node) });
            }
            ...
            if (keyCount < MIN_RECORDS || keyCount > MAX_RECORDS) {
                ...
                MessageFormat.//$NON-NLS-1$
                format(//$NON-NLS-1$
                Messages.getString("BTree.IntegrityErrorC"), new Object[] { Long.valueOf(node) });
            }
            ...
            if (childCount == 0) {
                ...
                if (depth != leafDepth.intValue()) {
                    valid = false;
                    //$NON-NLS-1$
                    msg += Messages.getString("BTree.IntegrityErrorD");
                }
            }
        }
    }
