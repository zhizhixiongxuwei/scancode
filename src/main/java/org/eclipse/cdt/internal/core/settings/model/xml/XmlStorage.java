...
public class XmlStorage implements ICSettingsStorage {
    ...
    public Map<String, InternalXmlStorageElement> fStorageElementMap = new HashMap<>();
    ...
    public void sanityCheck(ICStorageElement element) throws CoreException {
        if (element.getValue() != null && element.getValue().trim().length() > 0)
            throw ExceptionFactory.createCoreException(//$NON-NLS-1$ //$NON-NLS-2$
            "XmlStorage '" + element.getName() + "' has unexpected child Value: " + element.getValue());
        for (ICStorageElement child : element.getChildren()) {
            if (!MODULE_ELEMENT_NAME.equals(child.getName()))
                throw ExceptionFactory.createCoreException(//$NON-NLS-1$ //$NON-NLS-2$
                "XmlStorage '" + element.getName() + "' has unexpected child element: " + child.getName());
            if (child.getAttribute(MODULE_ID_ATTRIBUTE) == null)
                throw ExceptionFactory.createCoreException(//$NON-NLS-1$ //$NON-NLS-2$
                "XmlStorage '" + element.getName() + "' has storageModule child without moduleId");
        }
    }
    ...
    public ICStorageElement importStorage(String id, ICStorageElement el) throws UnsupportedOperationException {
        ...
        synchronized (fLock) {
            ...
            if (el instanceof XmlStorageElement) {
                ...
                for (String attrName : el.getAttributeNames()) storageEl.setAttribute(attrName, el.getAttribute(attrName));
                for (ICStorageElement child : el.getChildren()) storageEl.importChild(child);
                return storageEl;
            }
        }
    }
