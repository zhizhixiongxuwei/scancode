...
public class GeneratePDOMApplication implements IApplication {
    ...
    //$NON-NLS-1$
    static final public String EXPORT_PROJECT_PROVIDER = "ExportProjectProvider";

    static final public String DEFAULT_PROJECT_PROVIDER = ExternalExportProjectProvider.class.getName();
    ...
    private Object startImpl(IApplicationContext context) throws CoreException {
        ...
        String[] oldvals = null;
        if (!quiet) {
            oldvals = new String[] { System.getProperty(IPDOMIndexerTask.TRACE_ACTIVITY), System.getProperty(IPDOMIndexerTask.TRACE_PROBLEMS), System.getProperty(IPDOMIndexerTask.TRACE_STATISTICS) };
        ...
        try {
            ...
            // CoreException handled in start method
            ...
            for (IExtension extension : extensions) {
                ...
                for (IConfigurationElement element : ce) {
                    if (element.getName().equals(EXPORT_PROJECT_PROVIDER)) {
                        try {
                            IExportProjectProvider epp = (IExportProjectProvider) element.createExecutableExtension(//$NON-NLS-1$
                            "class");
        ...
        ProgressProvider pp = new ProgressProvider() {
            ...
            class IndexingStreamProgressMonitor extends StreamProgressMonitor {
                ...
                protected boolean shouldOutput() {
                    //$NON-NLS-1$
                    return taskName != null && taskName.equals(CCorePlugin.getResourceString("pdom.indexer.task"));
                }
            }
