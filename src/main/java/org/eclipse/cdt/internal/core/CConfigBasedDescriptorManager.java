...
final public class CConfigBasedDescriptorManager implements ICDescriptorManager {
    ...
    volatile static public CConfigBasedDescriptorManager fInstance;

    //$NON-NLS-1$
    public static final String NULL_OWNER_ID = "";

    static volatile public Map<String, COwnerConfiguration> fOwnerConfigMap;

    volatile static public ICProjectDescriptionListener fDescriptionListener;

    public Collection<ICDescriptorListener> fListeners = new CopyOnWriteArraySet<>();
    ...
    private static final COwnerConfiguration NULLCOwner = new COwnerConfiguration(NULL_OWNER_ID, //$NON-NLS-1$
    CCorePlugin.getResourceString("CDescriptorManager.internal_owner"));
    ...
    public void configure(IProject project, String id) throws CoreException {
        if (id.equals(NULLCOwner.getOwnerID())) {
            IStatus status = new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, -1, //$NON-NLS-1$
            CCorePlugin.getResourceString("CDescriptorManager.exception.invalid_ownerID"), (Throwable) null);
            ...
            try {
                ...
                if (dr != null) {
                    ...
                    if (dr.getProjectOwner().getID().equals(NULLCOwner.getOwnerID())) {
                        ...
                        IStatus status = new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, CCorePlugin.STATUS_CDTPROJECT_EXISTS, //$NON-NLS-1$
                        CCorePlugin.getResourceString("CDescriptorManager.exception.alreadyConfigured"), (Throwable) null);
                        throw new CoreException(status);
                    } else {
                        // already configured with same owner.
                        return;
                    }
                } else {
    ...
    public void convert(IProject project, String id) throws CoreException {
        ...
        if (dr == null)
            ...
            CCorePlugin.getResourceString("CConfigBasedDescriptorManager.0"));
        try {
            dr.fLock.acquire();
            dr = updateDescriptor(project, dr, id);
            dr.apply(true);
    ...
    public void runDescriptorOperation(IProject project, ICDescriptorOperation op, IProgressMonitor monitor) throws CoreException {
        ...
        if (dr == null)
            ...
            new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, -1, "Failed to create descriptor", null));
        try {
            dr.fLock.acquire();
    ...
    public void runDescriptorOperation(IProject project, ICProjectDescription des, ICDescriptorOperation op, IProgressMonitor monitor) throws CoreException {
        ...
        if (des.isReadOnly())
            throw new CoreException(new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, -1, CCorePlugin.getResourceString("CConfigBasedDescriptorManager.2"), //$NON-NLS-1$
            null));
        //create a new descriptor
        CConfigBasedDescriptor dr = loadDescriptor((CProjectDescription) des);
        if (dr == null)
            throw new CoreException(new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, -1, CCorePlugin.getResourceString("CConfigBasedDescriptorManager.3"), //$NON-NLS-1$
            null));
        op.execute(dr, monitor);
    ...
    private static CConfigBasedDescriptor loadDescriptor(CProjectDescription des) throws CoreException {
        ...
        if (cfgDes != null) {
            if (cfgDes.isReadOnly())
                throw ExceptionFactory.createCoreException(//$NON-NLS-1$
                CCorePlugin.getResourceString("CConfigBasedDescriptorManager.4"));
            return new CConfigBasedDescriptor(cfgDes);
        } else if (!des.isCdtProjectCreating()) {
            throw ExceptionFactory.createCoreException(//$NON-NLS-1$
            CCorePlugin.getResourceString("CConfigBasedDescriptorManager.5"));
        }
        return null;
    }
    ...
    public static synchronized COwnerConfiguration getOwnerConfiguration(String id) {
        ...
        if (config == null) {
            // no install owner, lets create place holder config for it.
            config = new COwnerConfiguration(id, //$NON-NLS-1$
            CCorePlugin.getResourceString("CDescriptorManager.owner_not_Installed"));
    ...
    private static void initializeOwnerConfiguration() {
        //$NON-NLS-1$
        IExtensionPoint extpoint = Platform.getExtensionRegistry().getExtensionPoint(CCorePlugin.PLUGIN_ID, "CProject");
        ...
        for (final ICDescriptorListener listener : fListeners) {
            SafeRunner.run(new ISafeRunnable() {
                ...
                public void handleException(Throwable exception) {
                    IStatus status = new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID, -1, CCorePlugin.getResourceString("CDescriptorManager.exception.listenerError"), //$NON-NLS-1$
                    exception);
