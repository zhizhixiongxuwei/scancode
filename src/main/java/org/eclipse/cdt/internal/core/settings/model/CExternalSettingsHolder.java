...
public class CExternalSettingsHolder extends CExternalSettingsContainer {
    ...
    public Map<ExtSettingMapKey, CExternalSetting> fSettingsMap;

    //$NON-NLS-1$
    static final public String ELEMENT_EXT_SETTINGS_CONTAINER = "externalSettings";
    ...
    CExternalSettingsHolder(ICStorageElement element) {
        ...
        if (externalSettingList != null && externalSettingList.size() != 0) {
            for (int i = 0; i < externalSettingList.size(); i++) {
                ...
                createExternalSetting(setting.getCompatibleLanguageIds(), setting.getCompatibleContentTypeIds(), setting.getCompatibleExtensions(), setting.getEntries());
            }
        }
    }
