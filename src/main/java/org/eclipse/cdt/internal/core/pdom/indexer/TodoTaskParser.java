...
public class TodoTaskParser {
    ...
    static final public Task[] EMPTY_TASK_ARRAY = {};

    final public char[][] tags;

    final public int[] priorities;

    final public boolean isTaskCaseSensitive;
    ...
    public TodoTaskParser(char[][] taskTags, int[] taskPriorities, boolean isTaskCaseSensitive) {
        ...
        for (int inc = order.length / 2; inc > 0; inc /= 2) {
            for (int i = inc; i < order.length; i++) {
                for (int j = i; j >= inc && taskTags[order[j - inc]].length < taskTags[order[j]].length; j -= inc) {
                    int temp = order[j];
                    order[j] = order[j - inc];
                    order[j - inc] = temp;
                }
    ...
    public Task[] parse(IASTComment[] comments) {
        ...
        for (IASTComment comment : comments) {
            ...
            if (location != null) {
                ...
                // once (bug 287181).
                if (locKeys.add(key)) {
                    parse(comment.getComment(), fileName, nodeOffset, location.getStartingLineNumber(), tasks);
                }
            }
        }
        if (tasks.isEmpty()) {
            ...
            nextTag: for (int j = 0; j < order.length; j++) {
                ...
                for (int t = 0; t < tagLength; t++) {
                    ...
                    if (sc != tc) {
                        ...
                        if (isTaskCaseSensitive || Character.toLowerCase(sc) != Character.toLowerCase(tc)) {
                ...
                if (i + tagLength < commentLength && isIdentifierPart(comment[i + tagLength - 1]) && isIdentifierPart(comment[i + tagLength])) {
                    continue nextTag;
                }
                //$NON-NLS-1$
                Task task = new Task(filename, i, i + tagLength, lineNumber, String.valueOf(tag), "", priorities[itag]);
                tasks.add(task);
