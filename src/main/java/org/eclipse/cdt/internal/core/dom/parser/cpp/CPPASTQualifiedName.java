...
public class CPPASTQualifiedName extends CPPASTNameBase implements ICPPASTQualifiedName, ICPPASTCompletionContext {
    ...
    public ICPPASTNameSpecifier[] fQualifier;

    public int fQualifierPos = -1;

    public ICPPASTName fLastName;

    public boolean fIsFullyQualified;
    ...
    public ICPPASTNameSpecifier[] getAllSegments() {
        ...
        int idx = 0;
        for (ICPPASTNameSpecifier nameSpecifier : getQualifier()) result[idx++] = nameSpecifier;
        if (fLastName != null)
    ...
    public IBinding[] findBindings(IASTName n, boolean isPrefix, String[] namespaces) {
        ...
        if (classQualifier != null) {
            ...
            if ((isDeclaration || isUsingDecl) && nameMatches(classQualifier.getNameCharArray(), n.getLookupKey(), isPrefix)) {
    ...
    private boolean isAddressOf() {
        return getParent() instanceof IASTIdExpression && getParent().getParent() instanceof IASTUnaryExpression && ((IASTUnaryExpression) getParent().getParent()).getOperator() == IASTUnaryExpression.op_amper;
    }
    ...
    private List<IBinding> filterClassScopeBindings(ICPPClassType classType, IBinding[] bindings, final boolean isDeclaration) {
        List<IBinding> filtered = new ArrayList<>();
        final boolean allowNonstatic = canBeFieldAccess(classType) || isAddressOf() || inUsingDecl();
        final IBinding templateDefinition = (classType instanceof ICPPTemplateInstance) ? ((ICPPTemplateInstance) classType).getTemplateDefinition() : null;
        for (final IBinding binding : bindings) {
            if (binding instanceof IField) {
                ...
                if (method.isImplicit())
                    continue;
                if (!isDeclaration) {
                    if (method.isDestructor() || method instanceof ICPPConstructor || (!allowNonstatic && !method.isStatic()))
                        continue;
                }
