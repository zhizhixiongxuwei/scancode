...
public abstract class ASTTranslationUnit extends ASTNode implements IASTTranslationUnit, ISkippedIndexedFilesListener {
    ...
    static final public IASTPreprocessorStatement[] EMPTY_PREPROCESSOR_STATEMENT_ARRAY = {};

    static final public IASTPreprocessorMacroDefinition[] EMPTY_PREPROCESSOR_MACRODEF_ARRAY = {};

    static final public IASTPreprocessorIncludeStatement[] EMPTY_PREPROCESSOR_INCLUSION_ARRAY = {};

    static final public IASTProblem[] EMPTY_PROBLEM_ARRAY = {};
    ...
    public final void freeze() {
        ...
        if (IndexFileSet.sDEBUG && fIndexFileSet != null && fASTFileSet != null) {
            long t = ((IndexFileSet) fIndexFileSet).getTimingContainsDeclarationNanos() + ((IndexFileSet) fASTFileSet).getTimingContainsDeclarationNanos();
            String forName = //$NON-NLS-1$
            fOriginatingTranslationUnit == null ? //$NON-NLS-1$
            "" : //$NON-NLS-1$
            " for " + fOriginatingTranslationUnit.getElementName();
            //$NON-NLS-1$
            System.out.println(String.format("IndexFileSet.containsDeclaration%s took %.2g ms", forName, t / 1.e6));
        }
    }
