...
public class PDOMASTProcessorManager {
    ...
    //$NON-NLS-1$
    static final public String EXTENSION_POINT = "PDOMASTProcessor";

    static final public List<PDOMASTProcessorDesc> processors = new ArrayList<>();

    static {
        // Load the extensions
        IConfigurationElement[] elements = Platform.getExtensionRegistry().getConfigurationElementsFor(CCorePlugin.PLUGIN_ID, EXTENSION_POINT);
        for (IConfigurationElement element : elements) processors.add(new PDOMASTProcessorDesc(element));
    }
