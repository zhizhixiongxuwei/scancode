...
public class CdtVariableManager implements ICdtVariableManager {
    ...
    static public CdtVariableManager fDefault;
    ...
    public String resolveValue(String value, String nonexistentMacrosValue, String listDelimiter, ICConfigurationDescription cfg) throws CdtVariableException {
        ...
        if (info != null)
            return CdtVariableResolver.resolveToString(value, getMacroSubstitutor(info, nonexistentMacrosValue, listDelimiter));
        return null;
    }
    ...
    public boolean isStringListValue(String value, ICConfigurationDescription cfg) throws CdtVariableException {
        try {
            CdtVariableResolver.resolveToStringList(value, //$NON-NLS-1$
            getMacroSubstitutor(getMacroContextInfo(getContextType(cfg), cfg), " ", null));
    ...
    public void checkVariableIntegrity(ICConfigurationDescription cfg) throws CdtVariableException {
        ...
        IVariableSubstitutor subst = new //$NON-NLS-1$
        CoreVariableSubstitutor(//$NON-NLS-1$
        info, //$NON-NLS-1$
        null, //$NON-NLS-1$
        "") {
    ...
    public String[] resolveStringListValues(String[] value, String nonexistentMacrosValue, String listDelimiter, ICConfigurationDescription cfg) throws CdtVariableException {
        IVariableContextInfo info = getMacroContextInfo(getContextType(cfg), cfg);
        if (info != null)
            return CdtVariableResolver.resolveStringListValues(value, getMacroSubstitutor(info, nonexistentMacrosValue, listDelimiter), true);
        return null;
    }
