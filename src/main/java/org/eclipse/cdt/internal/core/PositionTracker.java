...
public class PositionTracker implements IPositionConverter {
    ...
    static final public int MEMORY_SIZE = 48;

    static final public int NODE_MEMORY_SIZE = 32;

    public Node fAboveRoot = new Node(0, 0, 0);

    public PositionTracker fFollowedBy = null;
    ...
    private static class Node {
        ...
        // Sum of this and pos2 of parent yields pos2.
        private int fDeltaPos2;

        private int fPos1;

        // Length of text change (+ add, - remove)
        private int fChange;
        ...
        void addChars(int value2, int add, int fPos2) {
            int rel2 = value2 - fPos2;
            if (fParent != null) {
                // This may change both the parent and fDeltaPos2;
                fParent.balance();
            }
            ...
            if (rel2 < 0) {
                ...
                if (fLeft != null) {
                    ...
                    // Use modified parent pos
                    fLeft.addChars(value2, add, childPos2);
                    return;
                }
                // Modify delta pos
                addLeft(rel2 + fPos1, rel2 - add, add);
                return;
            }
        ...
        boolean removeChars(int firstChar2, int remove, int fPos2, boolean mustRemove) {
            ...
            assert fPos1 >= 0;
            if (fLeft != null) {
                // lhs is unaffected, undo
                fLeft.fDeltaPos2 += delAbove;
            }
            if (fRight != null) {
                // rhs is additionally affected.
                fRight.fDeltaPos2 -= delInside;
            }
            return true;
        }
        ...
        public void print(int i, PrintStream out, int parentOffset) {
            ...
            for (int j = 0; j < i; j++) //$NON-NLS-1$
            out.print("  ");
            //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            out.println(fPos1 + "<->" + parentOffset + " : " + fChange + (fColor ? " // red" : ""));
            if (fLeft != null) {
