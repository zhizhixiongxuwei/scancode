...
public class CASTExpressionList extends ASTNode implements IASTExpressionList, IASTAmbiguityParent {
    ...
    public CASTExpressionList copy(CopyStyle style) {
        ...
        for (IASTExpression expr : getExpressions()) copy.addExpression(expr == null ? null : expr.copy(style));
    ...
    public IASTExpression[] expressions = new IASTExpression[2];
    ...
    public boolean accept(ASTVisitor action) {
        ...
        for (int i = 0; i < exps.length; i++) if (!exps[i].accept(action))
            return false;
        if (action.shouldVisitExpressions) {
