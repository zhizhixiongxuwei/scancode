...
public class GeneratePDOM {
    ...
    public IExportProjectProvider pm;
    ...
    public final IStatus run() throws CoreException {
        ...
        if (cproject == null) {
            fail(MessageFormat.format(Messages.GeneratePDOM_ProjectProviderReturnedNullCProject, new Object[] { pm.getClass().getName() }));
            // Cannot be reached, inform the compiler
            return null;
        }
        IIndexLocationConverter converter = pm.getLocationConverter(cproject);
        if (converter == null) {
            fail(MessageFormat.format(Messages.GeneratePDOM_NullLocationConverter, new Object[] { pm.getClass().getName() }));
        }
        // Index the project
