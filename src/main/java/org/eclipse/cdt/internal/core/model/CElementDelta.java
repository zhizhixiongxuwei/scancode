...
public class CElementDelta implements ICElementDelta {
     ...
     * @see #getElement()
     */
    public ICElement fChangedElement;
     ...
     * @see #getFlags()
     */
    public int fChangeFlags = 0;

    /**
     * @see #getMovedFromElement()
     */
    public ICElement fMovedFromHandle = null;

    /**
     * @see #getMovedToElement()
     */
    public ICElement fMovedToHandle = null;
    ...
    protected CElementDelta find(ICElement e) {
        if (equalsAndSameParent(fChangedElement, e)) {
            // handle case of two jars that can be equals but not in the same project
            return this;
        }
    ...
    protected CElementDelta getDeltaFor(ICElement element) {
        ...
        equalsAndSameParent(getElement(), element))
            return this;
        if (fAffectedChildren.length == 0)
            return null;
    ...
    public String toDebugString(int depth) {
        ...
        if ((changeFlags & ICElementDelta.F_CHANGED_PATHENTRY_INCLUDE) != 0) {
            if (prev)
                ...
                buffer.append(" | ");
            //$NON-NLS-1$
            buffer.append("CHANGED TO PATHENTRY INCLUDE");
            prev = true;
        }
        if ((changeFlags & ICElementDelta.F_CHANGED_PATHENTRY_MACRO) != 0) {
            if (prev)
                ...
                buffer.append(" | ");
            //$NON-NLS-1$
            buffer.append("CHANGED TO PATHENTRY MACRO");
            prev = true;
        }
        if ((changeFlags & ICElementDelta.F_ADDED_PATHENTRY_LIBRARY) != 0) {
            if (prev)
                ...
                buffer.append(" | ");
            //$NON-NLS-1$
            buffer.append("ADDED TO PATHENTRY LIBRARY");
            prev = true;
        }
        ...
        if ((changeFlags & ICElementDelta.F_PATHENTRY_REORDER) != 0) {
            if (prev)
                ...
                buffer.append(" | ");
            //$NON-NLS-1$
            buffer.append("PATHENTRY REORDER");
            prev = true;
        }
