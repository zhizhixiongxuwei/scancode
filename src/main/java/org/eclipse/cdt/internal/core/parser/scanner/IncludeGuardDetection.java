...
public class IncludeGuardDetection {
    ...
    //$NON-NLS-1$
    static final public String ONCE = "once";
    ...
    private static Token skipPragmaOnce(Lexer l, CharArrayIntMap ppKeywords) throws OffsetLimitReachedException {
        ...
        // Save the state in case we don't find a "#pragma once".
        l.saveState();
        while (!quit) {
            switch(t.getType()) {
                case IToken.tPOUND:
                ...
                case IToken.tIDENTIFIER:
                    switch(ppKeywords.get(t.getCharImage())) {
                        case IPreprocessorDirective.ppPragma:
                            // Get the next token (expecting "once").
                            t = l.nextToken();
                            if (CharArrayUtils.equals(t.getCharImage(), ONCE)) {
                                foundPragma = true;
                                t = skipAll(l, Lexer.tNEWLINE);
                                if (// Just quit if we are not in an '#if' block.
                                !foundIf)
                                    quit = true;
                            }
                            break;
            ...
            if (skipPragmaOnce(l, ppKeywords).getType() == IToken.tPOUND) {
                ...
                if (t.getType() == IToken.tIDENTIFIER) {
                    ...
                    if (guard != null) {
                        ...
                        if (skipAll(l, Lexer.tNEWLINE).getType() == IToken.tPOUND && checkToken(l.nextToken(), Keywords.cDEFINE) && checkToken(l.nextToken(), guard)) {
                            l.consumeLine(ORIGIN_PREPROCESSOR_DIRECTIVE);
                            return guard;
                        }
                    }
    ...
    private static char[] findNotDefined(Lexer l) throws OffsetLimitReachedException {
        Token t;
        if (skipAll(l, IToken.tLPAREN).getType() == IToken.tNOT && checkToken(skipAll(l, IToken.tLPAREN), Keywords.cDEFINED)) {
            // Only a single parenthesis is allowed.
            t = l.nextToken();
