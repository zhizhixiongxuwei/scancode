...
public class DoxygenPreferenceAccess implements DoxygenConfiguration, DoxygenPreferences {
    ...
    private IEclipsePreferences preferences(IScopeContext scope) {
        return //
        Optional.ofNullable(scope.getNode(nodeQualifier())).map(//
        n -> n.node(nodePath())).filter(//
        IEclipsePreferences.class::isInstance).map(//
        IEclipsePreferences.class::cast).orElseThrow(() -> new //
        IllegalStateException(//
        NLS.//
        bind(//
        DoxygenCoreMessages.DoxygenPreferenceAccess_e_get_preferences, nodeQualifier(), nodePath())));
    }
